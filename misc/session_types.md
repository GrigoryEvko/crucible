...
**En-route transmissions** (PMY25 v4 §3.1): at the global-type level, the analog of a queued message is an "en-route transmission", syntactically `p ⇝_m q : {m_i(P_i) . G_i}`. This models "p has committed to send but q has not yet received." En-route transmissions appear only in runtime global types (not in static specifications). They are resolved when q receives: the en-route node is eliminated and G_i becomes the new global state. PMY25 v4 Thm 12 (soundness) proves: if Δ is associated with G and G can take a step, then G and Δ both step and the results are still associated.

**Balanced+** (Pischke, Masters and Yoshida, arXiv 2505.17676 version 4, Definitions 13 to 17). The depth of a role in a global type is the number of steps before the next action of that role. A global type is balanced when each role has a bounded depth in each global type that the protocol can get to (Definitions 13 and 14). A global type is balanced+ when it is balanced and the en-route count exists for each pair of roles (Definitions 16 and 17). The count exists when it is the same in all branches of each transmission, and no en-route message is below a recursion binder that loops. Theorem 13 of the paper uses balanced+ to show that each typing context associated with the global type is safe, deadlock-free and live.

*Bounded buffers do not give balanced+.* Balanced+ is a property of the shape of the global type, not of the runtime. A loop that can continue for ever without an action of one of its roles is not balanced, at each capacity of the channels. Equation (46) of the paper gives such a type. `fixy/session/Global.h` checks balanced+ at compile time, and `fixy/session/Liveness.h` requires it before it accepts a protocol as live.
...
After SpscSession.h ships (commit c2ceb86, 2026-04-27): the framework now has its first **production-shape** wired exercise. `sessions/SpscSession.h` composes PermissionedSessionHandle over the production `PermissionedSpscChannel` primitive that TraceRing / MetaLog / CNTP wire bytes / bg-pipeline drain channels all share. Two artifacts:
- `test/test_spsc_session.cpp` — round-trip 1024 ints between two jthreads via the typed-session API on a real `PermissionedSpscChannel<int, 1024, Tag>`. Validates the wiring composes; sizeof equality re-asserted under the test TU's build flags. (DOES NOT exercise PermSet evolution — plain payloads only by design; that path is exercised by `test_permissioned_session_handle.cpp`.)
- `bench/bench_spsc_session.cpp` — 2×2 matrix bare/typed × producer/consumer round-trip with pair-compare deltas isolating per-side overhead. Two-tier evidence: TIER A asm-identical compile-time witness (verifiable via `cmake --preset default -DCRUCIBLE_DUMP_ASM=ON` + grep through `build/asm-dump/bench_spsc_session.s`); TIER B timed measurement (informational at sub-ns scale).

What's still pending: no Crucible production CALLER (Vigil's recording path, BackgroundThread's drain, MerkleDag's TraceRing consumer, vessel_api's dispatch hot path) has switched to consume the typed view. The wiring exists; the production-side switchover is the SAFEINT-R31 / SEPLOG-INT-1 (#384 / #413) follow-up. K-series tasks #355-#358 (Vessel dispatch, KernelCache, InferenceSession, CNTP Raft) remain pending.

The framework is no longer a museum piece — it has one wired-in production-shape end-to-end exercise. But it doesn't yet have any production CALLER consuming the typed view; that is the next phase of work.
