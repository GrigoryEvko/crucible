...
No ML runtime proves its own correctness. They test. They fuzz. They hope. F\*X PROVES — at build time, without hardware, generating mathematical certificates that hold universally. Not "we tested 500 inputs." For ALL inputs. The build process is a formal verification pipeline. The binary is a theorem.

F\*X is Crucible's fork of the F\* proof assistant — 10× more powerful. Write, prove, extract to hardware-optimized C++/CUDA. The runtime is a theorem. No autotuning — SMT-optimal kernel synthesis for every (op, shape, device). Proved allocators. Proved fusion. Proof certificates persist in the Cipher and survive reincarnation.

**The four-layer proof architecture.** Every invariant is proved at the highest layer that can reach it. Each layer covers what the layers below cannot. Each layer's proofs are axioms for the layers above.

*Layer 4 — Z3 SMT Solver (universal theorems, all inputs).* Crucible integrates a Z3 fork enhanced with CaDiCaL dual-mode search, ProbSAT walker, on-the-fly self-subsumption, arena allocator — 85 commits, +13,700 lines ahead of upstream. Bitvector problems bit-blast to SAT where the enhanced solver runs 2-5× faster. Z3 proves: Arena alignment for ALL 2^192 (base, offset, align) combinations. fmix64 has no fixed points in ALL 2^64 inputs, avalanche ≥20 bits per input bit. SPSC bitmask equals modulo for ALL power-of-two capacities. Sweep-line non-overlap for ALL lifetime configurations. Kernel access safety for ALL (blockIdx, threadIdx). mul\_sat/add\_sat correctness for ALL 2^128 input pairs. Build integration: `crucible_verify` executable runs proofs; if any theorem fails, the build fails.

*Layer 3 — consteval (bounded model checking, compiler-as-verifier).* A `consteval` function runs inside the compiler's abstract machine — a SOUND interpreter where null deref, OOB, overflow, UAF, double-free, memory leak, and uninitialized read are all compile errors. `constexpr` Arena via `if consteval`: same algorithm, compile-time fully tracked by the compiler, runtime 2ns bump pointer. Consteval Philox-driven fuzzing: 500 random memory plans verified at compile time, all correct. Exhaustive finite-state model checking: SPSC protocol (81 states, BFS, deadlock-free), mode transitions (4 states × 3 events, no livelock). Cross-checks: serialization roundtrip, hash determinism, CSR consistency.

*Layer 2 — Reflection (structural proofs, GCC 16 with `-freflection`).* P2996 static reflection + P1306 expansion statements. `nonstatic_data_members_of(^^T)` iterates all fields at compile time. Four checks per field: `has_default_member_initializer` (InitSafe), offset sequence (no padding holes), `type_of` (TypeSafe — raw integers with ID-like names → compile error), `sizeof` (MemSafe). Auto-generation: `reflect_hash<T>()` that can NEVER forget a field, `reflect_serialize<T>()`, `reflect_compare<T>()`, `reflect_print<T>()`. Auto-SoA transformation and layout optimization via `define_class()`.

*Layer 1 — Type System (structural invariants, zero-cost).* Capability tokens: `fx::Alloc`, `fx::IO`, `fx::Block` — empty structs as parameters restricting effectful code to authorized contexts (`fx::Bg`, `fx::Init`, `fx::Test`). Thread-affinity phantom types: `FgTag`/`BgTag` on handles prevent cross-thread misuse at compile time. Typestate for mode transitions: `start_recording(Inactive) → Recording` compiles; `start_recording(Compiled)` — no overload exists. Refinement types: `NonZero<uint32_t>`, `InRange<0,7>`, `Positive<int64_t>`. Strong types for every ID and hash: `OpIndex`, `SlotId`, `NodeId`, `SchemaHash`, `ContentHash` — no raw integers for anything with meaning.

**SMT kernel engine.** F\*X doesn't autotune — it SOLVES. Hardware spec = SMT axioms. Kernel config = SMT variables. Cost function = SMT objective. For any (op, shape, device): one Z3 query, global optimum. Proved safe: no OOB, no bank conflicts, coalesced access, no register spill. CUTLASS tries 500 configs via benchmarking. Triton tries 50. F\*X solves the exact constraint system in 2-5 seconds. Shape-specific: GEMM 4096×4096×4096 gets different optimal config than GEMM 32×4096×4096. Every shape gets its proved-optimal kernel.

**Proved allocators.** CPU: jemalloc-like with size classes, thread-local caches — proved disjointness, bounded fragmentation. CUDA: pool allocator with static plans — proved non-overlap, proved OOM-impossible. Arena: bump-pointer for DAG metadata — proved alignment and no UAF via lifetime tracking.
...
**Z3 as proof-guided optimizer.** Z3 doesn't just verify — it GENERATES optimal solutions. Memory layout: given tensor lifetimes, sizes, alignments as constraints, minimize total footprint — the mathematically optimal memory plan. Kernel configurations: given hardware spec and kernel shape, find the (tile\_M, tile\_N, tile\_K, pipeline\_stages, warp\_count, vectorization\_width) that minimizes roofline execution time subject to ALL hardware constraints simultaneously. Topology: given measured link bandwidths and GPU capacities, find optimal TP×DP×PP factorization, GPU placement, communication algorithms — all simultaneously, as one optimization problem. The prover IS the optimizer.

**Algebraic verification.** Hash combining must be a monoid (associative, identity). ScalarType promotion must be a lattice. Gradient accumulation must be commutative. DAG transforms must be functorial. TensorMeta abstraction must form a Galois connection. C++ concepts encode these: `template<Monoid M> void parallel_reduce(...)`. consteval proves for test values. Z3 proves universally. The laws aren't academic — they're the correctness conditions for parallelism.

**Proof-carrying binaries.** Every Z3 proof produces an UNSAT certificate. Every consteval proof is a `static_assert`. Every reflection check is a compile-time assertion. The build generates a proof manifest — a cryptographic hash of all theorems. The Cipher stores this alongside DAG and weights. When a model loads on new hardware, Meridian re-proves hardware-specific properties (kernel configs, topology) but inherits universal proofs (hash correctness, protocol safety, arithmetic soundness) from the Cipher. Proofs survive reincarnation. When DAG fragments are spliced (L10), proofs compose — same sub-computation → same proof → reuse across models.

**The remaining gap:** memory ordering on real hardware. `acquire`/`release` correctness depends on the CPU's memory model. No compile-time mechanism in C++ can prove this. TSan at runtime. That's the one thing F\*X cannot speak.

**Crucible's role at L0:** prove everything that CAN be proved before execution. Generate mathematical certificates that hold universally. Solve optimization problems (kernels, topology, memory) that other runtimes search. Extract to C++/CUDA with proved properties. Persist proof certificates in the Cipher for reincarnation.

**L0 connects upward:** F\*X's SMT engine generates optimal kernel configs (L2). F\*X's proved allocators back the memory system (L3). F\*X's protocol proofs validate dispatch logic (L4). F\*X's topology optimization feeds distribution (L13). F\*X's calibration model feeds Meridian (L15). Every layer above builds on F\*X's certified foundations.
...
Crucible reads CUPTI after every compiled kernel — not for autotuning (F\*X at L0 already found the proved-optimal configuration via SMT) but for calibration: comparing actual measured time against F\*X's roofline prediction to compute correction factors that absorb cache effects, warp scheduler efficiency, TLB pressure, and instruction-level overhead. These corrections flow to Meridian (L15) and make predictions ±3-5% accurate. When CUPTI reveals a systematic gap (kernel class consistently 20% slower than predicted), Meridian refines the correction, and every future prediction for that class improves automatically.
...
**Crucible's role at L1:** observe the body's state (CUPTI counters, thermal sensors, power draw, health metrics), feed hardware measurements to Meridian (L15) for calibration, modulate power for cost efficiency, monitor health for fault prediction and pre-emptive migration, abstract over vendor differences so every layer above is hardware-agnostic, and report body health to the Keeper for mesh-wide coordination.

**L1 connects upward:** L1 measurements flow to Meridian (L15) for kernel calibration (correction factors that make F\*X's roofline predictions ±3-5% accurate) and topology optimization. F\*X (L0) uses hardware specs as SMT axioms to generate proved-optimal kernels at L2. A GPU that's thermally throttling at L1 gets reduced batch size at L13. Hardware state propagates upward through every layer.
...
**F\*X SMT kernel engine** (L0) replaces autotuning entirely. Current autotuners (Triton, AutoTVM) try N random configurations and pick the fastest — slow (thousands of trials), finds local optima. F\*X SOLVES the exact constraint system:

- Hardware spec = SMT axioms (SM count, shared memory, register file, bandwidth, FLOPS from Meridian calibration)
- Kernel config = SMT variables (tile\_M, tile\_N, tile\_K, pipeline\_stages, warp\_count, vectorization\_width)
- Cost function = SMT objective (minimize roofline execution time)
- Subject to ALL hardware constraints simultaneously: shared memory ≤ limit, registers ≤ 255, occupancy sufficient, no bank conflicts, coalesced access

One Z3 query. Global optimum. Proved safe: no OOB, no bank conflicts, coalesced access, no register spill. Shape-specific: GEMM 4096×4096×4096 gets different optimal config than GEMM 32×4096×4096. Every shape gets its proved-optimal kernel. CUTLASS tries 500 configs. Triton tries 50. F\*X solves. CUPTI validates predictions post-hoc; corrections feed back into Meridian (L15) calibration.
...
**Crucible's role at L2:** compile kernels from parameterized templates per device using F\*X's SMT-optimal configurations (L0), cache compiled kernels through content-addressing, bake deterministic Philox RNG into any kernel needing randomness, schedule independent kernels across CUDA streams for parallelism, fuse adjacent ops to eliminate memory round trips, persist the KernelCache as part of the Cipher for reincarnation.

**L2 connects upward:** L3 (memory) determines tensor addresses — L2 kernels are compiled with pre-baked pointer arguments from L3's memory plan. No dynamic allocation at kernel launch time. L4 (operations) determines WHAT to compile — L2 determines HOW.

**L2 connects downward:** kernel variants are selected and evolved based on L1 hardware counters. The hardware shapes the compilation; the compilation shapes the hardware utilization. A closed loop.

**L2 connects to L7 (DAG):** the KernelCache is indexed by content_hash from RegionNodes in the Merkle DAG. Each compilable region maps to one or more fused kernels. The DAG is the specification; L2 is the implementation. Different DAG branches may produce different content_hashes (different ops) that require different kernels — all cached, all hot-swappable.
...
Crucible eliminates the allocator from the hot path entirely. The mechanism is a **static memory plan** computed offline by the background thread from the dataflow graph (L6).
...
**Aliased tensors** (views, transposes, reshapes, slices, expand, in-place operations) share underlying storage. The alias edges in the DFG (L6) identify exactly which tensors point to the same memory — the BackgroundThread's PtrMap detects when two ops' output data_ptrs are identical (alias) vs when an op's input data_ptr matches a previous op's output (dataflow). The memory plan assigns ONE offset for the base storage; all aliases reference it with different (offset, sizes, strides) tuples. Views are zero-cost at both plan time and runtime — they're metadata, not memory.

**Dynamic shape adaptation.** When shapes change (different batch size, different sequence length, dynamic-length inputs), the memory plan must recompute. The new plan is built by the background thread and swapped atomically at the next iteration boundary — same mechanism as DAG branch activation (L7). The foreground thread never sees a partially-updated plan. If the new plan requires more memory than available: Crucible either (a) falls back to eager execution for that iteration while replanning, or (b) activates recomputation for the largest tensors to fit within budget.

**Automatic activation checkpointing.** The DFG knows exactly which tensors are produced in the forward pass and consumed in the backward pass. The memory plan knows each tensor's size. L2 knows each tensor's recomputation cost (kernel timing for the producer op). The tradeoff is explicit:
...
**Crucible's role at L3:** compute static memory plans from DFG lifetimes, replace dynamic allocation with pointer arithmetic, handle views/aliases through DFG edge tracking, adapt plans when shapes change, auto-checkpoint from measured cost tradeoffs, prevent OOM through prediction and adaptation, balance memory strategy per device.

**L3 connects upward:** L4 (operations) produces tensors that L3 places. L5 (tensor metadata) provides shapes and strides that determine sizes. Shadow handles (L5) carry the pre-computed offset as their storage pointer — the shadow IS the memory plan made concrete.

**L3 connects downward:** L2 kernels are compiled against L3's offsets — kernel arguments include pre-baked pointers, not allocation calls. L1 hardware capacity determines the memory plan's budget.

**L3 connects to L6 (graphs):** the DFG's lifetime intervals ARE the memory plan's input. Alias edges determine shared storage. The forward edge structure determines death times. The graph and the memory plan are two lenses on the same information.
...
**Operation-level matrix structure discovery.** Not every dense matmul needs to be dense. Crucible observes weight matrices through the latent space (L5) and categorizes them per layer:
...
These replacements are DAG branches (L7): old arm with dense matmul, new arm with the cheaper variant. Quality is verified (run both arms, compare outputs). If the cheap variant's error is within tolerance: commit. If not: discard. The model doesn't know or care — it sees the same input/output shapes. The internal computation changed.

**Communication ops are just ops.** The Dispatcher intercepts c10d operations — all_reduce, all_gather, reduce_scatter, broadcast, all_to_all — identically to compute ops. They get the same schema_hash, TensorMeta recording, timing measurement. This is what enables Crucible to optimize communication at higher layers (L13). A communication op that's slow gets the same treatment as a compute kernel that's slow: measure, diagnose, try alternatives, keep what works. The ring all_reduce can be replaced with a tree all_reduce. The all_gather can overlap with computation. All discovered from the same recording pipeline.
...
There is no training or inference at L4. The fallback is the same function. In RECORD mode: record and execute eagerly — works for both training and inference. In COMPILED mode: return shadow handles — works for both. The distinction between "training" and "inference" exists only in what DATA enters the pipeline and whether a backward pass follows. L4 doesn't know or care. The same op interception, the same recording, the same compiled execution. A model in Crucible doesn't switch modes — it just processes whatever computation Python describes.

**Crucible's role at L4:** intercept every operation via DispatchKey, record complete metadata (schema, shapes, scalars, tensors, scope, callsite) in RECORD mode, return shadow handles in ~2ns in COMPILED mode, detect divergence with graduated pre-emptive warnings, discover per-layer matrix structure for cheaper alternatives, treat communication and compute ops uniformly, produce the event log that feeds the Cipher.

**L4 connects upward:** operations produce tensors with metadata (L5). Sequences of operations form the dataflow graph (L6). The recorded metadata per op IS the input to graph construction — schema_hash becomes the node identity, TensorMeta becomes the edge label, data_ptr tracking builds the DFG edges.

**L4 connects downward:** each operation maps to a compiled kernel (L2) that runs in planned memory (L3) on measured hardware (L1). The full chain: L4 says WHAT to compute, L2 says HOW to compute it, L3 says WHERE the data lives, L1 says WHICH hardware executes it.

**L4 connects to L7 (DAG):** the op sequence IS the DAG's content. The schema_hash sequence IS the implicit guard in COMPILED mode. The scalar values ARE compilation constants baked into kernels. The scope_hash IS the hierarchical structure that organizes the DAG into module-level regions. Everything recorded at L4 feeds into the DAG at L7 — L4 is the mouth of the organism, L7 is the digestive system.
...
**Crucible's role at L5:** decouple tensor metadata from tensor data via shadow handles, maintain full TensorMeta in MetaLog for recording, support dense and sparse tensor formats, observe the latent space for dimensionality analysis, detect representation pathologies, enable steering and augmentation, provide tensor provenance through DFG ancestry, present a mode-agnostic interface where training and inference produce identical shadow handles.

**L5 connects upward:** tensors are the edges of the graph (L6). TensorMeta at L5 determines edge properties (shapes, dtypes, devices). Shadow handles at L5 are what COMPILED mode returns to Python — they ARE the compiled execution made visible to the user.

**L5 connects downward:** tensor shapes determine memory plan sizes (L3). Tensor dtypes and layouts determine kernel selection (L2). Tensor device_type determines which hardware backend compiles the kernel (L1).

**L5 connects to L9 (layers):** latent space observations — effective rank, dead dimensions, representation collapse, steering vectors — feed into layer-level decisions. Which layers need bottlenecks, which heads are dead, which layers are redundant. The tensor data at L5 is the measurement; the architectural action at L9 is the response.
...
A sequence of operations (L4) with tensor metadata (L5) becomes a graph. The **TraceGraph** is a bidirectional CSR (Compressed Sparse Row) property graph built by the background thread from one iteration of recorded ops. Nodes are operations. Edges are relationships between operations. Two kinds of edges:
...
The graph is the foundation for everything above. Memory plans (L3) read lifetimes from forward edges. Kernel fusion (L2) reads producer-consumer chains. The Merkle DAG (L7) reads op sequences and computes content hashes. Layer-level analysis (L9) reads scope_hash groupings. Architecture mutation (L10) reads the full graph topology.
...
Once the iteration boundary is confirmed, the background thread knows exactly where one iteration ends and the next begins. The completed iteration's ops become a TraceGraph. The TraceGraph becomes a RegionNode in the Merkle DAG (L7). The cycle repeats.
...
**Self-referential optimization.** Crucible's own calibration loop is a cycle: run kernel → measure CUPTI → update Meridian correction factor → F\*X re-verifies predictions → measure again. This can be represented as a LoopNode with termination "until prediction error < ε." The meta-calibration loop is compiled just like any other loop. Crucible optimizes the loop that calibrates Crucible.
...
**Crucible's role at L6:** build bidirectional CSR property graphs from recorded ops via single-pass DFG/alias construction, detect iteration boundaries from schema_hash signatures, provide lifetime/topology information to L3 (memory) and L2 (fusion/scheduling), extend the flat DAG with LoopNodes for cyclic computation (recurrence, convergence, cross-iteration pipelining, nested training loops), fix execution order for deterministic replay.

**L6 connects upward:** the graph IS the input to the Merkle DAG (L7). Each confirmed iteration's TraceGraph becomes a RegionNode. LoopNodes become higher-level DAG constructs. The graph topology determines content_hash (which determines kernel cache lookup, guard checking, branch structure).

**L6 connects downward:** DFG edges determine memory lifetimes (L3). Producer-consumer chains determine fusion candidates (L2). Edge structure determines CUDA stream scheduling (L2). Alias edges determine shared storage (L3).

**L6 connects laterally:** the graph is the bridge between the recording pipeline (L4, L5) and the compilation/optimization pipeline (L7, L8, L9). Below L6, everything is per-op. Above L6, everything is per-graph or per-region. L6 is where individual operations become structured computation.
...
**BranchNodes are the mechanism for everything that changes.** Architecture mutation (L10): add a layer → new branch arm with extra ops. Attention replacement (L9): swap an attention head with convolution → new branch arm with different ops. Hyperparameter change (L11): different learning rate → different scalar values → different content_hash for the optimizer region → new branch arm. Continuous learning (L14): parameter update changes model behavior → if the new behavior produces a different op sequence, new branch arm. Every form of adaptation is a branch. Every branch is versioned. Every branch is rollbackable.
...
- Changing the communication topology (adaptive topology at L13)
...
This enables cross-iteration optimization (pipeline iteration N+1's forward with iteration N's optimizer step via double-buffering), compiled recurrence (RNNs as a single compiled loop body, no Python per timestep), convergence-based execution (DEQ fixed-point layers, diffusion denoising — stop when converged, not after a fixed count), and self-referential calibration (Crucible's own calibration loop as a LoopNode). Detailed mechanics in L6.

**Crucible's role at L7:** maintain the content-addressed Merkle DAG as the central computation structure, compute content and merkle hashes for cache lookup and change detection, manage branches for dynamic behavior and all forms of adaptation, integrate LoopNodes for cyclic computation within the acyclic hash framework, provide atomic swaps for zero-downtime activation/rollback, serve as model format (training=inference), serve as audit trail (cryptographic provenance), enable version control semantics for computation.

**L7 connects upward:** L8 (tokens) and L9 (layers) and L10 (models) all modify the DAG — inserting token merging nodes, swapping attention mechanisms, growing layers. Every optimization above L7 is expressed as a DAG transformation.

**L7 connects downward:** the DAG indexes into the KernelCache (L2) via content_hash. The DAG's op sequences define the memory plan (L3) via the TraceGraph (L6). The DAG's guard values are checked by the fallback (L4).

**L7 connects to everything:** the Merkle DAG is the spine of the organism. L1-L6 feed observations into it. L8-L16 read from it and transform it. It is simultaneously the specification, the cache key, the guard, the version history, the deployment artifact, and the audit trail. Every other layer exists in relation to the DAG.
...
Crucible observes information density at runtime through L5 (tensor observations) and L6 (graph structure). Three mechanisms, from proven to speculative:
...
**Crucible's role at L8:** measure per-token information density (similarity, convergence, entropy), merge low-information tokens to reduce sequence length, exit early for converged tokens, apply quadtree decomposition for adaptive patching, handle variable-length batching at the memory/kernel level, apply per-token precision, extend adaptive tokenization across modalities (video, audio, time series).

**L8 connects upward:** fewer tokens flowing through layers means L9 (layer operations) runs faster — attention is O(n²), so token reduction is quadratically amplified. Token merging decisions feed into L9's per-head analysis (a head that only processes merged tokens might be replaceable with something even cheaper). Adaptive token counts feed into L10 (model architecture) — a model that processes variable-length inputs might evolve a different architecture than one with fixed-length inputs.

**L8 connects downward:** token merging/exit decisions are BranchNodes in the DAG (L7). The merged/exited token sequences produce different TraceGraphs (L6) with different memory plans (L3) and different compiled kernels (L2). Variable-length kernels require L2 to compile ragged variants. Token count affects memory plan size at L3.

**L8 connects to L5 (tensors):** latent space observations at L5 (representation similarity, convergence delta) are the MEASUREMENTS that drive L8 decisions. L5 observes; L8 acts. The token is both a data carrier (L5) and a compute allocation unit (L8).
...
Crucible observes each layer independently through L5 (latent space measurements) and L6 (graph structure per scope_hash). Every layer has a scope_hash from the nn.Module hierarchy: `model.layers.7.self_attn` is a different scope than `model.layers.7.ffn`. Crucible groups ops by scope and analyzes each group separately. What it finds is that layers are NOT uniform — they do different things, they carry different amounts of information, and they need different computational strategies.
...
**Iterative message passing for routing heads.** The 10-15% of heads that genuinely need content-based routing don't necessarily need O(n²) attention. Crucible can replace them with LoopNodes (L6) that do iterative local message passing:
...
- **K-FAC factors**: the Fisher information matrix F ≈ E[gg^T] can be approximated per-layer as A ⊗ G (Kronecker product of input activations covariance and gradient covariance). Crucible already has both: activations are in the MetaLog (L5), gradients flow through the backward trace (L6). The K-FAC inverse gives the natural gradient direction: F⁻¹g, where g is the standard gradient. Per-layer matrix inversions, tractable and cached.
...
**Adaptive bottlenecks from latent space measurement.** L5 measures the effective rank of activations per layer. L9 acts on this: if layer 7 has effective rank 600 in a 4096-dim model, insert a bottleneck:
...
**Layer growing.** Crucible monitors training dynamics through L9 measurements: loss plateau detection (loss hasn't decreased significantly in N steps), gradient analysis (gradients are small everywhere — the model may lack capacity). When capacity seems insufficient:
...
**Layer pruning.** The inverse of growing. Crucible detects redundant layers through L5's CKA analysis (adjacent layers with CKA > 0.95 compute nearly the same function) and L9's gradient analysis (layers with near-zero gradient norms are converged or dead). Pruning a layer:
...
**Width mutation.** Beyond adding/removing layers (depth), Crucible can adjust the hidden dimension per layer. L5 measures effective rank. L9 inserts bottlenecks. L10 goes further: permanently change the hidden dimension.
...
**Live surgery on a running Vigil.** Because continuous learning (L14) means the Vigil is always running, architectural changes happen live:
...
**Crucible's role at L10:** grow layers when capacity is needed, prune layers when they're redundant, adjust hidden dimensions per layer from measured effective rank, evolve activation functions per layer, enable progressive growing from small-to-large during training, compose models by DAG splicing, support evolutionary architecture search across model populations, perform live architecture surgery on running models.

**L10 connects upward:** architecture mutations change the training dynamics (L11) — a deeper model has different gradient flow, different curvature, different optimal learning rate. Model composition connects to L13 (distribution) — composed models may span different devices (vision encoder on GPU A, language model on GPU B, with the adapter bridging them).

**L10 connects downward:** every architectural change is a DAG branch (L7). New layers create new RegionNodes with new content_hashes, requiring new compiled kernels (L2) and new memory plans (L3). The full compilation pipeline runs for each mutation, but the KernelCache means only genuinely new computations need compilation — shared sub-structures are cache hits.

**L10 connects to L9 (layers):** L9 replaces individual mechanisms within layers (heads, activations, losses). L10 operates on entire layers (add, remove, resize). They compose: L9 replaces all attention heads in layer 7 with convolutions → L10 recognizes that layer 7 is now structurally different from a transformer layer → L10 might split it into a separate specialized block or merge it with adjacent layers that have similar structure. L9 is microsurgery; L10 is macrosurgery.
...
**Meta-gradients: differentiating through the training loop.** The DAG captures everything: forward pass, loss computation, backward pass, optimizer step. All of these are ops recorded by L4, structured by L6, compiled by L2. The training loop is just more computation in the DAG. And computation in the DAG is differentiable.
...
**Per-layer learning rate from curvature.** L9 computes Hessian-vector products periodically. From these, Crucible extracts per-parameter curvature estimates. The optimal learning rate for parameter θ_i is proportional to 1/H_ii (inverse of the Hessian diagonal at that parameter). Parameters in sharp regions of the loss landscape need small steps. Parameters in flat regions can take large steps.
...
K-FAC approximates F per-layer as a Kronecker product: F_layer ≈ A ⊗ G, where A = E[a·a^T] (input activation covariance) and G = E[g·g^T] (output gradient covariance). Crucible already has both: activations are recorded in MetaLog (L5), gradients flow through the backward trace (L6). The Kronecker inverse is tractable: (A ⊗ G)⁻¹ = A⁻¹ ⊗ G⁻¹. Two per-layer matrix inversions (d_in × d_in and d_out × d_out), cached and recomputed periodically.
...
**Curriculum learning from loss visibility.** Crucible sees the loss per sample (it's in the trace — the loss op's output tensor, observable at L5). It can compute per-sample difficulty:
...
- Add an auxiliary contrastive loss at layer 6 (L9 local losses) and weight it against the main loss.
- Add a regularization term that penalizes representation collapse (from L5's CKA measurements).
...
**L11 connects upward:** training optimization feeds L12 (data pipeline) — curriculum learning requires controlling data ordering. Hyperparameter evolution feeds L13 (distribution) — per-island learning rates in DiLoCo, per-device batch sizes. Optimizer evolution feeds L14 (lifecycle) — a continuously learning model needs an optimizer that adapts as the data distribution shifts.

**L11 connects downward:** all training modifications are DAG branches (L7). Meta-gradient computation uses the TraceGraph (L6) for backward-of-backward traversal. K-FAC uses activations from MetaLog (L5) and gradients from the backward trace. Precision decisions create new compiled kernels (L2) with different dtype specializations.

**L11 connects to L9 (layers):** L9 determines the gradient strategy per layer (backprop, K-FAC, synthetic, skip). L11 determines the overall training dynamics (learning rate, momentum, curriculum, loss). They compose: L9 says "layer 7 should use K-FAC" and L11 says "the K-FAC learning rate for layer 7 should be 3e-4 based on the meta-gradient." L9 is the local strategy; L11 is the global strategy.
...
Crucible absorbs the data pipeline into the organism. The boundary between "data loading" and "training" dissolves because Crucible sees both sides: the GPU utilization (from L1 hardware monitoring) and the data consumption pattern (from the TraceGraph at L6, which includes the first op of each iteration — typically a tensor receive or copy).
...
Manifold Mixup is proven to improve generalization, calibration, and robustness. It works because the latent manifold is smoother than the input space — interpolation in latent space is more semantically meaningful than interpolation in pixel space. Crucible implements it as a DAG modification at L7: insert an interpolation node at layer K, branch between the normal forward and the mixed forward, compute loss for both. The interpolation layer K is chosen by L5's measurement: the layer where representations are most linearly separable (measured by linear probe accuracy) is the best interpolation point.

**Representation steering at inference.** L5 identifies direction vectors in the latent space that correspond to behaviors (truthfulness, creativity, toxicity, language). L12 applies them: at inference time, add α × direction_vector to the hidden state at the optimal layer. No weight changes. No fine-tuning. One vector addition per layer, ~O(d) cost, measurable behavior modification.
...
**Representation monitoring for distribution shift.** In production, the input distribution changes over time (new user behavior, seasonal patterns, emerging topics). Crucible monitors the distribution of latent representations: if the activations at layer K shift significantly from the training distribution (measured by KL divergence or MMD between current batch and a reference distribution), that's a distribution shift signal. Response options at higher layers: trigger continuous learning (L14) to adapt, alert the operator, increase redundancy (L13) because the model's reliability has decreased.

**Crucible's role at L12:** integrate data pipeline with training through backpressure and GPU-side augmentation, implement curriculum learning data ordering from L11's difficulty measurements, enable latent-space augmentation (Manifold Mixup) via DAG modification, apply representation steering for controllable inference, monitor for distribution shift through latent space statistics.

**L12 connects upward:** data ordering feeds into training dynamics (L11). Distribution shift detection feeds into continuous learning (L14). Steering vectors feed into the deployment lifecycle (L14).

**L12 connects downward:** GPU-side augmentations are DAG ops compiled at L2, stored in the memory plan at L3. Backpressure is driven by L1 hardware monitoring (GPU idle time). Latent augmentation inserts nodes in the DAG (L7) using representations measured at L5.
...
Same Vigil. Four different compiled implementations. Each proved-optimal for its Relay's body via F\*X (L0) SMT solving + Meridian (L15) calibration. Content-addressing handles this naturally: content_hash is the same (same ops, same shapes) but the KernelCache key includes device_capability, so each Relay has its own compiled kernel for the same content.
...
- **Hierarchical DiLoCo**: within a node (NVLink, ~900 GB/s): sync every step. Across a rack (InfiniBand, ~400 GB/s): sync every 5 steps. Across the ocean (WAN, ~1 GB/s): sync every 50 steps. Three nested LoopNodes (L6), H automatically tuned per level from measured latencies.
...
**Crucible's role at L13:** form the Canopy — the masterless mesh of Keepers. Support heterogeneous multi-vendor compute through per-Relay compilation, distribute work via LOR based on measured throughput, use UCX/OpenMPI for cross-vendor communication, adapt network topology from continuous N×N latency probing, provide RAID-like redundancy as the hot Cipher, handle spot eviction and dynamic Relay join/leave, enhance DiLoCo with adaptive sync/selective sync/compression/async outer sync, auto-tune 5D parallelism from measured per-dimension costs.

**L13 connects upward:** the Canopy feeds the lifecycle (L14) — a fault-tolerant mesh means the Vigil runs continuously without fear of hardware failure. DiLoCo's island structure feeds the ecosystem (L16) — federated learning is DiLoCo + differential privacy.

**L13 connects downward:** per-Relay compilation uses L2 (kernels) and L1 (hardware profiles). Communication ops are recorded at L4. Topology decisions are DAG branches at L7. Redundancy (hot Cipher) uses the memory plan (L3) to know which tensors to replicate.
...
**Live model surgery.** L10's architecture mutations happen to a running model. In continuous learning mode, this means the model evolves while serving production traffic:
...
- **Hot Cipher** — other Relays' RAM. Already there from RAID redundancy (L13). Single Relay dies → other Relays already have the shard. Recovery: zero cost, zero delay. This is the common case.
...
**Crucible's role at L14:** erase the training/inference boundary — the Vigil is always running, always ready. Enable continuous learning with verification and rollback through DAG branching. Prevent catastrophic forgetting through stability tracking and selective freezing. Support live architecture mutation on a running Vigil. Provide deterministic reproducibility from fixed execution order and software Philox RNG. Enable time-travel debugging through DAG history. Persist the Cipher across three tiers for survival against any failure mode. Support self-updating Keepers.

**L14 connects upward:** the continuous Vigil feeds the ecosystem (L16) — a Vigil that's always learning is always enriching the KernelCache, always producing DAG versions for audit trails, always contributing to the computation genome.

**L14 connects downward:** continuous learning uses L11's training loop (meta-gradients, curriculum), L10's architecture mutations, L9's attention replacements, all the way down to L2's compiled kernels and L1's hardware measurement. The full stack is active, continuously, for the lifetime of the Vigil. The Cipher (persistent state) flows through L13's hot tier (Canopy redundancy), L3's memory plan (recomputed on reincarnation), and L2's KernelCache (recompiled for new hardware).
...
*Phase 1: GPU profiling (2s, parallel across all Relays).* Per-GPU: GEMM benchmark → actual sustained tensor TFLOPS (a throttling H100 delivers 700, not 989). Streaming copy → actual HBM/PCIe bandwidth. Clock reading under sustained compute → true frequency. NVML/rocm-smi: power draw, temperature, ECC errors, throttle events, usable memory. Result: `GPUProfile { actual_tflops, actual_hbm_bw, sustained_clock, power_watts, usable_memory }`. Spec sheets are marketing. Thermal throttling is physics. Meridian measures truth.

*Phase 2: Network probing (3s, all-pairs parallel).* N×N: ping-pong → latency matrix (μs, half-RTT). Unidirectional flood → bandwidth matrix (GB/s). Topology detection: NVSwitch (all-to-all full bandwidth) vs NVLink (point-to-point, 900 GB/s on H100) vs PCIe (64 GB/s Gen5) vs InfiniBand (400 Gb/s HDR) vs RoCE vs TCP. Result: the complete weighted network graph. Not what the vendor says — what the electrons deliver.

*Phase 3: Kernel calibration (5s, representative shapes).* Top-20 kernels by F\*X-predicted execution time × 5 shapes = 100 benchmarks × 50ms each. Compare actual measured time vs F\*X's roofline prediction → correction factor per kernel class. These factors absorb L2 cache effects, warp scheduler efficiency, TLB pressure, instruction-level overhead. After calibration: F\*X prediction × Meridian correction = ±3-5% accuracy on any kernel for any shape.
...
*Continuous monitoring.* Every iteration: predicted vs actual. Error <5% → accurate. 5-10% sustained → update correction factors. >10% sustained → diagnose: thermal throttling (NVML clock drop)? ECC degradation? Network congestion? Dynamic shapes triggered re-planning? Competing process? Trigger Meridian recalibration if hardware changed.

*Bottleneck identification — the single most valuable output.* Not "you're at 52% MFU." Instead: "you're at 52% because attention QKV projection saturates HBM bandwidth. Switching to FP8 attention: +12%. Switching to TP=4: +18%. Both: +26%, MFU to 66%." Each bottleneck (COMPUTE, MEMORY\_BW, COMMUNICATION, BUBBLE, IMBALANCE) determines which recommendation has the highest impact.
...
- Organization A trains GPT-7B on 8×H100. F\*X generates 2,000 proved-optimal kernels.
...
**Federated learning built-in.** DiLoCo (L13) + differential privacy = federated learning:
...
Privacy guarantee: no raw data or raw gradients leave any site. Only noised pseudo-gradients, every H steps. The noise is calibrated by sensitivity analysis — Crucible knows the gradient norms (from L11), which bounds the sensitivity, which determines the minimum noise for ε-differential privacy. Crucible controls EXACTLY what data leaves each site because it controls all communication (L13). Auditable through the Merkle hash trail (L7) — prove that the noised gradient was computed correctly.

**Cross-Vigil knowledge transfer.** Beyond kernel sharing: DAG fragments transfer between Vigils. A vision encoder trained on ImageNet produces a DAG subgraph with content_hashes for patch embedding, self-attention, feed-forward layers. A new vision-language Vigil can IMPORT that subgraph — same content_hashes → same compiled kernels → zero compilation. The pre-trained weights load into the imported regions. Fine-tuning modifies only the new adapter regions. The DAG splicing from L10, scaled to an ecosystem.
...
**Model marketplace.** Content-addressed DAG fragments + persistent KernelCache + verified quality metrics (from L14's continuous evaluation) = a marketplace for model components:
...
This is empirical data about real workloads that no synthetic benchmark captures. Feed it to hardware designers: build tensor cores optimized for the shapes that actually occur. Build sparse execution units for the sparsity patterns that actually emerge. Build memory hierarchies sized for the working sets that actually matter. The ecosystem's workload data shapes the next generation of hardware → F\*X re-solves optimal kernels for new silicon → new data → co-evolution between software and silicon.
...
─────────────────────────────────────────────────────────────────────────────
L0   F*X              proved foundation: SMT kernel synthesis, extraction to C++/CUDA, proof certificates
```

The ontology:

```
F*X      = the law         proved foundation, SMT kernel synthesis, extraction to C++/CUDA
...
L4 Operations: TraceRing SPSC, MetaLog, recording pipeline. L6 Graphs: TraceGraph CSR. L7 Merkle DAG: RegionNode, BranchNode, content/merkle hashing. L3 Memory: MemoryPlan sweep-line, PoolAllocator. L4/L7 Compiled Tier 1: ReplayEngine, CrucibleContext, dispatch\_op, divergence recovery. L2 Kernels: CKernel 146-op taxonomy. L14: Serialize/Deserialize, Cipher. L6 Graph IR: Graph.h, ExprPool, SymbolTable. L0 partial: Effects.h (fx::Alloc/IO/Block), Reflect.h (reflect\_hash, reflect\_print). Vessel: PyTorch adapter.

**Phase 2: F\*X Core (NEXT)**

Goal: The four-layer proof architecture, fully operational.

- *Layer 4 (Z3):* `verify/` directory with `crucible_verify.cpp` linking libz3. Proof suites: arena alignment, hash (fmix64 no fixed points, avalanche, determinism), ring (bitmask, invariant, linearizability), arithmetic (mul\_sat, add\_sat), memory plan (non-overlap N=2..16, optimality). Z3 optimization: memory layout, kernel configs, topology. Protocol: mode transition guards. Communication: ring all-reduce, FSDP shard coverage. CMake: `add_custom_target(verify)`, all targets depend on proofs.
- *Layer 3 (consteval):* `constexpr` Arena via `if consteval`. Algorithm verification: 500 random memory plans, 500 random toposorts, serialization roundtrip, hash determinism, CSR consistency. Model checking: SPSC deadlock freedom (81 states BFS), mode transition completeness. `constexpr` Philox4x32 for compile-time random generation.
- *Layer 2 (reflection):* Expand Reflect.h: `verify_init_safe<T>()`, `verify_type_safe<T>()`, `verify_layout<T>()`, `verify_hash_completeness<T>()`. Auto-serde. `define_class()` for SoA and optimal physical layouts. Registration: `static_assert(verify_struct<RegionNode>(...))` for every layout-critical struct.
- *Layer 1 (types):* Complete fx::IO/Block wiring. Thread-affinity phantom types. State-machine typestate. Refinement types: NonZero, InRange, Positive. Algebraic concepts: Lattice, Monoid, Semiring, Functor.
