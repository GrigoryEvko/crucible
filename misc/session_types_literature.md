...
1. **The top-down path is the primary path.** A global type goes through a well-formedness check, then projection, then subtyping. Liveness then holds by construction. Do not check liveness by a walk over local types. (Pischke and Yoshida, OOPSLA 2026.) A projection alone proves nothing. Equation 49 of Pischke, Masters and Yoshida (page 17) projects, but its context is unsafe. Association must therefore require balanced+.
2. **Well-formedness is balanced+.** Each participant must have a bounded depth in each reachable global type. The count of en-route messages for each pair of roles must agree across branches. No en-route message can occur below a recursion binder that loops, that is, a binder whose variable occurs in its body. Definition 16 and Example 14 (page 18) accept an en-route message below a binder whose variable does not occur. The check is structural and decidable. A bounded buffer does not give balanced+. (Pischke, Masters and Yoshida, v4, Definitions 13 to 17.)
3. **Projection is the sound and complete projection of Tirore, Bengtson and Carbone.** A role that has no part in a loop projects to `End`. It never projects to `Loop<Continue>`. Coinductive full merge is necessary to type the same live sessions as a bottom-up check. (ITP 2023 and JAR 2025. Pischke and Yoshida, OOPSLA 2026.)
   - The implementation in `include/fixy/session/Projection.h` uses an inductive full merge with bounded loop unfolding. It is sound but not complete, because the inductive projection is a subrelation of the coinductive projection (Pischke, Masters and Yoshida, page 9). For example, it refuses Example 12 G2, whose context is live.
   - Recursion uses the nearest binder. An inner loop cannot continue an outer loop (Tirore, Bengtson and Carbone, ITP 2023, equation 7). This limits expressiveness, not safety.
   - Association requires each role that is not in the global type to have finished. This follows Pischke and Yoshida, OOPSLA 2026, Definition 6.9 (page 17). It is stricter than Pischke, Masters and Yoshida, Definition 21 (page 25), which puts no constraint on extra roles.
...
6. **Subtyping has two parts.**
   ...
   - The bound limits the number of unrolls and also the number of messages in the buffer. Rumpsteak assumes buffers that have no limit (PPoPP 2022, section 3). The mint must set the bound from the capacity of the channel, not from a number that the caller gives. A check at capacity 4 accepts a pair that deadlocks on a channel of capacity 1.
   ...
   - A wrapper that is its own dual has no order on its value. An order there breaks the closure under duality. For example, `VendorPinned` compares its vendor for identity only.
   - The papers match choice branches by label, because their wire carries the label. Our `select<I>()` puts the index on the wire, so the position is the label. Label branches must pair by position. A match by label needs a stable label id on the wire first. A crash branch is not a label, and it pairs by its payload.
   - A choice with no branch is not well-formed. Under the branch rule it refines each larger choice, and a substitute of that type never sends.
