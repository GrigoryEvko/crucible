...
2. Specialize `splits_into_pack<MyData, Slice<MyData, 0>, ..., Slice<MyData, 7>>` as `std::true_type` so `mint_permission_split_n` accepts the parent
3. Auto-generate `mint_permission_combine_n` for the inverse direction
...
This is what lets `mint_permission_fork` be sound: every child thread mutates its disjoint region, the array<jthread> destructor joins them all, and the parent scope can read the results with plain non-atomic loads.  No memory fences needed at the user level.
...
│         Permission<Tag> · mint_permission_split/combine/n           │
│         mint_permission_fork<Children>                              │
...
```cpp
mint_permission_root<Tag>()      → Permission<Tag>            // create
mint_permission_split<L, R>(p)        → (Permission<L>, Permission<R>)  // CSL split
mint_permission_combine<In>(l, r)     → Permission<In>             // CSL merge
mint_permission_split_n<Cs...>(p)     → tuple<Permission<Cs>...>   // n-ary split
...
The structured-concurrency primitive `mint_permission_fork`:
...
auto whole = mint_permission_root<channel_tag::Whole<MyChannel>>();
...
auto perm = mint_permission_root<MyData>();
...
| mint_permission_split_n | parallel_for entry | 0 ns (no-op) |
...
| mint_permission_combine_n + recombine | parallel_for exit | 0 ns |
...
| `mint_permission_fork<Children...>(parent, callables...)` | `Permission<Parent>` | 0 (jthreads on stack array) | 0 user-level |
...
auto perm = mint_permission_root<MetricsRegion>();
...
auto channel_perm = mint_permission_root<queue_tag::Whole<MyChannel>>();
...
auto pipeline_perm = mint_permission_root<BgPipeline>();
...
| `mint_permission_split_n` | constexpr no-op |
...
- 100% of cross-thread handoffs use either `mint_permission_fork` (structured) or move-into-jthread-lambda (single owner)
...
- ✅ **SEPLOG-A4**: `mint_permission_fork<Children...>` structured concurrency
...
| Parent permission rebuilt before all children finish | `mint_permission_fork`'s `array<jthread>` destructor joins ALL workers BEFORE the rebuild line executes |
...
| Two values with the same Permission tag created at different sites | grep audit + `mint_permission_root<Tag>` is unique-call-site by convention |
...
| Async-cancelled jthread leaves Permission orphaned | `mint_permission_fork` does not currently propagate stop_token mid-body; future work |
...
auto whole = mint_permission_root<MyChannelWhole>();
auto [prod_perm, cons_perm] = mint_permission_split<
    queue_tag::Producer<MyChannel>,
...
- `mint_permission_split` + `mint_permission_combine` are inverse (algebraic laws)
- `SharedPermissionPool`'s atomic state machine satisfies CSL's mode-transition triple
- `mint_permission_fork` preserves the parallel composition rule under Crucible's noexcept-only execution model
...
public:
    PermissionedMpmcChannel()
        : producer_pool_{mint_permission_root<mpmc_tag::Producer<UserTag>>()},
          consumer_pool_{mint_permission_root<mpmc_tag::Consumer<UserTag>>()} {}
...
For large iterations (>10K ops), the compile stage further parallelises: each kernel compile is independent, dispatched by AdaptiveScheduler with `mint_permission_fork` over per-kernel sub-permissions.
...
struct BackgroundThread : Pinned<BackgroundThread> {
    ...
    void start_pipeline() {
        ...
        // (Implementation uses mint_permission_fork variant that allows long-running stages.)
...
auto whole = mint_permission_root<queue_tag::Whole<MyChan>>();
auto [pp, cp] = mint_permission_split<
    queue_tag::Producer<MyChan>,
    queue_tag::Consumer<MyChan>>(std::move(whole));

mint_permission_fork<queue_tag::Producer<MyChan>, queue_tag::Consumer<MyChan>>(
    std::move(whole),
...
- `mint_permission_fork` provides automatic join (no `producer.join()` / `consumer.join()` to forget)
...
- Both threads' completion is guaranteed before `mint_permission_fork` returns
...
| Operation | Per-call shape |
|---|---|
| `mint_permission_root<T>()` | constexpr, compile-time |
| `mint_permission_split<L,R>(p)` | constexpr |
| `mint_permission_combine<I>(l, r)` | constexpr |
| `mint_permission_split_n<C0..C7>(p)` | constexpr |
| `mint_permission_fork<C...>(parent, ...)` | N × pthread_create + RAII join (paid once per fork-join) |
...
For a parallel decision:
  ...
  WorkBudget evaluation → cost-model heuristic → mint_permission_split_n → OwnedRegion::split_into<N> → pthread_create × N → body execution → pthread_join × N → OwnedRegion::recombine<N>
...
### F.1 Algebraic laws of mint_permission_split / mint_permission_combine
...
theorem split_combine_id {Parent L R : Type}
    ...
    (p : Permission Parent) :
    let (l, r) := mint_permission_split L R p
    mint_permission_combine Parent l r = p

theorem combine_split_id {Parent L R : Type}
    [SplitsInto Parent L R]
    (l : Permission L) (r : Permission R) :
    let p := mint_permission_combine Parent l r
    mint_permission_split L R p = (l, r)
```
...
### F.3 mint_permission_fork structural invariants
...
theorem fork_preserves_parent {Parent : Type} {Children : List Type}
    ...
    let p' := mint_permission_fork Children p callables
...
### G.4 "What's the cost of `mint_permission_fork` for very small N?"
...
Crucible globally bans exceptions (`-fno-exceptions` per `code_guide.md` §III).  Worker bodies inside `mint_permission_fork` and `parallel_for_views` are required to be `noexcept` — violation is a `static_assert`.  Errors propagate via `std::expected<T, E>` returns or via abort; Crucible's policy is to abort on unrecoverable errors rather than try to unwind.
...
- **Cilk / OpenCilk**: Charles Leiserson and the Cilk team (MIT, OpenCilk Project).  Their work on randomised work-stealing and the spawn-sync structured concurrency idea predated our `mint_permission_fork`.
