...
[[nodiscard]] inline std::size_t key_position(std::string_view json, std::string_view key) {
    std::string needle;
...
[[nodiscard]] inline std::string extract_json_string(std::string_view json, std::string_view key) {
...
[[nodiscard]] inline unsigned extract_json_uint(std::string_view json, std::string_view key) {
    const std::size_t key_pos = key_position(json, key);
    if (key_pos == std::string_view::npos) return 0;
    const std::size_t colon = json.find(':', key_pos);
    if (colon == std::string_view::npos) return 0;
    std::size_t begin = colon + 1;
    while (begin < json.size() && json[begin] == ' ')
        ++begin;
    ...
    unsigned value = 0;
    const auto [ptr, ec] = std::from_chars(json.data() + begin, json.data() + end, value);
...
[[nodiscard]] inline std::string title_from(CrucibleDiagnostic const& d) {
    ...
    std::string_view source = d.suggestion.empty() ? std::string_view{d.error_code} : std::string_view{d.suggestion};
...
[[nodiscard]] inline CrucibleDiagnostic parse_diagnostic_json(std::string_view json) {
...
[[nodiscard]] inline std::string to_lsp_code_action(std::string_view diagnostic_json) {
    ...
    std::string out;
    out.reserve(512 + d.error_code.size() + d.gap.size() + d.suggestion.size() + d.file.size());
    out += "{\"title\":";
