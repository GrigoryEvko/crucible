...
void fill_random_entry(crucible::fuzz::prop::Rng& rng, TraceRing::Entry& e) noexcept {
    ...
    for (auto& v : e.scalar_values)
        v = static_cast<int64_t>(rng.next64());
}
...
// equality predicate.
[[nodiscard]] bool entry_equal(const TraceRing::Entry& a, const TraceRing::Entry& b) noexcept {
...
int main(int argc, char** argv) {
    ...
    // the two-segment wrap path is hit when filling to CAP.
    {
        ...
        for (uint32_t i = 0; i < CAP; ++i) {
            ...
            if (!ring->try_append(e, MetaIndex{i}, ScopeHash{0xA0000000ULL | i}, CallsiteHash{0xB0000000ULL | i}))
                std::abort();
        }
        ...
        while (delivered < CAP) {
            const uint32_t got = ring->drain(out_e.data(), CHUNK, out_m.data(), out_s.data(), out_c.data());
            ...
            for (uint32_t k = 0; k < got; ++k) {
                ...
                if (out_s[k] != ScopeHash{0xA0000000ULL | i}) std::abort();
                if (out_c[k] != CallsiteHash{0xB0000000ULL | i}) std::abort();
            }
            delivered += got;
        }
    ...
    return run(
        "TraceRing SPSC append/drain round-trip", cfg,
        [](Rng& rng) {
            Schedule s{};
            for (unsigned p = 0; p < K_PHASES; ++p) {
                s.is_append[p] = (rng.next32() & 1u) != 0u;
                s.count[p] = static_cast<uint16_t>(rng.next_below(MAX_OPS_PER_PHASE + 1));
            }
            for (auto& slot : s.payload) {
                ...
                slot.meta_start = (rng.next32() & 0xFu) == 0u ? MetaIndex{} : MetaIndex{rng.next32() & 0x00FFFFFFu};
        ...
        [&ring](const Schedule& s) -> bool {
            ...
            for (unsigned p = 0; p < K_PHASES; ++p) {
                ...
                if (s.is_append[p]) {
                    for (uint16_t i = 0; i < n; ++i) {
                        ...
                        const bool accepted =
                            ring->try_append(src.entry, src.meta_start, src.scope_hash, src.callsite_hash);
                    ...
                    const uint32_t got = ring->drain(out_e.data(), want, out_m.data(), out_s.data(), out_c.data());
                    ...
                    for (uint32_t k = 0; k < got; ++k) {
                        const auto& want_slot = mirror[(mirror_tail + k) & (MIRROR_CAPACITY - 1u)];
            ...
            if (total_drained + (mirror_head - mirror_tail) != mirror_head) return false;
