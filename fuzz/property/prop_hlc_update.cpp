...
[[nodiscard]] std::uint64_t gen_u64(Rng& rng) noexcept {
    switch (rng.next_below(6u)) {
        ...
        case 3:
            return kPMax - 1u;
        case 4:
            return rng.next_below(64u);  // small cluster → physical collisions
        default:
            return rng.next64();
    }
}
...
int main(int argc, char** argv) {
    ...
    return run(
        ...
        [](const Spec& spec) noexcept -> bool {
            ...
            if (cc::detail::unpack_hlc_timestamp(cc::detail::pack_hlc_timestamp(old_ts)) != old_ts) {
                return false;
            }
            if (cc::detail::unpack_hlc_timestamp(cc::detail::pack_hlc_timestamp(peer_ts)) != peer_ts) {
                return false;
            }
            ...
            const std::uint64_t recv_floor = std::max({old_ts.physical_ns, peer_ts.physical_ns, now});
