...
// These layout assertions deliberately duplicate those of the sibling syscall
// smoke test.  Both share the wire format, so an include-chain refactor that
// breaks one fence is caught at both.
...
struct DummyState {};
static_assert(sizeof(crucible::perf::SyscallTpBtf) == sizeof(std::unique_ptr<DummyState>),
...
int main() {
    ...
    if (hub.has_value()) {
        ...
        // The facade needs both sys_enter and sys_exit attached.  load()
        // returns nullopt when fewer than two attach, so a populated hub
        // holds at least two links.
        if (attached.value() < 2) {
            std::fprintf(stderr,
        ...
        if (timeline.size() != crucible::perf::TIMELINE_CAPACITY) {
            std::fprintf(stderr, "perf::SyscallTpBtf::timeline_view() — expected %u, got %zu\n",
        ...
        if (hub->total_syscalls() != 0u || hub->timeline_write_index() != 0u || !hub->timeline_view().empty()
            || hub->attached_programs().value() != 0u || hub->attach_failures().value() != 0u) {
            std::fprintf(stderr, "perf::SyscallTpBtf moved-from accessors — expected zeros\n");
            return 1;
        }
        (void)moved_into;
    }
