...
void test_source_position_parser() {
    ...
    EXPECT(oversized.line == 0, "oversized line values must leave source line unset");
}
...
void test_json_escape() {
    ...
    EXPECT(json.find("\\\"") != std::string::npos, "JSON string values must escape quotes");
    EXPECT(json.find("\\\\") != std::string::npos, "JSON string values must escape backslashes");
    EXPECT(json.find("\\n") != std::string::npos, "JSON string values must escape newlines");
    EXPECT(json.find("\"related_snippets\":[\"x < y\"]") != std::string::npos, "related snippet array must be emitted");
    std::fclose(f);
}
...
void test_violation_json_and_lsp_action() {
    ...
    const bool ok = diag::emit_json_violation(f, diag::Category::EffectRowMismatch,
                                              "test_diag_json_emitter.cpp:77:9@test_fn", "caller row lacks Bg");
    EXPECT(ok, "violation JSON emission must succeed");
    const std::string json = read_tmp_file(f);
    std::fclose(f);

    EXPECT(json.find("\"format_version\":1") != std::string::npos, "JSON must carry the diagnostic format version");
    EXPECT(json.find("\"error_code\":\"EffectRowMismatch\"") != std::string::npos,
           "JSON must carry the named diagnostic code");
    EXPECT(json.find("\"line\":77") != std::string::npos, "JSON must carry source line");
    EXPECT(json.find("\"column\":9") != std::string::npos, "JSON must carry source column");
    EXPECT(json.find("\"gap\":\"caller row lacks Bg\"") != std::string::npos, "JSON must carry gap detail");

    const auto parsed = lsp::parse_diagnostic_json(json);
    EXPECT(parsed.file == "test_diag_json_emitter.cpp", "LSP parser must recover diagnostic file");
    EXPECT(parsed.line == 77, "LSP parser must recover source line");
    EXPECT(parsed.column == 9, "LSP parser must recover source column");
    EXPECT(parsed.error_code == "EffectRowMismatch", "LSP parser must recover diagnostic code");

    const std::string action = lsp::to_lsp_code_action(json);
    EXPECT(action.find("\"kind\":\"quickfix\"") != std::string::npos, "CodeAction must be a quickfix");
    EXPECT(action.find("\"source\":\"crucible\"") != std::string::npos,
           "CodeAction diagnostic source must be crucible");
    EXPECT(action.find("\"code\":\"EffectRowMismatch\"") != std::string::npos, "CodeAction must carry diagnostic code");
    EXPECT(action.find("\"line\":76") != std::string::npos, "LSP range line must be zero-based");
    EXPECT(action.find("\"character\":8") != std::string::npos, "LSP range character must be zero-based");
    EXPECT(action.find("caller row lacks Bg") != std::string::npos, "CodeAction must carry the original gap message");
}
