// The header carries most of the coverage in static_asserts.  Driving a
// few patterns end to end over an in-memory transport is what shows each
...
auto send_str = [](Wire& w, std::string&& s) noexcept { w.bytes->push_back(std::move(s)); };
...
int run_request_response_once() {
    ...
    auto [client, server] = mint_channel<RequestResponseOnce_Client<Req, Resp>>(ctx, ctx, std::move(a), std::move(b));
...
int run_fan_out() {
    ...
    if (wire.size() != 3 || wire[0] != "REQ:job0" || wire[1] != "REQ:job1" || wire[2] != "REQ:job2") {
...
int run_scatter_gather() {
    ...
    auto [coord, worker] =
        mint_channel<ScatterGather<N, Req, Resp>>(ctx, ctx, std::move(client_res), std::move(server_res));
    ...
    // One actor plays all the workers here, so it receives every task
...
auto send_vote = [](Wire& w, Vote&& v) noexcept { w.bytes->push_back(v.yes ? "VOTE:YES" : "VOTE:NO"); };
...
int run_two_phase_commit() {
    ...
    auto [coord, follower] = mint_channel<Proto>(ctx, ctx, std::move(a), std::move(b));
    ...
    auto follower_rc = std::move(follower3).branch(recv_idx, [&](auto branch_handle) -> int {
