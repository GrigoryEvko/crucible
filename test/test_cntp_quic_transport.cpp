...
[[nodiscard]] std::array<std::byte, 32> fingerprint_bytes(std::byte seed) {
    ...
    for (std::size_t i = 0; i < out.size(); ++i) {
        out[i] = static_cast<std::byte>(std::to_integer<unsigned>(seed) ^ static_cast<unsigned>(i * 7u));
    }
    return out;
}
...
[[nodiscard]] cntp::DeclaredMtlsConfig mtls_config(cntp::MtlsDnsName peer) {
    ...
    return cntp::mint_mtls_config(std::move(*ca), std::move(*cert), std::move(*key), policy);
}

[[nodiscard]] cntp::DeclaredQuicConfig quic_config(std::uint16_t streams = 2, cntp::QuicFeatureMask features = {
    ...
    return cntp::mint_quic_config(*max_streams, *max_datagram,
                                  cntp::mint_cc_choice<cntp::CcAlgorithm::Bbr3, cntp::LinkClass::CrossDatacenter>(),
                                  features, cntp::QuicBackend::Quiche);
}
...
void test_admission_and_backend_boundary() {
    ...
    auto connect = cntp::connect_quic(*fd, mtls, config, *peer, fingerprint(std::byte{0x41}));
    ...
    auto bad_peer = cntp::admit_quic_peer(mtls, *denied, fingerprint(std::byte{0x41}));
...
void test_stream_budgeting() {
    ...
    auto peer = cntp::admit_quic_peer(mtls, *peer_name, fingerprint(std::byte{0x41}));
    assert(peer.has_value());
    auto fd = cntp::admit_socket_fd(4).value();

    auto connection = cntp::mint_quic_connection<2>(init, fd, *peer, quic_config(2));
...
void test_datagram_zero_rtt_and_migration_plans() {
    ...
    auto connection = cntp::mint_quic_connection<4>(init, fd, *peer, quic_config());
    ...
    assert(disabled.send_datagram(bg, payload).error() == cntp::QuicError::DatagramDisabled);
    assert(disabled.enable_0rtt(bg, *token).error() == cntp::QuicError::ZeroRttDisabled);
    assert(disabled.plan_migration(bg, path_plan()).error() == cntp::QuicError::MigrationDisabled);
