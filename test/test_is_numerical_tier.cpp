...
using NT_struct_bitexact = safety::NumericalTier<extract::Tolerance::BITEXACT, payload_struct>;

using NT_nested = safety::NumericalTier<extract::Tolerance::BITEXACT, NT_int_bitexact>;

}  // namespace

// These declarations sit in a named namespace deliberately.  In the
// anonymous namespace, a function reached only through `&fn` inside a
// static_assert still trips the unused-function warning, and the build
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_numerical_tier_smoke_test()); }
...
void test_value_type_extraction() {
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_int_bitexact>, int>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_int_relaxed>, int>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_double_bitexact>, double>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_float_bitexact>, float>);
}

void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_int_bitexact const&>, int>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_int_bitexact&&>, int>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_double_bitexact const>, double>);
}

void test_tier_extraction_every_value() {
    static_assert(extract::numerical_tier_v<NT_int_bitexact> == extract::Tolerance::BITEXACT);
    static_assert(extract::numerical_tier_v<NT_int_ulp_fp64> == extract::Tolerance::ULP_FP64);
    static_assert(extract::numerical_tier_v<NT_int_ulp_fp32> == extract::Tolerance::ULP_FP32);
    static_assert(extract::numerical_tier_v<NT_int_ulp_fp16> == extract::Tolerance::ULP_FP16);
    static_assert(extract::numerical_tier_v<NT_int_ulp_fp8> == extract::Tolerance::ULP_FP8);
    static_assert(extract::numerical_tier_v<NT_int_ulp_int8> == extract::Tolerance::ULP_INT8);
    static_assert(extract::numerical_tier_v<NT_int_relaxed> == extract::Tolerance::RELAXED);
}

void test_tier_extraction_cv_ref_strips() {
    static_assert(extract::numerical_tier_v<NT_int_bitexact const&> == extract::Tolerance::BITEXACT);
    static_assert(extract::numerical_tier_v<NT_int_relaxed&&> == extract::Tolerance::RELAXED);
}

void test_distinct_tiers_distinct_specs() {
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_int_bitexact>,
                                 extract::numerical_tier_value_t<NT_int_relaxed>>);
    static_assert(extract::numerical_tier_v<NT_int_bitexact> != extract::numerical_tier_v<NT_int_relaxed>);

    static_assert(extract::numerical_tier_v<NT_int_bitexact> == extract::numerical_tier_v<NT_double_bitexact>);
    static_assert(!std::is_same_v<extract::numerical_tier_value_t<NT_int_bitexact>,
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_struct_bitexact>, payload_struct>);
    static_assert(extract::numerical_tier_v<NT_struct_bitexact> == extract::Tolerance::BITEXACT);
}

void test_positive_nested_numerical_tier() {
    static_assert(extract::is_numerical_tier_v<NT_nested>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<NT_nested>, NT_int_bitexact>);
    static_assert(extract::is_numerical_tier_v<extract::numerical_tier_value_t<NT_nested>>);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<extract::numerical_tier_value_t<NT_nested>>, int>);
}
...
void test_dispatcher_function_parameter_use_case() {
    ...
    static_assert(ex::numerical_tier_v<P0_relaxed> == ex::Tolerance::RELAXED);
...
void test_local_alias_round_trip() {
    using LocalBitexactInt = safety::NumericalTier<extract::Tolerance::BITEXACT, int>;
    using LocalFp32Double = safety::NumericalTier<extract::Tolerance::ULP_FP32, double>;

    static_assert(extract::is_numerical_tier_v<LocalBitexactInt>);
    static_assert(extract::is_numerical_tier_v<LocalFp32Double>);
    static_assert(extract::numerical_tier_v<LocalBitexactInt> == extract::Tolerance::BITEXACT);
    static_assert(extract::numerical_tier_v<LocalFp32Double> == extract::Tolerance::ULP_FP32);
    static_assert(std::is_same_v<extract::numerical_tier_value_t<LocalBitexactInt>, int>);
}
...
void test_runtime_consistency() {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        EXPECT_TRUE(baseline_pos == extract::is_numerical_tier_v<NT_int_bitexact>);
        EXPECT_TRUE(baseline_neg == !extract::is_numerical_tier_v<int>);
...
int main() {
    ...
    run_test("test_positive_nested_numerical_tier", test_positive_nested_numerical_tier);
