...
static_assert(!dc::factorization_eq<uint32_t>(overflowing, 8589934592ull & 0xFFFFFFFFu));
...
static_assert(!dc::fmix_preserves_non_zero(0, crucible::detail::fmix64(0)));
static_assert(!dc::fmix_preserves_non_zero(0, 0));

// Artificial pairs the real mixer cannot produce, so that each clause
// is witnessed on its own.
...
int main() {
    ...
    // fmix_preserves_non_zero
    {
        ...
        if (!dc::fmix_preserves_non_zero(static_cast<std::uint64_t>(seed_nz), static_cast<std::uint64_t>(mix_nz_v))) {
            std::fprintf(stderr, "test_decide: fmix_preserves_non_zero(non-zero seed, fmix(seed)) "
                                 "WRONGLY rejected\n");
            return 1;
        }
        volatile std::uint64_t zero_seed = 0;
        const std::uint64_t mix_of_zero = crucible::detail::fmix64(static_cast<std::uint64_t>(zero_seed));
        if (mix_of_zero != 0) {
            std::fprintf(stderr, "test_decide: fmix64(0) returned non-zero — bijection "
                                 "theorem violated\n");
            return 1;
        }
