...
//
// mint_sched_tp_btf rejects ColdInitCtx.  The load calls
// bpf(BPF_PROG_LOAD), which enters the kernel and waits while the
// verifier walks the program, so sched_tp_btf_required_row carries
...
int main() {
    auto hub = crucible::perf::mint_sched_tp_btf(crucible::effects::ColdInitCtx{}, crucible::effects::testing::init());
    (void)hub;
    return 0;
}
