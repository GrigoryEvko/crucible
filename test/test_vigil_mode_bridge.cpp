...
static_assert(crucible::safety::proto::is_well_formed_v<crucible::Vigil::ModeProtocol>);
static_assert(std::is_same_v<typename ModeSession::resource_type, const ModeCell*>);
...
int run_atomic_mode_session_lifecycle() {
    ...
    auto apply_transition = []<typename Transition>(ModeCell*& c, Transition&&) noexcept {
        static_assert(crucible::Vigil::mode_transition_allowed(Transition::from, Transition::to));
        assert(crucible::safety::atomic_machine_state(*c) == Transition::from);
        crucible::safety::publish_atomic_machine_transition<Transition, ModeCell>(c, Transition{});
        assert(crucible::safety::atomic_machine_state(*c) == Transition::to);
    };

    auto session = crucible::safety::mint_atomic_session<crucible::Vigil::ModeProtocol>(cell);
    auto publish_compiled = std::move(session).select_local<0>();
    auto after_compiled =
        std::move(publish_compiled).send(crucible::Vigil::ModeRecordingToCompiled{}, apply_transition);

    auto publish_recording = std::move(after_compiled).select_local<1>();
    auto after_recording =
        std::move(publish_recording).send(crucible::Vigil::ModeCompiledToRecording{}, apply_transition);
