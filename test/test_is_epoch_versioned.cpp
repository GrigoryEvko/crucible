...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_epoch_versioned_smoke_test()); }
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EV_int const&>, int>);
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EV_int&&>, int>);
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EV_double const>, double>);
}

void test_distinct_element_types_distinct_specs() {
    static_assert(!std::is_same_v<EV_int, EV_double>);
    static_assert(
        !std::is_same_v<extract::epoch_versioned_value_t<EV_int>, extract::epoch_versioned_value_t<EV_double>>);
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EV_struct>, payload_struct>);
}

void test_positive_nested_epoch_versioned() {
    static_assert(extract::is_epoch_versioned_v<EV_nested>);
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EV_nested>, EV_int>);
    static_assert(extract::is_epoch_versioned_v<extract::epoch_versioned_value_t<EV_nested>>);
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<extract::epoch_versioned_value_t<EV_nested>>, int>);
}
...
void test_runtime_grade_layout_invariant() {
    // The grade is a pair of runtime values costing 16 bytes on top of
    // the wrapped element.
...
void test_distinct_product_wrapper_identity() {
    // These two wrappers share a layout byte for byte and stay distinct
    // types anyway, because the detector keys on wrapper identity rather
    // than on shape.  Folding both into one generic product wrapper would
    // pass every other test in this file and fail here.
    static_assert(!std::is_same_v<EV_int, B_int>);
    static_assert(sizeof(EV_int) == sizeof(B_int));
    static_assert(extract::is_epoch_versioned_v<EV_int>);
...
void test_public_aliases_round_trip() {
    ...
    static_assert(std::is_same_v<extract::epoch_versioned_value_t<EpochVersionedInt>, int>);
}
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<EV_int>, extract::epoch_versioned_value_t<EV_int>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfEv = safety::Consistency<extract::Consistency_v::STRONG, EV_int>;
    ...
    static_assert(extract::is_epoch_versioned_v<extract::consistency_value_t<ConsistencyOfEv>>);

    static_assert(extract::is_epoch_versioned_v<EvOfConsistency>);
    static_assert(!extract::is_consistency_v<EvOfConsistency>);
    static_assert(extract::is_consistency_v<extract::epoch_versioned_value_t<EvOfConsistency>>);
}
...
int main() {
    ...
    run_test("test_positive_nested_epoch_versioned", test_positive_nested_epoch_versioned);
    ...
    run_test("test_distinct_product_wrapper_identity", test_distinct_product_wrapper_identity);
