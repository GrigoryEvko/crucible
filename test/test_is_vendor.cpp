// Every backend is exercised on every path, because the backend
// ordinals are not contiguous: the last one is 255 while its neighbour
// is 6.  Any detector that round-trips a backend through plain integer
...
using V_nested = safety::Vendor<extract::VendorBackend_v::Portable, V_int_nv>;
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_vendor_smoke_test()); }
...
void test_negative_lookalike_struct() {
    struct LookalikeVendor {
        int value;
        extract::VendorBackend_v backend;
    };
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::vendor_value_t<V_int_nv const&>, int>);
    static_assert(std::is_same_v<extract::vendor_value_t<V_int_nv&&>, int>);
    static_assert(std::is_same_v<extract::vendor_value_t<V_double_nv const>, double>);
}

void test_backend_extraction_every_value() {
    static_assert(extract::vendor_backend_v<V_int_none> == extract::VendorBackend_v::None);
    static_assert(extract::vendor_backend_v<V_int_cpu> == extract::VendorBackend_v::CPU);
    static_assert(extract::vendor_backend_v<V_int_nv> == extract::VendorBackend_v::NV);
    static_assert(extract::vendor_backend_v<V_int_amd> == extract::VendorBackend_v::AMD);
    static_assert(extract::vendor_backend_v<V_int_tpu> == extract::VendorBackend_v::TPU);
    static_assert(extract::vendor_backend_v<V_int_trn> == extract::VendorBackend_v::TRN);
    static_assert(extract::vendor_backend_v<V_int_cer> == extract::VendorBackend_v::CER);
    static_assert(extract::vendor_backend_v<V_int_portable> == extract::VendorBackend_v::Portable);
}

void test_backend_extraction_cv_ref_strips() {
    static_assert(extract::vendor_backend_v<V_int_nv const&> == extract::VendorBackend_v::NV);
    static_assert(extract::vendor_backend_v<V_int_portable&&> == extract::VendorBackend_v::Portable);
}

void test_distinct_backends_distinct_specs() {
    static_assert(std::is_same_v<extract::vendor_value_t<V_int_nv>, extract::vendor_value_t<V_int_amd>>);
    static_assert(extract::vendor_backend_v<V_int_nv> != extract::vendor_backend_v<V_int_amd>);

    static_assert(extract::vendor_backend_v<V_int_nv> == extract::vendor_backend_v<V_double_nv>);
    static_assert(!std::is_same_v<extract::vendor_value_t<V_int_nv>, extract::vendor_value_t<V_double_nv>>);
}

void test_non_contiguous_underlying_value_invariant() {
    static_assert(static_cast<std::uint8_t>(extract::VendorBackend_v::Portable) == 255);
    static_assert(static_cast<std::uint8_t>(extract::VendorBackend_v::CER) == 6);
    static_assert(extract::vendor_backend_v<V_int_portable> != extract::vendor_backend_v<V_int_cer>);
    static_assert(extract::vendor_backend_v<V_int_portable> != extract::vendor_backend_v<V_int_none>);

    static_assert(static_cast<std::uint8_t>(extract::VendorBackend_v::None) == 0);
    static_assert(extract::vendor_backend_v<V_int_none> != extract::vendor_backend_v<V_int_portable>);
}

void test_dispatcher_integration_example() {
    // Each arm returns a different integer, so an arm that matched the
    // wrong backend would be visible rather than merely compiling.
    auto select_lowering = []<typename T>() consteval
        requires extract::IsVendor<T>
    {
        constexpr auto b = extract::vendor_backend_v<T>;
        if constexpr (b == extract::VendorBackend_v::None)
            return 0;
        else if constexpr (b == extract::VendorBackend_v::CPU)
            return 1;
        else if constexpr (b == extract::VendorBackend_v::NV)
            return 2;
        else if constexpr (b == extract::VendorBackend_v::AMD)
            return 3;
        else if constexpr (b == extract::VendorBackend_v::TPU)
            return 4;
        else if constexpr (b == extract::VendorBackend_v::TRN)
            return 5;
        else if constexpr (b == extract::VendorBackend_v::CER)
            return 6;
        else if constexpr (b == extract::VendorBackend_v::Portable)
            return 7;
        else
            return -1;
    };
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::vendor_value_t<V_struct_nv>, payload_struct>);
    static_assert(extract::vendor_backend_v<V_struct_nv> == extract::VendorBackend_v::NV);
}

void test_positive_nested_vendor() {
    static_assert(extract::is_vendor_v<V_nested>);
    static_assert(std::is_same_v<extract::vendor_value_t<V_nested>, V_int_nv>);
    static_assert(extract::is_vendor_v<extract::vendor_value_t<V_nested>>);
    static_assert(std::is_same_v<extract::vendor_value_t<extract::vendor_value_t<V_nested>>, int>);

    // Nesting one wrapper inside another must not let the outer backend
    // shadow the inner one.
    static_assert(extract::vendor_backend_v<V_nested> == extract::VendorBackend_v::Portable);
    static_assert(extract::vendor_backend_v<extract::vendor_value_t<V_nested>> == extract::VendorBackend_v::NV);
}
...
void test_local_alias_round_trip() {
    ...
    static_assert(extract::vendor_backend_v<LocalNvInt> == extract::VendorBackend_v::NV);
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::vendor_backend_v<NoneInt> == extract::VendorBackend_v::None);
    static_assert(extract::vendor_backend_v<CpuInt> == extract::VendorBackend_v::CPU);
    static_assert(extract::vendor_backend_v<NvInt> == extract::VendorBackend_v::NV);
    static_assert(extract::vendor_backend_v<AmdInt> == extract::VendorBackend_v::AMD);
    static_assert(extract::vendor_backend_v<TpuInt> == extract::VendorBackend_v::TPU);
    static_assert(extract::vendor_backend_v<TrnInt> == extract::VendorBackend_v::TRN);
    static_assert(extract::vendor_backend_v<CerInt> == extract::VendorBackend_v::CER);
    static_assert(extract::vendor_backend_v<PortableInt> == extract::VendorBackend_v::Portable);

    // The public aliases are spelled in mixed case and the enumerators
    // in upper case.  Each alias must still resolve to the same type as
    // the enumerator it names, rather than to a near neighbour.
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<V_int_nv>, extract::vendor_value_t<V_int_nv>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfV = safety::Consistency<extract::Consistency_v::STRONG, V_int_nv>;
    using VOfConsistency = safety::Vendor<extract::VendorBackend_v::NV, C_int_strong>;

    static_assert(extract::is_consistency_v<ConsistencyOfV>);
    static_assert(!extract::is_vendor_v<ConsistencyOfV>);
    static_assert(extract::is_vendor_v<extract::consistency_value_t<ConsistencyOfV>>);

    static_assert(extract::is_vendor_v<VOfConsistency>);
    static_assert(!extract::is_consistency_v<VOfConsistency>);
    static_assert(extract::is_consistency_v<extract::vendor_value_t<VOfConsistency>>);
}
...
int main() {
    ...
    run_test("test_backend_extraction_every_value", test_backend_extraction_every_value);
    run_test("test_backend_extraction_cv_ref_strips", test_backend_extraction_cv_ref_strips);
    run_test("test_distinct_backends_distinct_specs", test_distinct_backends_distinct_specs);
    run_test("test_non_contiguous_underlying_value_invariant", test_non_contiguous_underlying_value_invariant);
    ...
    run_test("test_positive_nested_vendor", test_positive_nested_vendor);
