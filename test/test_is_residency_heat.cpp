...
using RH_struct_hot = safety::ResidencyHeat<extract::ResidencyHeatTag_v::Hot, payload_struct>;
using RH_nested = safety::ResidencyHeat<extract::ResidencyHeatTag_v::Warm, RH_int_hot>;
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_residency_heat_smoke_test()); }
...
void test_negative_lookalike_struct() {
    struct LookalikeRH {
        int value;
        extract::ResidencyHeatTag_v tier;
    };
...
void test_value_type_extraction() {
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_int_hot>, int>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_int_cold>, int>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_double_hot>, double>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_float_cold>, float>);
}

void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_int_hot const&>, int>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_int_hot&&>, int>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_double_hot const>, double>);
}

void test_tag_extraction_every_value() {
    static_assert(extract::residency_heat_tag_v<RH_int_hot> == extract::ResidencyHeatTag_v::Hot);
    static_assert(extract::residency_heat_tag_v<RH_int_warm> == extract::ResidencyHeatTag_v::Warm);
    static_assert(extract::residency_heat_tag_v<RH_int_cold> == extract::ResidencyHeatTag_v::Cold);
}

void test_tag_extraction_cv_ref_strips() {
    static_assert(extract::residency_heat_tag_v<RH_int_hot const&> == extract::ResidencyHeatTag_v::Hot);
    static_assert(extract::residency_heat_tag_v<RH_int_cold&&> == extract::ResidencyHeatTag_v::Cold);
}

void test_distinct_tiers_distinct_specs() {
    static_assert(
        std::is_same_v<extract::residency_heat_value_t<RH_int_hot>, extract::residency_heat_value_t<RH_int_cold>>);
    static_assert(extract::residency_heat_tag_v<RH_int_hot> != extract::residency_heat_tag_v<RH_int_cold>);

    static_assert(extract::residency_heat_tag_v<RH_int_hot> == extract::residency_heat_tag_v<RH_double_hot>);
    static_assert(
        !std::is_same_v<extract::residency_heat_value_t<RH_int_hot>, extract::residency_heat_value_t<RH_double_hot>>);
}
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_struct_hot>, payload_struct>);
    static_assert(extract::residency_heat_tag_v<RH_struct_hot> == extract::ResidencyHeatTag_v::Hot);
}

void test_positive_nested_residency_heat() {
    static_assert(extract::is_residency_heat_v<RH_nested>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<RH_nested>, RH_int_hot>);
    static_assert(extract::is_residency_heat_v<extract::residency_heat_value_t<RH_nested>>);
    static_assert(std::is_same_v<extract::residency_heat_value_t<extract::residency_heat_value_t<RH_nested>>, int>);
}
...
void test_dispatcher_function_parameter_use_case() {
    ...
    static_assert(ex::residency_heat_tag_v<P0> == ex::ResidencyHeatTag_v::Hot);
    ...
    static_assert(ex::residency_heat_tag_v<P0_cold> == ex::ResidencyHeatTag_v::Cold);
...
void test_local_alias_round_trip() {
    using LocalHotInt = safety::ResidencyHeat<extract::ResidencyHeatTag_v::Hot, int>;

    static_assert(extract::is_residency_heat_v<LocalHotInt>);
    static_assert(extract::residency_heat_tag_v<LocalHotInt> == extract::ResidencyHeatTag_v::Hot);
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::residency_heat_tag_v<HotInt> == extract::ResidencyHeatTag_v::Hot);
    static_assert(extract::residency_heat_tag_v<WarmInt> == extract::ResidencyHeatTag_v::Warm);
    static_assert(extract::residency_heat_tag_v<ColdInt> == extract::ResidencyHeatTag_v::Cold);
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<RH_int_hot>, extract::residency_heat_value_t<RH_int_hot>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfRH = safety::Consistency<extract::Consistency_v::STRONG, RH_int_hot>;
    using RHOfConsistency = safety::ResidencyHeat<extract::ResidencyHeatTag_v::Hot, C_int_strong>;

    static_assert(extract::is_consistency_v<ConsistencyOfRH>);
    static_assert(!extract::is_residency_heat_v<ConsistencyOfRH>);
    static_assert(extract::is_residency_heat_v<extract::consistency_value_t<ConsistencyOfRH>>);

    static_assert(extract::is_residency_heat_v<RHOfConsistency>);
    static_assert(!extract::is_consistency_v<RHOfConsistency>);
    static_assert(extract::is_consistency_v<extract::residency_heat_value_t<RHOfConsistency>>);
}
...
void test_cipher_tier_vs_residency_heat_distinct() {
    // These two wrappers read alike: a three-tier tag enum as a template
    // parameter over one element type.  They stay distinct types anyway,
    // because the detectors key on the wrapper class and not on the tag
    // enum or the lattice.  Unifying the two behind a shared base would
...
void test_runtime_consistency() {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        EXPECT_TRUE(baseline_pos == extract::is_residency_heat_v<RH_int_hot>);
        EXPECT_TRUE(baseline_neg == !extract::is_residency_heat_v<int>);
...
int main() {
    ...
    run_test("test_tag_extraction_every_value", test_tag_extraction_every_value);
    run_test("test_tag_extraction_cv_ref_strips", test_tag_extraction_cv_ref_strips);
    run_test("test_distinct_tiers_distinct_specs", test_distinct_tiers_distinct_specs);
    ...
    run_test("test_positive_nested_residency_heat", test_positive_nested_residency_heat);
    ...
    run_test("test_cipher_tier_vs_residency_heat_distinct", test_cipher_tier_vs_residency_heat_distinct);
