// mint_durable_append_file carries its own requires-clause over the
// three atoms it fixes, so a context without Block is refused at this
// signature rather than one layer down inside mint_file.
...
int main() {
    ...
    [[maybe_unused]] auto r = fs::mint_durable_append_file(ctx, sanitized("/tmp/fixy-neg-fs"));
    return 0;
}
