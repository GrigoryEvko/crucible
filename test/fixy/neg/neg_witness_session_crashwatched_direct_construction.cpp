...
//
// fixy::session::CrashWatched<fixy::session::SessionHandle<fixy::session::End, int*>, int, long, fixy::session::ReliableSet<>> attests to a fact it cannot see.  The expression below is the
...
int main() {
    [[maybe_unused]] auto forged = fixy::session::CrashWatched<fixy::session::SessionHandle<fixy::session::End, int*>, int, long, fixy::session::ReliableSet<>>{forge::rvalue<fixy::session::SessionHandle<fixy::session::End, int*>>(), forge::lvalue<const fixy::session::PeerCrashCell>()};
    return 0;
}
