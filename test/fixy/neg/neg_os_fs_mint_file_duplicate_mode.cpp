// A file is opened with exactly one mode.  Two mode atoms in one pack
// would fold two O_* words together; has_duplicate_mode_v is true and
// the gate refuses the instantiation.
...
int main() {
    ...
    [[maybe_unused]] auto r =
        fs::mint_file<atom_fs::mode<fs::open_mode::ReadOnly>, atom_fs::mode<fs::open_mode::WriteTruncate>>(
            ctx, sanitized("/tmp/fixy-neg-fs"));
    return 0;
}
