...
int main() {
    ...
    auto plan = crucible::cntp::mint_gossip_multicast_plan<4, 4>(init, iface, ifindex);
    crucible::cntp::GossipTopicKey raw{.hash = 7};
    auto target = crucible::cntp::gossip_neighbor_target(crucible::cog::CogIdentity{.uuid = crucible::cog::Uuid{1, 2}},
                                                         ifindex, {}, 0x0A000001U);
