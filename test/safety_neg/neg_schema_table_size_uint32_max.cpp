...
// SCHEMA_TABLE_CAP bound.
//
// Per WRAP-SchemaTab-1 (#1003), SchemaTable::SizeCounter is
// safety::BoundedMonotonic<uint32_t, SCHEMA_TABLE_CAP=512>.
...
//
// Companion fixture: neg_schema_table_size_above_cap.cpp
...
//     `BoundedMonotonic<uint32_t, SCHEMA_TABLE_CAP>` into a plain
//     `Monotonic<uint32_t>` (or an unwrapped uint32_t typedef);
//     under that drift any size value is silently accepted at
//     construction and the only remaining defense is register_name's
//     silent-truncation runtime check (and crucially, the
//     compile-time witness that would fire BEFORE any register_name()
