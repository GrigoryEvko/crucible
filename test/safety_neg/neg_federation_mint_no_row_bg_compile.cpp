...
int main() {
    ...
    auto handshake = perm::make_self_signed_handshake<neg_fed_row_bg::PeerOrg>(
        /*peer_key_fp=*/perm::PeerKeyFingerprint{0xFEDCDEULL},
        /*nonce=*/perm::Nonce{0xC0FFEEULL});
    auto admitted =
        perm::mint_federation_admittance<neg_fed_row_bg::PeerOrg, perm::policy::admit_orgs<neg_fed_row_bg::PeerOrg>>(
            local, handshake);
    auto pool = fp::mint_federation_pool<neg_fed_row_bg::PeerOrg>(std::move(*admitted));
