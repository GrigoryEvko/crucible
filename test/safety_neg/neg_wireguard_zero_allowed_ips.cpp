...
int main() {
    auto key =
        crucible::cntp::_wip::admit_wireguard_public_key_b64("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB=").value();
    ...
    (void)crucible::cntp::_wip::declare_wireguard_peer(
        key, crucible::cntp::_wip::WireguardEndpoint{.ipv4_be = 0xc0000201u, .port = port}, allowed);
}
