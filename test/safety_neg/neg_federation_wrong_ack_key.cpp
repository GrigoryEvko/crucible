...
using SenderHandle = decltype(fp::mint_sender<PeerOrg, ExpectedKey>(std::declval<FederationFitCtx const&>(), Endpoint{},
                                                                    std::declval<Admittance>()));

using AfterAnnounce = decltype(std::declval<SenderHandle&&>().send(
    fp::HeaderPayload<ExpectedKey>{}, [](Endpoint&, fp::HeaderPayload<ExpectedKey>&&) noexcept {}));

template <typename Handle>
concept CanReceiveWrongAck =
    requires(Handle h) { std::move(h).recv([](Endpoint&) noexcept -> fp::Ack<WrongKey> { return {}; }); };

static_assert(CanReceiveWrongAck<AfterAnnounce>, "FederationAck_ContentHashMismatch_Rejected");
