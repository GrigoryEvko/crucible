...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_recipe_spec_smoke_test()); }
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RS_int const&>, int>);
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RS_int&&>, int>);
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RS_double const>, double>);
}

void test_distinct_element_types_distinct_specs() {
    static_assert(!std::is_same_v<RS_int, RS_double>);
    static_assert(!std::is_same_v<extract::recipe_spec_value_t<RS_int>, extract::recipe_spec_value_t<RS_double>>);
...
void test_runtime_grade_independence_from_type_identity() {
    RS_int rs_default{};
    RS_int rs_explicit{0, safety::Tolerance::BITEXACT, safety::RecipeFamily::Kahan};
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RS_struct>, payload_struct>);
}

void test_positive_nested_recipe_spec() {
    static_assert(extract::is_recipe_spec_v<RS_nested>);
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RS_nested>, RS_int>);
    static_assert(extract::is_recipe_spec_v<extract::recipe_spec_value_t<RS_nested>>);
    static_assert(std::is_same_v<extract::recipe_spec_value_t<extract::recipe_spec_value_t<RS_nested>>, int>);
}
...
void test_runtime_grade_layout_invariant() {
    // The grade is 2 bytes: one for Tolerance, one for RecipeFamily.
    ...
    // A compile-time grade costs nothing, unlike the runtime grade above.
...
void test_three_distinct_product_wrapper_layouts() {
    // Grade sizes: Budgeted and EpochVersioned 16 bytes each,
    // NumaPlacement 40, RecipeSpec 2.  Only Budgeted and EpochVersioned
    // are layout-interchangeable.
    static_assert(sizeof(RS_int) < sizeof(B_int));
    static_assert(sizeof(B_int) < sizeof(NP_int));
    static_assert(sizeof(B_int) == sizeof(EV_int));
...
void test_shared_sub_lattice_distinct_wrapper() {
    // Both wrappers key on Tolerance.  RecipeSpec carries it as a
    // runtime value, NumericalTier pins it as a template argument.
    // Sharing a sub-lattice must not collapse the two identities.
...
void test_public_aliases_round_trip() {
    ...
    static_assert(std::is_same_v<extract::recipe_spec_value_t<RecipeSpecInt>, int>);
}
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<RS_int>, extract::recipe_spec_value_t<RS_int>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfRs = safety::Consistency<extract::Consistency_v::STRONG, RS_int>;
    ...
    static_assert(extract::is_recipe_spec_v<extract::consistency_value_t<ConsistencyOfRs>>);

    static_assert(extract::is_recipe_spec_v<RsOfConsistency>);
    static_assert(!extract::is_consistency_v<RsOfConsistency>);
    static_assert(extract::is_consistency_v<extract::recipe_spec_value_t<RsOfConsistency>>);
}
...
int main() {
    ...
    run_test("test_distinct_element_types_distinct_specs", test_distinct_element_types_distinct_specs);
    run_test("test_runtime_grade_independence_from_type_identity", test_runtime_grade_independence_from_type_identity);
    run_test("test_dispatcher_value_type_use_case", test_dispatcher_value_type_use_case);
    run_test("test_positive_non_fundamental_wrapped_type", test_positive_non_fundamental_wrapped_type);
    run_test("test_positive_nested_recipe_spec", test_positive_nested_recipe_spec);
    ...
    run_test("test_runtime_grade_layout_invariant", test_runtime_grade_layout_invariant);
    run_test("test_three_distinct_product_wrapper_layouts", test_three_distinct_product_wrapper_layouts);
    run_test("test_shared_sub_lattice_distinct_wrapper", test_shared_sub_lattice_distinct_wrapper);
    run_test("test_public_aliases_round_trip", test_public_aliases_round_trip);
