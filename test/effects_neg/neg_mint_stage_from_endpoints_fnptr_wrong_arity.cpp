...
inline void body3(typename ChIn::ConsumerHandle&&, typename ChOut::ProducerHandle&&, int) noexcept {}
...
int main() {
    ...
    auto bad = conc::mint_stage_from_endpoints<&body3>(ctx, std::move(cons_ep), std::move(prod_ep));
    (void)bad;
    (void)ppi;
    (void)cpo;
    return 0;
