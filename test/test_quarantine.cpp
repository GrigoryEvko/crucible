...
static topology::HealthSnapshot health(std::uint16_t score, topology::HealthState state, std::uint64_t sequence) {
...
static void test_name_accessors() {
    assert(warden::quarantine_state_name(warden::QuarantineState::Quarantined) == std::string_view{"Quarantined"});
...
static void test_health_hysteresis() {
    auto policy = warden::mint_quarantine_policy<effects::ColdInitCtx, 4>(effects::ColdInitCtx{}, test_config());
    auto target = peer(1);
    ...
    assert(policy.on_health_event(effects::BgDrainCtx{}, target, health(1000, topology::HealthState::Healthy, 3), 300));
...
static void test_asymmetric_failure_policy() {
    ...
    auto tx = peer(2);
    assert(policy.on_asymmetric_failure(effects::BgDrainCtx{}, tx, topology::FailureClass::TxBroken, 100, 1));
    ...
    auto dead = peer(3);
    assert(policy.on_asymmetric_failure(effects::BgDrainCtx{}, dead, topology::FailureClass::BidiFailed, 100, 2));
...
static void test_event_ring_wrap_chronological_order() {
    // Indexing must be chronological, not positional.  A four-slot ring
    // is small enough that six transitions wrap it twice, leaving events
    // three through six alive with the oldest of them no longer in slot
    // zero.  The alternating states make each event distinguishable.
    auto policy = warden::mint_quarantine_policy<effects::ColdInitCtx, 2, 4>(effects::ColdInitCtx{}, test_config());
    auto target = peer(99);
    auto authority = saf::mint_permission_root<warden::quarantine_tag::OperatorOverride>();
    ...
    for (std::uint64_t seq = 1; seq <= 6; ++seq) {
        ...
        authority =
            policy.operator_override(effects::ColdInitCtx{}, std::move(authority), target, next, 100u * seq, seq);
    }
    saf::permission_drop(std::move(authority));
    ...
    // Slot zero holds event five, which overwrote it, so reading the
    // storage positionally answers with the wrong event.
    assert(oldest->sequence == 3u);
    assert(newest->sequence == 6u);
    // Odd sequences quarantine and even ones return to healthy.
