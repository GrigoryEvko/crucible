...
void fuzz_all_in_range() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyAllInRange);
        ...
        for (std::size_t k = 0; k < len; ++k) {
            ...
            auto const sub = generate(static_cast<std::uint64_t>(i) * 64 + k, kKeyAllInRange);
...
void fuzz_strictly_increasing() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyStrictInc);
        std::size_t const len = ctr[0] % (kMaxSpanLen + 1);
        ...
        for (std::size_t k = 0; k < len; ++k) {
            auto const sub = generate(static_cast<std::uint64_t>(i) * 64 + k, kKeyStrictInc);
            if (make_monotone) {
...
void fuzz_factorization_eq() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyFactorEq);
        ...
        for (std::size_t k = 0; k < len; ++k) {
            auto const sub = generate(static_cast<std::uint64_t>(i) * 64 + k, kKeyFactorEq);
...
void fuzz_conjunction() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyConjunction);
        std::size_t const len = ctr[0] % (kMaxSpanLen + 1);
        ...
        for (std::size_t k = 0; k < len; ++k) {
            ...
            auto const sub = generate(static_cast<std::uint64_t>(i) * 64 + k, kKeyConjunction);
            buf[k] = (sub[0] & 0x3u) != 0u;
        }
...
void fuzz_disjunction() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyDisjunction);
        std::size_t const len = ctr[0] % (kMaxSpanLen + 1);
        bool buf[kMaxSpanLen]{};
        for (std::size_t k = 0; k < len; ++k) {
            // The mirror of the bias above, for the same reason: the
            // rejecting case is the rare one here.
            auto const sub = generate(static_cast<std::uint64_t>(i) * 64 + k, kKeyDisjunction);
            buf[k] = (sub[0] & 0x3u) == 0u;
        }
        std::span<const bool> xs{buf, len};
...
void fuzz_aligned_in_range() {
    for (int i = 0; i < kIterations; ++i) {
        auto const ctr = generate(static_cast<std::uint64_t>(i), kKeyAlignInRange);
