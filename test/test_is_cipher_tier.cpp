...
// These five stand in for the other shipped wrappers, so the exclusion
// case below can show the trait admits none of them.
struct test_tag {};
...
using CT_struct_hot = safety::CipherTier<extract::CipherTierTag_v::Hot, payload_struct>;
using CT_nested = safety::CipherTier<extract::CipherTierTag_v::Warm, CT_int_hot>;
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_cipher_tier_smoke_test()); }
...
void test_value_type_extraction() {
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_int_hot>, int>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_int_cold>, int>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_double_hot>, double>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_float_cold>, float>);
}

void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_int_hot const&>, int>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_int_hot&&>, int>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_double_hot const>, double>);
}

void test_tag_extraction_every_value() {
    static_assert(extract::cipher_tier_tag_v<CT_int_hot> == extract::CipherTierTag_v::Hot);
    static_assert(extract::cipher_tier_tag_v<CT_int_warm> == extract::CipherTierTag_v::Warm);
    static_assert(extract::cipher_tier_tag_v<CT_int_cold> == extract::CipherTierTag_v::Cold);
}

void test_tag_extraction_cv_ref_strips() {
    static_assert(extract::cipher_tier_tag_v<CT_int_hot const&> == extract::CipherTierTag_v::Hot);
    static_assert(extract::cipher_tier_tag_v<CT_int_cold&&> == extract::CipherTierTag_v::Cold);
}

void test_distinct_tiers_distinct_specs() {
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_int_hot>, extract::cipher_tier_value_t<CT_int_cold>>);
    static_assert(extract::cipher_tier_tag_v<CT_int_hot> != extract::cipher_tier_tag_v<CT_int_cold>);

    static_assert(extract::cipher_tier_tag_v<CT_int_hot> == extract::cipher_tier_tag_v<CT_double_hot>);
    static_assert(
        !std::is_same_v<extract::cipher_tier_value_t<CT_int_hot>, extract::cipher_tier_value_t<CT_double_hot>>);
}
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_struct_hot>, payload_struct>);
    static_assert(extract::cipher_tier_tag_v<CT_struct_hot> == extract::CipherTierTag_v::Hot);
}

void test_positive_nested_cipher_tier() {
    static_assert(extract::is_cipher_tier_v<CT_nested>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<CT_nested>, CT_int_hot>);
    static_assert(extract::is_cipher_tier_v<extract::cipher_tier_value_t<CT_nested>>);
    static_assert(std::is_same_v<extract::cipher_tier_value_t<extract::cipher_tier_value_t<CT_nested>>, int>);
}
...
void test_dispatcher_function_parameter_use_case() {
    ...
    static_assert(ex::cipher_tier_tag_v<P0_cold> == ex::CipherTierTag_v::Cold);
...
void test_local_alias_round_trip() {
    ...
    static_assert(extract::cipher_tier_tag_v<LocalHotInt> == extract::CipherTierTag_v::Hot);
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::cipher_tier_tag_v<HotInt> == extract::CipherTierTag_v::Hot);
    static_assert(extract::cipher_tier_tag_v<WarmInt> == extract::CipherTierTag_v::Warm);
    static_assert(extract::cipher_tier_tag_v<ColdInt> == extract::CipherTierTag_v::Cold);
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<CT_int_hot>, extract::cipher_tier_value_t<CT_int_hot>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfCipherTier = safety::Consistency<extract::Consistency_v::STRONG, CT_int_hot>;
    using CipherTierOfConsistency = safety::CipherTier<extract::CipherTierTag_v::Hot, C_int_strong>;
    ...
    static_assert(extract::is_cipher_tier_v<extract::consistency_value_t<ConsistencyOfCipherTier>>);
    ...
    static_assert(extract::is_consistency_v<extract::cipher_tier_value_t<CipherTierOfConsistency>>);
}

void test_cross_wrapper_exclusion() {
    // The trait is disjoint from every other shipped wrapper.
    static_assert(extract::is_cipher_tier_v<CT_int_hot>);
...
void test_runtime_consistency() {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        EXPECT_TRUE(baseline_pos == extract::is_cipher_tier_v<CT_int_hot>);
        EXPECT_TRUE(baseline_neg == !extract::is_cipher_tier_v<int>);
...
int main() {
    ...
    run_test("test_positive_nested_cipher_tier", test_positive_nested_cipher_tier);
