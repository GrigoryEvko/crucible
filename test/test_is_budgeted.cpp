// Budgeted takes only an element type.  Both of its budget axes are
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_budgeted_smoke_test()); }
...
void test_negative_lookalike_struct() {
    // The detector keys on the wrapper class, not on the storage shape,
    // so a struct with the same fields is still rejected.
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::budgeted_value_t<B_int const&>, int>);
    static_assert(std::is_same_v<extract::budgeted_value_t<B_int&&>, int>);
    static_assert(std::is_same_v<extract::budgeted_value_t<B_double const>, double>);
}

void test_distinct_element_types_distinct_specs() {
    static_assert(!std::is_same_v<B_int, B_double>);
    static_assert(!std::is_same_v<extract::budgeted_value_t<B_int>, extract::budgeted_value_t<B_double>>);
...
void test_runtime_grade_independence_from_type_identity() {
    // Promoting the budget axes to template parameters would defeat the
    // point of measuring them at the construction site, and it would show
    // up here first: two values with different budgets must stay one type.
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::budgeted_value_t<B_struct>, payload_struct>);
}

void test_positive_nested_budgeted() {
    static_assert(extract::is_budgeted_v<B_nested>);
    static_assert(std::is_same_v<extract::budgeted_value_t<B_nested>, B_int>);
    static_assert(extract::is_budgeted_v<extract::budgeted_value_t<B_nested>>);
    static_assert(std::is_same_v<extract::budgeted_value_t<extract::budgeted_value_t<B_nested>>, int>);
}
...
void test_runtime_grade_layout_invariant() {
    // The grade is a pair of runtime 64-bit values, so it adds 16 bytes
    // on top of the element.  Only the case whose alignment already
    // matches lands on that figure exactly.
    ...
    // A wrapper whose grade is a template parameter collapses to nothing
    // through the empty base.  This one cannot, because its grade is
    // per-instance data.
...
void test_public_aliases_round_trip() {
    ...
    static_assert(std::is_same_v<extract::budgeted_value_t<BudgetedInt>, int>);
    static_assert(std::is_same_v<extract::budgeted_value_t<BudgetedDouble>, double>);
}
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<B_int>, extract::budgeted_value_t<B_int>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfB = safety::Consistency<extract::Consistency_v::STRONG, B_int>;
    ...
    static_assert(extract::is_budgeted_v<extract::consistency_value_t<ConsistencyOfB>>);

    static_assert(extract::is_budgeted_v<BOfConsistency>);
    static_assert(!extract::is_consistency_v<BOfConsistency>);
    static_assert(extract::is_consistency_v<extract::budgeted_value_t<BOfConsistency>>);
}
...
int main() {
    ...
    run_test("test_positive_nested_budgeted", test_positive_nested_budgeted);
