...
[[nodiscard]] auto mint_handles(::crucible::MetaLog& raw_log) {
    ...
    auto [pp, cp] =
        mint_permission_split<PermissionedLog::producer_tag, PermissionedLog::consumer_tag>(std::move(whole));
...
void test_permissioned_bulk_drain() {
    ...
    const std::uint32_t count = consumer.drain([&](const ::crucible::TensorMeta& meta) { drained.push_back(meta); });
...
void test_typed_session_round_trip() {
    ...
    std::jthread prod_thread{[&producer, &producer_done](auto) mutable {
        auto psh = ses::mint_metalog_producer_session<PermissionedLog>(::crucible::effects::HotFgCtx{}, producer);
        for (int i = 0; i < kCount; ++i) {
            auto next = std::move(psh).send(make_meta(i + 10), ses::blocking_append);
            psh = std::move(next);
        }
    ...
    std::jthread cons_thread{[&consumer, &received](auto) mutable {
        auto psh = ses::mint_metalog_consumer_session<PermissionedLog>(::crucible::effects::HotFgCtx{}, consumer);
        for (int i = 0; i < kCount; ++i) {
...
// The permission set is carried in the type only.  Dressing a session
// handle in one must not make the handle any larger.
namespace witness {
namespace proto = ::crucible::safety::proto;
using PSH_End_Prod =
    proto::PermissionedSessionHandle<proto::End, proto::EmptyPermSet, PermissionedLog::ProducerHandle*>;
using SH_End_Prod = proto::SessionHandle<proto::End, PermissionedLog::ProducerHandle*>;
static_assert(sizeof(PSH_End_Prod) == sizeof(SH_End_Prod));

using PSH_End_Cons =
    proto::PermissionedSessionHandle<proto::End, proto::EmptyPermSet, PermissionedLog::ConsumerHandle*>;
using SH_End_Cons = proto::SessionHandle<proto::End, PermissionedLog::ConsumerHandle*>;
