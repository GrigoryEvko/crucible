...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_det_safe_smoke_test()); }
...
void test_value_type_extraction() {
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_int_pure>, int>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_int_nds>, int>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_double_pure>, double>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_float_philox>, float>);
}

void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_int_pure const&>, int>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_int_pure&&>, int>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_double_pure const>, double>);
}

void test_tier_extraction_every_value() {
    static_assert(extract::det_safe_tier_v<DS_int_pure> == extract::DetSafeTier_v::Pure);
    static_assert(extract::det_safe_tier_v<DS_int_philox> == extract::DetSafeTier_v::PhiloxRng);
    static_assert(extract::det_safe_tier_v<DS_int_monoclock> == extract::DetSafeTier_v::MonotonicClockRead);
    static_assert(extract::det_safe_tier_v<DS_int_wallclock> == extract::DetSafeTier_v::WallClockRead);
    static_assert(extract::det_safe_tier_v<DS_int_entropy> == extract::DetSafeTier_v::EntropyRead);
    static_assert(extract::det_safe_tier_v<DS_int_fsmtime> == extract::DetSafeTier_v::FilesystemMtime);
    static_assert(extract::det_safe_tier_v<DS_int_nds> == extract::DetSafeTier_v::NonDeterministicSyscall);
}

void test_tier_extraction_cv_ref_strips() {
    static_assert(extract::det_safe_tier_v<DS_int_pure const&> == extract::DetSafeTier_v::Pure);
    static_assert(extract::det_safe_tier_v<DS_int_nds&&> == extract::DetSafeTier_v::NonDeterministicSyscall);
}

void test_distinct_tiers_distinct_specs() {
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_int_pure>, extract::det_safe_value_t<DS_int_nds>>);
    static_assert(extract::det_safe_tier_v<DS_int_pure> != extract::det_safe_tier_v<DS_int_nds>);

    static_assert(extract::det_safe_tier_v<DS_int_pure> == extract::det_safe_tier_v<DS_double_pure>);
    static_assert(!std::is_same_v<extract::det_safe_value_t<DS_int_pure>, extract::det_safe_value_t<DS_double_pure>>);
}
...
void test_dispatcher_integration_example() {
        ...
        requires extract::IsDetSafe<T>
    {
        ...
        if constexpr (tier == extract::DetSafeTier_v::Pure) {
            ...
            return 6;
        } else if constexpr (tier == extract::DetSafeTier_v::MonotonicClockRead) {
            return 5;
        } else if constexpr (tier == extract::DetSafeTier_v::WallClockRead) {
            return 4;
        } else if constexpr (tier == extract::DetSafeTier_v::EntropyRead) {
            return 3;
        } else if constexpr (tier == extract::DetSafeTier_v::FilesystemMtime) {
            return 2;
        } else {
            return 1;  // most permissive
        }
    };
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_struct_pure>, payload_struct>);
    static_assert(extract::det_safe_tier_v<DS_struct_pure> == extract::DetSafeTier_v::Pure);
}

void test_positive_nested_det_safe() {
    static_assert(extract::is_det_safe_v<DS_nested>);
    static_assert(std::is_same_v<extract::det_safe_value_t<DS_nested>, DS_int_pure>);
    static_assert(extract::is_det_safe_v<extract::det_safe_value_t<DS_nested>>);
    static_assert(std::is_same_v<extract::det_safe_value_t<extract::det_safe_value_t<DS_nested>>, int>);
}
...
void test_dispatcher_function_parameter_use_case() {
    ...
    static_assert(ex::det_safe_tier_v<P0_nds> == ex::DetSafeTier_v::NonDeterministicSyscall);
...
void test_local_alias_round_trip() {
    ...
    static_assert(extract::det_safe_tier_v<LocalPureInt> == extract::DetSafeTier_v::Pure);
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::det_safe_tier_v<PureInt> == extract::DetSafeTier_v::Pure);
    static_assert(extract::det_safe_tier_v<PhiloxInt> == extract::DetSafeTier_v::PhiloxRng);
    static_assert(extract::det_safe_tier_v<NDSInt> == extract::DetSafeTier_v::NonDeterministicSyscall);
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<DS_int_pure>, extract::det_safe_value_t<DS_int_pure>>);
    static_assert(std::is_same_v<extract::value_type_of_t<DS_double_pure>, extract::det_safe_value_t<DS_double_pure>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfDetSafe = safety::Consistency<extract::Consistency_v::STRONG, DS_int_pure>;
    using DetSafeOfConsistency = safety::DetSafe<extract::DetSafeTier_v::Pure, C_int_strong>;

    static_assert(extract::is_consistency_v<ConsistencyOfDetSafe>);
    static_assert(!extract::is_det_safe_v<ConsistencyOfDetSafe>);
    static_assert(extract::is_det_safe_v<extract::consistency_value_t<ConsistencyOfDetSafe>>);

    static_assert(extract::is_det_safe_v<DetSafeOfConsistency>);
    static_assert(!extract::is_consistency_v<DetSafeOfConsistency>);
    static_assert(extract::is_consistency_v<extract::det_safe_value_t<DetSafeOfConsistency>>);
}
...
void test_runtime_consistency() {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        EXPECT_TRUE(baseline_pos == extract::is_det_safe_v<DS_int_pure>);
        EXPECT_TRUE(baseline_neg == !extract::is_det_safe_v<int>);
...
int main() {
    ...
    run_test("test_tier_extraction_every_value", test_tier_extraction_every_value);
    run_test("test_tier_extraction_cv_ref_strips", test_tier_extraction_cv_ref_strips);
    ...
    run_test("test_positive_nested_det_safe", test_positive_nested_det_safe);
