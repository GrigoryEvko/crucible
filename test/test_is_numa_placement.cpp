...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_numa_placement_smoke_test()); }
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::numa_placement_value_t<NP_int const&>, int>);
    static_assert(std::is_same_v<extract::numa_placement_value_t<NP_int&&>, int>);
    static_assert(std::is_same_v<extract::numa_placement_value_t<NP_double const>, double>);
}

void test_distinct_element_types_distinct_specs() {
    static_assert(!std::is_same_v<NP_int, NP_double>);
    static_assert(!std::is_same_v<extract::numa_placement_value_t<NP_int>, extract::numa_placement_value_t<NP_double>>);
...
void test_runtime_grade_independence_from_type_identity() {
    NP_int np_default{};
    NP_int np_explicit{0, safety::NumaNodeId::Any, safety::AffinityMask{}};
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::numa_placement_value_t<NP_struct>, payload_struct>);
}

void test_positive_nested_numa_placement() {
    static_assert(extract::is_numa_placement_v<NP_nested>);
    static_assert(std::is_same_v<extract::numa_placement_value_t<NP_nested>, NP_int>);
    static_assert(extract::is_numa_placement_v<extract::numa_placement_value_t<NP_nested>>);
    static_assert(std::is_same_v<extract::numa_placement_value_t<extract::numa_placement_value_t<NP_nested>>, int>);
}
...
void test_runtime_grade_layout_invariant() {
    // The grade holds one node id plus a 32-byte affinity mask, so 33
    // bytes of content.  Alignment of the mask pads it out to 40.
    ...
    // A wrapper carrying its grade as a template parameter instead of a
    // member costs nothing, which is the contrast drawn here.
    static_assert(sizeof(NP_int) > sizeof(int));
    static_assert(sizeof(V_int_nv) == sizeof(int));
}

void test_distinct_product_wrapper_layouts() {
    // Grade size is not uniform across the wrappers that carry a runtime
    // grade.  Code walking a tuple of them cannot assume one overhead,
    // and these three sizes disagree by construction.
...
void test_public_aliases_round_trip() {
    ...
    static_assert(std::is_same_v<extract::numa_placement_value_t<NumaPlacementInt>, int>);
}
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<NP_int>, extract::numa_placement_value_t<NP_int>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfNp = safety::Consistency<extract::Consistency_v::STRONG, NP_int>;
    ...
    static_assert(extract::is_numa_placement_v<extract::consistency_value_t<ConsistencyOfNp>>);

    static_assert(extract::is_numa_placement_v<NpOfConsistency>);
    static_assert(!extract::is_consistency_v<NpOfConsistency>);
    static_assert(extract::is_consistency_v<extract::numa_placement_value_t<NpOfConsistency>>);
}
...
int main() {
    ...
    run_test("test_positive_nested_numa_placement", test_positive_nested_numa_placement);
    ...
    run_test("test_distinct_product_wrapper_layouts", test_distinct_product_wrapper_layouts);
