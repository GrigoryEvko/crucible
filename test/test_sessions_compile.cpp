...
// unit pulls the header in. This file pulls in every session header so
// each block runs under the test target's full warning matrix. It does not
// duplicate the dedicated per-header tests. When a new session header
...
// One probe per header. The bodies are empty on purpose: including the
// header is the whole test, and a body would only add a second claim.
...
void test_session_row_extraction_compile() { crucible::safety::proto::runtime_smoke_test_payload_row(); }
void test_session_mint_compile() { crucible::safety::proto::runtime_smoke_test_session_mint(); }
