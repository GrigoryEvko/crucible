...
void test_redundant_equal_results_mint_verified_tag() {
    auto detector = observe::mint_sdc_detector<effects::ColdInitCtx, 4, 8>(effects::ColdInitCtx{});
    ...
    auto verified = detector.run_with_redundancy(
        effects::BgDrainCtx{}, [](cog::CogIdentity const&) noexcept { return std::uint64_t{0xfeedcafe}; });
...
void test_mismatch_records_implicated_cog_and_threshold() {
    ...
    auto detector = observe::mint_sdc_detector<effects::ColdInitCtx, 3, 4>(effects::ColdInitCtx{}, config);
    ...
    auto result = detector.run_with_redundancy(effects::BgDrainCtx{}, [](cog::CogIdentity const& id) noexcept {
...
void test_signed_tolerance_does_not_use_modular_distance() {
    ...
    auto detector = observe::mint_sdc_detector<effects::ColdInitCtx, 2, 4>(effects::ColdInitCtx{}, config);
...
void test_insufficient_replicas_and_observation_publication() {
    ...
    auto result = detector.run_with_redundancy(effects::BgDrainCtx{},
...
void test_sampling_decision_is_deterministic() {
    observe::SdcConfig config{};
    config.sampling_rate_ppm = observe::SdcSamplingRatePpm{1000000};
    auto all = observe::mint_sdc_detector<effects::ColdInitCtx, 1, 1>(effects::ColdInitCtx{}, config);
    CRUCIBLE_REQUIRE(all.should_sample(42));

    config.sampling_rate_ppm = observe::SdcSamplingRatePpm{1};
    auto sparse = observe::mint_sdc_detector<effects::ColdInitCtx, 1, 1>(effects::ColdInitCtx{}, config);
