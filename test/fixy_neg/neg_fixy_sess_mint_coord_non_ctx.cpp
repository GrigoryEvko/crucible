...
int main() {
    ...
    auto handshake = perm::make_self_signed_handshake<neg_fixy_coord_non_ctx::PeerOrg>(
        /*peer_key_fp=*/perm::PeerKeyFingerprint{0xC04DCAULL},
        /*nonce=*/perm::Nonce{0xC0DEC3ULL});
    auto admitted =
        perm::mint_federation_admittance<neg_fixy_coord_non_ctx::PeerOrg,
                                         perm::policy::admit_orgs<neg_fixy_coord_non_ctx::PeerOrg>>(local, handshake);
    auto pool = fp::mint_federation_pool<neg_fixy_coord_non_ctx::PeerOrg>(std::move(*admitted));
    auto guard = pool.lend();

    int not_a_ctx = 0;
    auto bad = fsess::mint_coord<neg_fixy_coord_non_ctx::PeerOrg, neg_fixy_coord_non_ctx::TraceKey>(
        not_a_ctx, neg_fixy_coord_non_ctx::Endpoint{}, guard->token());
    (void)bad;
    return 0;
}
