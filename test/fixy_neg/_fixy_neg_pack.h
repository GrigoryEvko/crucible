...
template <D Omit>
using PackWithout = std::tuple<std::conditional_t<Omit == D::Type, void, strict<D::Type>>,
...
template <typename... Ts>
struct filter_void_tuple<std::tuple<Ts...>> : filter_void<Ts...> {};
...
#define FIXY_NEG_FIXTURE(AxisName)                                                                          \
    using AxisName##Probe =                                                                                 \
        ::fixy_neg_detail::rejects_via<::fixy_neg_detail::FilteredPack<::fixy_neg_detail::D::AxisName>>;    \
    static_assert(AxisName##Probe::value, "IsAcceptedGrants must reject when " #AxisName " is unengaged."); \
    static_assert(AxisName##Probe::first_missing == ::fixy_neg_detail::D::AxisName,                         \
                  "first_missing_axis_v must point at " #AxisName);                                         \
    using AxisName##Tag = ::crucible::fixy::diag::tag_for_axis_t<::fixy_neg_detail::D::AxisName>;           \
    ...
    static_assert(sizeof(AxisName##Tag) > 0 && false,                                                       \
                  ...
                  "grant::accept_default_strict_for<dim::DimensionAxis::" #AxisName "> OR a per-axis relaxation tag.")
