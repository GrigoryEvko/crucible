...
int main() {
    using Snap = conc::PermissionedSnapshot<int,
        neg_fixy_substr_snapshot_reader_session_non_ctx::UserTag>;

    Snap snap{};
    auto reader_opt = snap.reader();

    [[maybe_unused]] auto bad =
        fsubstr::snapshot::mint_snapshot_reader_session<Snap>(
            neg_fixy_substr_snapshot_reader_session_non_ctx::NotAnExecCtx{},
            *reader_opt);
    return 0;
}
