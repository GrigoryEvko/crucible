...
int main() {
    int not_a_ctx = 0;
    neg_fixy_meta_producer_session_non_ctx::Log::ProducerHandle* handle = nullptr;

    auto bad = fmeta::mint_metalog_producer_session<neg_fixy_meta_producer_session_non_ctx::Log>(not_a_ctx, *handle);
    (void)bad;
    return 0;
}
