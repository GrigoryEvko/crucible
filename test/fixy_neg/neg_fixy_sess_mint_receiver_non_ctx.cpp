...
int main() {
    ...
    auto handshake = perm::make_self_signed_handshake<neg_fixy_receiver_non_ctx::PeerOrg>(
        /*peer_key_fp=*/perm::PeerKeyFingerprint{0xCEDEAFULL},
        /*nonce=*/perm::Nonce{0xC0DEC1ULL});
    auto admitted = perm::mint_federation_admittance<neg_fixy_receiver_non_ctx::PeerOrg,
                                                     perm::policy::admit_orgs<neg_fixy_receiver_non_ctx::PeerOrg>>(
        local, handshake);
    auto pool = fp::mint_federation_pool<neg_fixy_receiver_non_ctx::PeerOrg>(std::move(*admitted));
