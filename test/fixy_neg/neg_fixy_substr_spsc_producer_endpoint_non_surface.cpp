...
//
// FIXY-V-045-audit fixture #1 for fixy::substr::spsc::mint_spsc_producer_endpoint
...
// neg_fixy_substr_spsc_producer_endpoint_near_miss.cpp (#2): there a type
...
// `Permission<typename Channel::producer_tag>&&`.  With Channel=FakeChannel,
// `typename FakeChannel::producer_tag` is invalid → parameter-list
...
// satisfied / no matching function / producer_tag.
...
namespace neg_fixy_substr_spsc_producer_endpoint_non_surface {
...
// because `typename FakeChannel::producer_tag` does not exist.
struct FakeChannel {};
...
// FakeChannel::producer_tag, but we never get that far — substitution
// fails earlier.
struct producer_tag_placeholder {};
}  // namespace neg_fixy_substr_spsc_producer_endpoint_non_surface

int main() {
    neg_fixy_substr_spsc_producer_endpoint_non_surface::FakeChannel fake{};
    auto perm =
        saf::mint_permission_root<neg_fixy_substr_spsc_producer_endpoint_non_surface::producer_tag_placeholder>();

    [[maybe_unused]] auto bad = fsubstr::spsc::mint_spsc_producer_endpoint(fake, std::move(perm));
    return 0;
}
