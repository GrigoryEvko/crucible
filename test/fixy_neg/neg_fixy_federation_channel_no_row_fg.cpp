...
int main() {
    ...
    auto handshake = perm::make_self_signed_handshake<neg_fixy_fed_fg::PeerOrg>(
        /*peer_key_fp=*/perm::PeerKeyFingerprint{0xFEDCDEULL},
        /*nonce=*/perm::Nonce{0xC0FFEEULL});
    auto admitted =
        perm::mint_federation_admittance<neg_fixy_fed_fg::PeerOrg, perm::policy::admit_orgs<neg_fixy_fed_fg::PeerOrg>>(
            local, handshake);
    auto pool = fp::mint_federation_pool<neg_fixy_fed_fg::PeerOrg>(std::move(*admitted));
    auto guard = pool.lend();

    eff::HotFgCtx fg{};
    auto channel = fsess::mint_federation_channel<neg_fixy_fed_fg::PeerOrg, neg_fixy_fed_fg::TraceKey>(
        fg, neg_fixy_fed_fg::Endpoint{}, neg_fixy_fed_fg::Endpoint{}, guard->token());
    (void)channel;
    return 0;
}
