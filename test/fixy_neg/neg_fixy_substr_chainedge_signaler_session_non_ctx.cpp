...
int main() {
    int not_a_ctx = 0;
    neg_fixy_chainedge_signaler_session_non_ctx::Edge::SignalerHandle* handle = nullptr;

    auto bad =
        fchain::mint_chainedge_signaler_session<neg_fixy_chainedge_signaler_session_non_ctx::Edge>(not_a_ctx, *handle);
    (void)bad;
    return 0;
}
