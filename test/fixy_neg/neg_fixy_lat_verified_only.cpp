// fixy_neg: gr::trust_verified is the only engagement — 19 axes unengaged.
//
...
struct TypeFixyLatVerifiedOnly {};

static_assert(!crucible::fixy::IsAccepted<TypeFixyLatVerifiedOnly, gr::trust_verified>,
              "A single Trust lattice tag must reject — 19 axes unengaged.");

static_assert(crucible::fixy::first_missing_axis_v<gr::trust_verified> == D::Type,
              "first_missing_axis_v on a single Trust tag must point at Type.");

using TypeAxisTag = crucible::fixy::diag::tag_for_axis_t<D::Type>;
static_assert(sizeof(TypeAxisTag) > 0 && false, "FixyNotEngaged_Type: a single gr::trust_verified engages only "
