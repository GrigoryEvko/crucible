...
[[gnu::cold]] int main() {
    ...
    auto* loop = crucible::make_loop(test.alloc, arena, body, crucible::compute_body_content_hash(body), feedback, 1,
                                     crucible::LoopTermKind::REPEAT, 3);
    ...
    auto* branch = new(arena.alloc_obj<crucible::BranchNode>(test.alloc)) crucible::BranchNode{};
    ...
    auto view = crucible::vis::extract_block_view(static_cast<const crucible::TraceNode*>(branch));
    ...
    for (const auto& edge : view.edges) {
        ...
        if (edge.kind == crucible::vis::DagEdgeKind::LOOP_FEEDBACK) saw_feedback = true;
    }
    assert(saw_guard0);
    assert(saw_guard1);
    assert(saw_feedback);

    auto metrics = crucible::observe::metrics_sample_at(crucible::observe::RuntimeMetrics{.ntk_alpha_drift = 0.125}, 4);
    crucible::vis::LiveTraceVisualizer live{static_cast<const crucible::TraceNode&>(*branch), metrics};
