...
static_assert(std::is_same_v<HotPromote<ContentHash>, proto::Send<tier::Hot<ContentHash>, proto::End>>);
static_assert(proto::is_well_formed_v<HotPromote<ContentHash>>);
static_assert(proto::DelegatesTo<HotPromoteDelegate<ContentHash>, HotPromote<ContentHash>>);
static_assert(proto::AcceptsFrom<HotPromoteAccept<ContentHash>, HotPromote<ContentHash>>);
...
void test_restore_error_surface() {
    ...
    auto empty_handle = mint_restore<ContentHash>(tier::Cold<ContentHash>{ContentHash{}}, hash);
    assert(!empty_handle.has_value());
    assert(empty_handle.error() == RestoreError::EmptyColdHandle);

    auto mismatch = mint_restore<ContentHash>(tier::Cold<ContentHash>{ContentHash{0xE005ULL}}, hash);
...
void test_hot_promote_delegate_protocol() {
    ...
    auto delegator_end = std::move(delegator).delegate(std::move(delegated_endpoint), transport_delegate_hot);
    ...
    auto acceptor = proto::mint_session_handle<HotPromoteAccept<ContentHash>>(CarrierWire{});
    auto [hot_endpoint, acceptor_end] = std::move(acceptor).accept(transport_accept_hot);
    (void)std::move(acceptor_end).close();

    auto hot_end = std::move(hot_endpoint).send(tier::Hot<ContentHash>{payload}, send_hot_payload);
