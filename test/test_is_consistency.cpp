...
using C_struct_strong = safety::Consistency<extract::Consistency_v::STRONG, payload_struct>;
using C_nested = safety::Consistency<extract::Consistency_v::CAUSAL_PREFIX, C_int_strong>;
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_consistency_smoke_test()); }
...
void test_value_type_extraction() {
    static_assert(std::is_same_v<extract::consistency_value_t<C_int_strong>, int>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_int_eventual>, int>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_double_strong>, double>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_float_eventual>, float>);
}

void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::consistency_value_t<C_int_strong const&>, int>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_int_strong&&>, int>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_double_strong const>, double>);
}

void test_level_extraction_every_value() {
    static_assert(extract::consistency_level_v<C_int_strong> == extract::Consistency_v::STRONG);
    static_assert(extract::consistency_level_v<C_int_bounded> == extract::Consistency_v::BOUNDED_STALENESS);
    static_assert(extract::consistency_level_v<C_int_causal> == extract::Consistency_v::CAUSAL_PREFIX);
    static_assert(extract::consistency_level_v<C_int_ryw> == extract::Consistency_v::READ_YOUR_WRITES);
    static_assert(extract::consistency_level_v<C_int_eventual> == extract::Consistency_v::EVENTUAL);
}

void test_level_extraction_cv_ref_strips() {
    static_assert(extract::consistency_level_v<C_int_strong const&> == extract::Consistency_v::STRONG);
    static_assert(extract::consistency_level_v<C_int_eventual&&> == extract::Consistency_v::EVENTUAL);
}

void test_distinct_levels_distinct_specs() {
    static_assert(std::is_same_v<extract::consistency_value_t<C_int_strong>,
                                 extract::consistency_value_t<C_int_eventual>>);  // both int
    static_assert(extract::consistency_level_v<C_int_strong> != extract::consistency_level_v<C_int_eventual>);

    static_assert(extract::consistency_level_v<C_int_strong> == extract::consistency_level_v<C_double_strong>);
    static_assert(
        !std::is_same_v<extract::consistency_value_t<C_int_strong>, extract::consistency_value_t<C_double_strong>>);
}
...
void test_dispatcher_integration_example() {
        ...
        requires extract::IsConsistency<T>
    {
        ...
        if constexpr (level == extract::Consistency_v::STRONG) {
            return 5;  // strictest
        } else if constexpr (level == extract::Consistency_v::BOUNDED_STALENESS) {
            return 4;
        } else if constexpr (level == extract::Consistency_v::CAUSAL_PREFIX) {
            return 3;
        } else if constexpr (level == extract::Consistency_v::READ_YOUR_WRITES) {
            return 2;
        } else {
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::consistency_value_t<C_struct_strong>, payload_struct>);
    static_assert(extract::consistency_level_v<C_struct_strong> == extract::Consistency_v::STRONG);
}

void test_positive_nested_consistency() {
    static_assert(extract::is_consistency_v<C_nested>);
    static_assert(std::is_same_v<extract::consistency_value_t<C_nested>, C_int_strong>);
    static_assert(extract::is_consistency_v<extract::consistency_value_t<C_nested>>);
    static_assert(std::is_same_v<extract::consistency_value_t<extract::consistency_value_t<C_nested>>, int>);
}
...
void test_dispatcher_function_parameter_use_case() {
    ...
    static_assert(ex::consistency_level_v<P0> == ex::Consistency_v::STRONG);
    ...
    static_assert(ex::consistency_level_v<P0_eventual> == ex::Consistency_v::EVENTUAL);
...
void test_local_alias_round_trip() {
    using LocalStrongInt = safety::Consistency<extract::Consistency_v::STRONG, int>;
    using LocalCausalDouble = safety::Consistency<extract::Consistency_v::CAUSAL_PREFIX, double>;

    static_assert(extract::is_consistency_v<LocalStrongInt>);
    static_assert(extract::is_consistency_v<LocalCausalDouble>);
    static_assert(extract::consistency_level_v<LocalStrongInt> == extract::Consistency_v::STRONG);
    static_assert(extract::consistency_level_v<LocalCausalDouble> == extract::Consistency_v::CAUSAL_PREFIX);
}

void test_zero_cost_layout_invariant() {
    // A function taking Consistency<L, T> by value has the same ABI as
    // one taking T.
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::consistency_level_v<StrongInt> == extract::Consistency_v::STRONG);
    static_assert(extract::consistency_level_v<EventualInt> == extract::Consistency_v::EVENTUAL);
    static_assert(extract::consistency_level_v<CausalDouble> == extract::Consistency_v::CAUSAL_PREFIX);
    static_assert(extract::consistency_level_v<BoundedFloat> == extract::Consistency_v::BOUNDED_STALENESS);
    static_assert(extract::consistency_level_v<RywChar> == extract::Consistency_v::READ_YOUR_WRITES);

    static_assert(std::is_same_v<extract::consistency_value_t<StrongInt>, int>);
    static_assert(std::is_same_v<extract::consistency_value_t<CausalDouble>, double>);
    static_assert(std::is_same_v<extract::consistency_value_t<RywChar>, char>);

    // The alias declares no new type: it is the same type, not merely
    // an equivalent one.
...
void test_graded_wrapper_conformance() {
    // One extractor is universal over graded wrappers, the other is
    // specific to Consistency.  They must agree on every Consistency.

    static_assert(extract::is_graded_wrapper_v<C_int_strong>);
    static_assert(extract::is_graded_wrapper_v<C_double_strong>);

    static_assert(std::is_same_v<extract::value_type_of_t<C_int_strong>, extract::consistency_value_t<C_int_strong>>);
    static_assert(
        std::is_same_v<extract::value_type_of_t<C_double_strong>, extract::consistency_value_t<C_double_strong>>);
...
void test_runtime_consistency() {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        EXPECT_TRUE(baseline_pos == extract::is_consistency_v<C_int_strong>);
        EXPECT_TRUE(baseline_neg == !extract::is_consistency_v<int>);
...
int main() {
    ...
    run_test("test_level_extraction_every_value", test_level_extraction_every_value);
    run_test("test_level_extraction_cv_ref_strips", test_level_extraction_cv_ref_strips);
    run_test("test_distinct_levels_distinct_specs", test_distinct_levels_distinct_specs);
    ...
    run_test("test_positive_nested_consistency", test_positive_nested_consistency);
