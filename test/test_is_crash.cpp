...
using C_struct_no_throw = safety::Crash<extract::CrashClass_v::NoThrow, payload_struct>;
using C_nested = safety::Crash<extract::CrashClass_v::ErrorReturn, C_int_no_throw>;
...
void test_runtime_smoke() { EXPECT_TRUE(extract::is_crash_smoke_test()); }
...
void test_negative_lookalike_struct() {
    struct LookalikeCrash {
        int value;
        extract::CrashClass_v crash_class;
    };
...
void test_value_type_extraction_cv_ref_strips() {
    static_assert(std::is_same_v<extract::crash_value_t<C_int_no_throw const&>, int>);
    static_assert(std::is_same_v<extract::crash_value_t<C_int_no_throw&&>, int>);
    static_assert(std::is_same_v<extract::crash_value_t<C_double_no_throw const>, double>);
}

void test_class_extraction_every_value() {
    static_assert(extract::crash_class_v<C_int_abort> == extract::CrashClass_v::Abort);
    static_assert(extract::crash_class_v<C_int_throw> == extract::CrashClass_v::Throw);
    static_assert(extract::crash_class_v<C_int_error_return> == extract::CrashClass_v::ErrorReturn);
    static_assert(extract::crash_class_v<C_int_no_throw> == extract::CrashClass_v::NoThrow);
}

void test_class_extraction_cv_ref_strips() {
    static_assert(extract::crash_class_v<C_int_no_throw const&> == extract::CrashClass_v::NoThrow);
    static_assert(extract::crash_class_v<C_int_abort&&> == extract::CrashClass_v::Abort);
}

void test_distinct_classes_distinct_specs() {
    static_assert(std::is_same_v<extract::crash_value_t<C_int_no_throw>, extract::crash_value_t<C_int_abort>>);
    static_assert(extract::crash_class_v<C_int_no_throw> != extract::crash_class_v<C_int_abort>);

    static_assert(extract::crash_class_v<C_int_no_throw> == extract::crash_class_v<C_double_no_throw>);
    static_assert(!std::is_same_v<extract::crash_value_t<C_int_no_throw>, extract::crash_value_t<C_double_no_throw>>);
}

void test_chain_ordinal_invariant() {
    // The detector does not care about the underlying values, but code
    // that compares strengths with <= does.  Abort is the weakest at 0,
    // NoThrow the strongest at 3.
    static_assert(static_cast<std::uint8_t>(extract::CrashClass_v::Abort) == 0);
    static_assert(static_cast<std::uint8_t>(extract::CrashClass_v::Throw) == 1);
    static_assert(static_cast<std::uint8_t>(extract::CrashClass_v::ErrorReturn) == 2);
    static_assert(static_cast<std::uint8_t>(extract::CrashClass_v::NoThrow) == 3);

    static_assert(extract::crash_class_v<C_int_abort> != extract::crash_class_v<C_int_throw>);
    static_assert(extract::crash_class_v<C_int_throw> != extract::crash_class_v<C_int_error_return>);
    static_assert(extract::crash_class_v<C_int_error_return> != extract::crash_class_v<C_int_no_throw>);
}

void test_dispatcher_integration_example() {
    // Models a phase gate that admits only the strongest class on a hot
    // path and routes the weaker classes elsewhere.
    auto select_lowering = []<typename T>() consteval
        requires extract::IsCrash<T>
    {
        constexpr auto c = extract::crash_class_v<T>;
        if constexpr (c == extract::CrashClass_v::Abort)
            return 0;
        else if constexpr (c == extract::CrashClass_v::Throw)
            return 1;
        else if constexpr (c == extract::CrashClass_v::ErrorReturn)
            return 2;
        else if constexpr (c == extract::CrashClass_v::NoThrow)
            return 3;
        else
...
void test_positive_non_fundamental_wrapped_type() {
    ...
    static_assert(std::is_same_v<extract::crash_value_t<C_struct_no_throw>, payload_struct>);
    static_assert(extract::crash_class_v<C_struct_no_throw> == extract::CrashClass_v::NoThrow);
}

void test_positive_nested_crash() {
    static_assert(extract::is_crash_v<C_nested>);
    static_assert(std::is_same_v<extract::crash_value_t<C_nested>, C_int_no_throw>);
    static_assert(extract::is_crash_v<extract::crash_value_t<C_nested>>);
    static_assert(std::is_same_v<extract::crash_value_t<extract::crash_value_t<C_nested>>, int>);

    static_assert(extract::crash_class_v<C_nested> == extract::CrashClass_v::ErrorReturn);
    static_assert(extract::crash_class_v<extract::crash_value_t<C_nested>> == extract::CrashClass_v::NoThrow);
}
...
void test_local_alias_round_trip() {
    using LocalNoThrowInt = safety::Crash<extract::CrashClass_v::NoThrow, int>;

    static_assert(extract::is_crash_v<LocalNoThrowInt>);
    static_assert(extract::crash_class_v<LocalNoThrowInt> == extract::CrashClass_v::NoThrow);
...
void test_public_convenience_aliases() {
    ...
    static_assert(extract::crash_class_v<AbortInt> == extract::CrashClass_v::Abort);
    static_assert(extract::crash_class_v<ThrowInt> == extract::CrashClass_v::Throw);
    static_assert(extract::crash_class_v<ErrorReturnInt> == extract::CrashClass_v::ErrorReturn);
    static_assert(extract::crash_class_v<NoThrowInt> == extract::CrashClass_v::NoThrow);
...
void test_graded_wrapper_conformance() {
    ...
    static_assert(std::is_same_v<extract::value_type_of_t<C_int_no_throw>, extract::crash_value_t<C_int_no_throw>>);
...
void test_outer_wrapper_identity_dominates_nested() {
    using ConsistencyOfC = safety::Consistency<extract::Consistency_v::STRONG, C_int_no_throw>;
    using COfConsistency = safety::Crash<extract::CrashClass_v::NoThrow, Cn_int_strong>;

    static_assert(extract::is_consistency_v<ConsistencyOfC>);
    static_assert(!extract::is_crash_v<ConsistencyOfC>);
    static_assert(extract::is_crash_v<extract::consistency_value_t<ConsistencyOfC>>);

    static_assert(extract::is_crash_v<COfConsistency>);
    static_assert(!extract::is_consistency_v<COfConsistency>);
    static_assert(extract::is_consistency_v<extract::crash_value_t<COfConsistency>>);
}
...
int main() {
    ...
    run_test("test_class_extraction_every_value", test_class_extraction_every_value);
    run_test("test_class_extraction_cv_ref_strips", test_class_extraction_cv_ref_strips);
    run_test("test_distinct_classes_distinct_specs", test_distinct_classes_distinct_specs);
    run_test("test_chain_ordinal_invariant", test_chain_ordinal_invariant);
    ...
    run_test("test_positive_nested_crash", test_positive_nested_crash);
