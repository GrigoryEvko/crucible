...
void test_typed_session_writer_reader_round_trip() {
    ...
    std::jthread writer_thread{[&writer, &writer_done](auto) mutable {
        auto psh = ses::mint_snapshot_writer_session<PermissionedSnap>(::crucible::effects::HotFgCtx{}, writer);
    ...
    std::jthread reader_thread{[&reader](auto) mutable {
        auto psh = ses::mint_snapshot_reader_session<PermissionedSnap>(::crucible::effects::HotFgCtx{}, reader);
    ...
    // The writer published 1 through kCount in order, so the value left in
    // the snapshot is kCount itself.
...
void test_typed_session_immediate_detach() {
    ...
    auto writer_psh = ses::mint_snapshot_writer_session<PermissionedSnap>(::crucible::effects::HotFgCtx{}, writer);
    auto reader_psh = ses::mint_snapshot_reader_session<PermissionedSnap>(::crucible::effects::HotFgCtx{}, *reader_opt);
...
static_assert(std::is_same_v<ses::WriterProto<int>, proto::Loop<proto::Send<int, proto::Continue>>>);
static_assert(std::is_same_v<ses::ReaderProto<int>, proto::Loop<proto::Recv<int, proto::Continue>>>);

using WriterHandle = PermissionedSnap::WriterHandle;
using ReaderHandle = PermissionedSnap::ReaderHandle;

using PSH_End_Write = proto::PermissionedSessionHandle<proto::End, proto::EmptyPermSet, WriterHandle*>;
using SH_End_Write = proto::SessionHandle<proto::End, WriterHandle*>;
static_assert(sizeof(PSH_End_Write) == sizeof(SH_End_Write));

using PSH_End_Read = proto::PermissionedSessionHandle<proto::End, proto::EmptyPermSet, ReaderHandle*>;
...
int main() {
    ...
    run_test("typed_session_writer_reader_round_trip", test_typed_session_writer_reader_round_trip);
