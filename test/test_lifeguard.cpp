...
[[nodiscard]] crucible::canopy::SwimPeer swim_peer(std::uint64_t id) noexcept {
...
int main() {
    ...
    assert(cc::lifeguard_error_name(cc::LifeguardError::PeerNotFound) == std::string_view{"PeerNotFound"});
    ...
    auto first = lifeguard.next_probe(1000);
    assert(first.has_value());
    assert(first->target == peer(1).uuid);
    assert(first->timeout_ns == 100000000ULL);
    assert(first->deadline_ns == 100001000ULL);
