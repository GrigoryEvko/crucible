...
static_assert(
    std::is_same_v<ses::OwnerProto<int>,
                   proto::Loop<proto::Select<proto::Send<int, proto::Continue>, proto::Recv<int, proto::Continue>>>>);
static_assert(std::is_same_v<ses::ThiefProto<int, Deque::thief_tag>,
                             proto::Loop<proto::Recv<proto::Borrowed<int, Deque::thief_tag>, proto::Continue>>>);
...
template <typename H>
concept CanUseOwnerHotPush = requires(H& h) { ses::owner_session_try_push<Deque>(h, 1); };

template <typename H>
concept CanUseThiefHotSteal = requires(H& h) { ses::thief_session_steal_borrowed<Deque>(h); };
...
void test_thief_session_receives_borrowed_work() {
    ...
    auto thief_psh = ses::mint_thief_session<Deque>(::crucible::effects::HotFgCtx{}, *thief_opt);

    auto [borrowed, next] = std::move(thief_psh).recv(ses::blocking_steal_borrowed);
...
void test_four_thief_sessions_steal_fixed_work() {
    ...
    auto owner = ses::mint_chaselev_owner<MultiDeque>(deque, std::move(owner_perm));
    ...
    for (int idx = 0; idx < kThieves; ++idx) {
        thieves[static_cast<std::size_t>(idx)] = std::jthread{[&] {
            ...
            auto psh = ses::mint_thief_session<MultiDeque>(::crucible::effects::HotFgCtx{}, *thief_opt);
            int local_sum = 0;
            for (int i = 0; i < kPerThief; ++i) {
                auto [borrowed, next] = std::move(psh).recv(ses::blocking_steal_borrowed);
...
int main() {
    ...
    run_test("mint_factories_and_thief_proof", test_mint_factories_and_thief_proof);
    run_test("owner_session_push_pop", test_owner_session_push_pop);
    run_test("thief_session_receives_borrowed_work", test_thief_session_receives_borrowed_work);
    run_test("fused_loop_hot_helpers_preserve_roles", test_fused_loop_hot_helpers_preserve_roles);
    run_test("four_thief_sessions_steal_fixed_work", test_four_thief_sessions_steal_fixed_work);
