...
static void test_name_coverage() {
    assert(topology::nic_telemetry_error_name(topology::NicTelemetryError::InvalidNicCog)
...
static void test_effective_bandwidth_and_history() {
    ...
    auto thermal = topology::declare_nic_thermal_sample(topology::NicThermalSample{.temperature_millicelsius = 41000});

    auto first = topology::mint_nic_telemetry_snapshot(nic(), 100000000000ull, counters, backlog, sysctl,
                                                       tcp_sample(80000000000ull, 1000, 1000), thermal, 1);
    assert(first.has_value());
    auto second = topology::mint_nic_telemetry_snapshot(nic(), 100000000000ull, counters, backlog, sysctl,
                                                        tcp_sample(50000000000ull, 1000, 1000), thermal, 2);
