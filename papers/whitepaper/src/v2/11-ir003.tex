...
The round-trip property is what allows the L2 KernelCache (Chapter~\ref{sec:cipher}) to load a cached binary back into a usable \IRstar{3} representation for the runtime observer's prediction path.
...
The decoder's primary role is not loading kernels back into Crucible (Mimic's runtime library loads compiled kernels by content hash through a binary loader that does not need the IR back); it is enabling the simulator and the runtime observer's prediction path to reason about cached or third-party-supplied binaries.
...
That is the simulator's job, calibrated against measurement, with drift attributed by runtime observation and corrected by Mimic's recalibration entry point.
