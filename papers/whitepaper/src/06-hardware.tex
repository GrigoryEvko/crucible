...
Longitude and runtime observation are designs; the hardware profiling interfaces they depend on (CUPTI, NVML, rocprofiler) are standard vendor APIs.
...
Longitude runs at startup and on topology change (Relay failure, new Relay, sustained drift detected by runtime observation).
...
\subsection{Runtime Observation: Continuous Monitoring}

runtime observation runs every iteration, comparing predictions against measurements.

\textbf{Digital twin.}
From the Merkle DAG, kernel cost predictions (roofline model with correction factors), and the memory plan, runtime observation predicts each iteration.
...
Periodically, runtime observation computes diagnostics of the Vigil's training dynamics from actual tensor data:
