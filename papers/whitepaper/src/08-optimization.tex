...
Operations grouped by \code{ScopeHash}; each layer analyzed independently via runtime observation diagnostics (Section~6.4).
...
\begin{tikzpicture}[
  ...
  %% Steps
  \node[stp, fill=blue!5] (diag) {\textbf{runtime observation}\\diagnose + predict};
...
\caption{Optimization activation protocol.
  ...
  runtime observation diagnoses, a BranchNode encodes the change,
...
\begin{enumerate}
  \item runtime observation diagnoses and predicts expected improvement.
