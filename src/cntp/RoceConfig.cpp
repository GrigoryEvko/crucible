...
[[nodiscard]] std::expected<std::uint64_t, RoceError> parse_u64(std::string_view text) noexcept {
...
[[nodiscard]] std::expected<std::uint64_t, RoceError> read_counter_file(char const* path) noexcept {
    ...
    return parse_u64(std::string_view{bytes.data(), static_cast<std::size_t>(nread)});
}

[[nodiscard]] std::array<char, 96> stat_path(NicInterfaceName iface, std::string_view leaf) noexcept {
    std::array<char, 96> out{};
    constexpr std::string_view prefix = "/sys/class/net/";
    constexpr std::string_view mid = "/statistics/";
    static_assert(prefix.size() + NicInterfaceName::max_bytes + mid.size() + std::string_view{"rx_pause_frames"}.size()
                      + 1u
                  <= out.size());
    static_assert(prefix.size() + NicInterfaceName::max_bytes + mid.size() + std::string_view{"tx_pause_frames"}.size()
                      + 1u
                  <= out.size());
    std::size_t pos = 0;
...
std::string_view roce_error_name(RoceError error) noexcept {
    switch (error) {
        case RoceError::InvalidPfcPriorityMask:
            return "InvalidPfcPriorityMask";
        case RoceError::InvalidDscp:
            return "InvalidDscp";
        case RoceError::InvalidDcqcnAlpha:
            return "InvalidDcqcnAlpha";
        case RoceError::InvalidDcqcnTargetPackets:
            return "InvalidDcqcnTargetPackets";
        case RoceError::InvalidCeThresholdBytes:
            return "InvalidCeThresholdBytes";
        case RoceError::CounterUnavailable:
            return "CounterUnavailable";
        case RoceError::CounterParseFailed:
            return "CounterParseFailed";
        case RoceError::PrivilegedApplyDeferred:
            return "PrivilegedApplyDeferred";
        case RoceError::VendorBackendUnavailable:
            return "VendorBackendUnavailable";
        case RoceError::DcqcnStatusUnavailable:
            return "DcqcnStatusUnavailable";
        default:
            return "<unknown RoceError>";
    }
}
...
std::expected<PfcPauseStats, RoceError> parse_pfc_pause_counters(std::string_view rx_text,
...
std::expected<PfcPauseStats, RoceError> query_pfc_pause_counters(NicInterfaceName iface) noexcept {
