...
std::string_view sharp_error_name(SharpError error) noexcept {
    switch (error) {
        case SharpError::None:
            return "None";
        case SharpError::ZeroSwitchCog:
            return "ZeroSwitchCog";
        case SharpError::NonSwitchCog:
            return "NonSwitchCog";
        case SharpError::MissingSwitchSharpCapability:
            return "MissingSwitchSharpCapability";
        case SharpError::EmptyParticipantSet:
        ...
        case SharpError::RecipeNotAssociative:
            return "RecipeNotAssociative";
        case SharpError::RecipeNotCommutative:
            return "RecipeNotCommutative";
        ...
        case SharpError::OutputShapeMismatch:
            return "OutputShapeMismatch";
        default:
            return "<unknown SharpError>";
    }
}

std::string_view sharp_fallback_name(SharpFallback fb) noexcept {
    switch (fb) {
        case SharpFallback::None:
            return "None";
        case SharpFallback::RingOrTree:
            return "RingOrTree";
        case SharpFallback::BitexactTree:
            return "BitexactTree";
        case SharpFallback::SoftwareCollectiveCatalog:
            return "SoftwareCollectiveCatalog";
        default:
            return "<unknown SharpFallback>";
    }
}
...
dispatch_sharp_allreduce(std::span<const float> input, std::span<float> output, NumericalRecipe const& recipe,
    ...
    if (!plan.value().runtime_loaded) {
        return fallback_dispatch(SharpError::RuntimeUnavailable, plan, input.size());
    }
    if (!plan.value().allow_backend_dispatch) {
        return fallback_dispatch(SharpError::DispatchDeferred, plan, input.size());
    }
    return fallback_dispatch(SharpError::VendorBackendUnavailable, plan, input.size());
}
