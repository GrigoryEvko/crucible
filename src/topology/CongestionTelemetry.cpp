...
[[nodiscard]] constexpr std::uint32_t sat_u32_mul(std::uint32_t a, std::uint32_t b) noexcept {
    const std::uint64_t product = static_cast<std::uint64_t>(a) * b;
    return product > std::numeric_limits<std::uint32_t>::max() ? std::numeric_limits<std::uint32_t>::max()
...
[[nodiscard]] constexpr CongestionMode mode_from_ca_state(std::uint8_t state) noexcept {
    switch (state) {
        case TCP_CA_Open:
            return CongestionMode::Open;
        case TCP_CA_Disorder:
            return CongestionMode::Disorder;
        case TCP_CA_CWR:
            return CongestionMode::Cwr;
        case TCP_CA_Recovery:
            return CongestionMode::Recovery;
        case TCP_CA_Loss:
            return CongestionMode::Loss;
        default:
            return CongestionMode::Unknown;
    }
}
...
[[nodiscard]] constexpr bool mode_worse(CongestionMode lhs, CongestionMode rhs) noexcept {
...
void record_sample(CongestionAggregate& aggregate, RttHistogram& rtt_hist, BandwidthHistogram& bw_hist,
    ...
    aggregate.ecn_mark_ppm = std::max(aggregate.ecn_mark_ppm, state.dctcp.ecn_mark_ppm);
    if (mode_worse(state.mode, aggregate.worst_mode)) {
        aggregate.worst_mode = state.mode;
    }
    const std::uint64_t rtt_value = std::min(state.rt_prop_us.value(), RttHistogram::max_trackable_value);
    const std::uint64_t bw_value = std::min(state.btl_bw_bps.value(), BandwidthHistogram::max_trackable_value);
...
std::expected<TcpInfoSnapshot, TelemetryError> harvest_socket(cntp::SocketFd fd) noexcept {
    ...
    const int cc_rc = ::getsockopt(fd.value(), IPPROTO_TCP, TCP_CC_INFO, &cc_info, &cc_len);
    if (cc_rc == 0) {
        if (algorithm == cntp::CcAlgorithm::Bbr1 || algorithm == cntp::CcAlgorithm::Bbr2
