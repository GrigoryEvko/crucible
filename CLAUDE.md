...
| `reinterpret_cast` | CI grep guard `scripts/check-no-reinterpret-cast.sh` (FIXY-U-105); per-line allowlist in `scripts/no-reinterpret-allowlist.txt` drains as FIXY-U-082 lands; stale allowlist entries red CI at exit 2 | Use `std::bit_cast<T>` for value reinterpretation; `std::start_lifetime_as<T>` for arena type-punning; `<simd>` first-class intrinsic interop for SIMD |
...
- **Inventory of every substrate mint** lives in `misc/mint-inventory.md`, regenerated by `scripts/gen-mint-inventory.sh` (FIXY-U-106). Surfaces 5 §XXI compliance flags (nodiscard / constexpr / noexcept / requires / Ctx-bound) plus the fixy re-export site + HS14 neg-compile-fixture count per mint. `--check` mode runs in CI as a drift gate; `[✗ NO-FIXY]` and `HS14: N ⚠` markers flag audit gaps without failing the build.
