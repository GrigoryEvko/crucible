...
# into the fixtures that ask for it, which keeps the other fixtures byte
...
def render(type_, header, expr, reason):
    brand_decl = BRAND_DECL if "forge::brand" in expr else ""
...
# A class head may carry any number of attribute-shaped prefixes before
# its name: an attribute specifier, an alignas, or a macro that expands
# to one.  The third is the case this walk used to read as the name
# itself, which reported the macro as an unrostered witness and left the
# real class unwalked.  A macro is recognised by its spelling — all
# upper case with an underscore — which no type in this tree has,
# because §XVII spells every type in CamelCase.  A type that did would
# not be misread as its own attribute: the walk would find no name after
# it and skip the head, which is a miss the roster's own entry reports.
CLASS_HEAD = re.compile(
    r"^\s*(?:template\s*<.*>\s*)?(?:class|struct)\s+"
    r"(?:(?:\[\[[^]]*\]\]|alignas\([^)]*\)|[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+)\s+)*"
...
#pragma once
// The head carries an attribute macro, so this arm also proves the walk
// reads the class's own name rather than the macro standing in front of
// it.  fixy/Qtt.h is the real case: its head gained an ABI tag that is
// on only in the tracked build, and the walk reported the macro.
#define PLANTED_ABI [[gnu::abi_tag("planted")]]
namespace planted {
class PLANTED_ABI Leaky;
