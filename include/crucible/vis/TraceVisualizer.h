...
[[nodiscard]] inline std::string hex64_short(uint64_t value) {
    char buf[17]{};
    std::snprintf(buf, sizeof(buf), "%08llx", value & 0xFFFFFFFFULL);
...
[[nodiscard]] inline const char* guard_kind_name(Guard::Kind kind) {
    switch (kind) {
        case Guard::Kind::SHAPE_DIM:
            return "shape";
        case Guard::Kind::SCALAR_VALUE:
            return "scalar";
        case Guard::Kind::DTYPE:
            return "dtype";
        case Guard::Kind::DEVICE:
            return "device";
        case Guard::Kind::OP_SEQUENCE:
            return "opseq";
        default:
            return "unknown";
    }
...
struct MerkleDagViewState {
    ...
    [[nodiscard]] Block make_branch_block(const BranchNode& branch) {
        ...
        block.label =
            std::string{"branch "} + guard_kind_name(branch.guard.kind) + " arms=" + std::to_string(branch.num_arms);
        return block;
    }
    ...
    [[nodiscard]] WalkResult walk_chain(const TraceNode* node, const TraceNode* stop,
        ...
        while (node != nullptr && node != stop) {
            ...
            if (auto it = node_to_block.find(node); it != node_to_block.end()) {
                ...
                return {.tails = {IncomingEdge{it->second, DagEdgeKind::SEQUENTIAL, {}}}, .first_block = first};
            }
            ...
            switch (node->kind) {
                ...
                case TraceNodeKind::BRANCH: {
                    ...
                    for (uint32_t i = 0; i < branch->num_arms; ++i) {
                        std::vector<IncomingEdge> arm_in{IncomingEdge{
                            block_id, DagEdgeKind::BRANCH_ARM, "guard=" + std::to_string(branch->arms[i].value)}};
                        WalkResult arm = walk_chain(branch->arms[i].target, branch->next, std::move(arm_in));
                        branch_tails.insert(branch_tails.end(), std::make_move_iterator(arm.tails.begin()),
                ...
                case TraceNodeKind::LOOP: {
                    ...
                    WalkResult body =
                        walk_chain(loop->body, nullptr, {IncomingEdge{block_id, DagEdgeKind::LOOP_BODY, "body"}});
...
[[nodiscard]] inline MerkleDagBlockView extract_block_view(const TraceNode* root) {
    detail::MerkleDagViewState state;
    ...
    if (!state.view.detection.blocks.empty()) {
        state.view.detection.fwd_end = static_cast<uint32_t>(state.view.detection.blocks.size() - 1);
        state.view.detection.bwd_end = state.view.detection.fwd_end;
        state.view.detection.optim_start = static_cast<uint32_t>(state.view.detection.blocks.size());
    }
    return std::move(state.view);
}

[[nodiscard]] inline MerkleDagBlockView extract_block_view(const TraceNode& root) { return extract_block_view(&root); }
...
[[nodiscard]] inline DetectionResult extract_blocks(const TraceNode& root) { return extract_blocks(&root); }
...
[[nodiscard]] inline NodeColors colors_for_family(OpFamily f) {
    switch (f) {
        case OpFamily::GEMM:
            return {palette::GEMM_FILL, palette::GEMM_BORDER};
        case OpFamily::CONV:
            return {palette::CONV_FILL, palette::CONV_BORDER};
        case OpFamily::NORM:
            return {palette::NORM_FILL, palette::NORM_BORDER};
        case OpFamily::ATTN:
            return {palette::ATTN_FILL, palette::ATTN_BORDER};
        case OpFamily::ACT:
            return {palette::ACT_FILL, palette::ACT_BORDER};
        case OpFamily::ELEM:
            return {palette::ELEM_FILL, palette::ELEM_BORDER};
        case OpFamily::MOVE:
            return {palette::MOVE_FILL, palette::MOVE_BORDER};
        case OpFamily::REDUCE:
            return {palette::REDUCE_FILL, palette::REDUCE_BORDER};
        case OpFamily::OPTIM:
            return {palette::OPTIM_FILL, palette::OPTIM_BORDER};
        case OpFamily::LOSS:
            return {palette::LOSS_FILL, palette::LOSS_BORDER};
        case OpFamily::EMBED:
            return {palette::NORM_FILL, palette::NORM_BORDER};
        case OpFamily::POOL:
            return {palette::CONV_FILL, palette::CONV_BORDER};
...
[[nodiscard]] inline Color color_for_dag_edge(DagEdgeKind kind) {
    switch (kind) {
        case DagEdgeKind::SEQUENTIAL:
            return palette::EDGE_DATA_FLOW;
        case DagEdgeKind::BRANCH_ARM:
            return Color::hex(0x7C3AED);
        case DagEdgeKind::LOOP_BODY:
            return Color::hex(0x059669);
        case DagEdgeKind::LOOP_FEEDBACK:
            return palette::EDGE_SKIP;
        default:
            return palette::EDGE_DATA_FLOW;
    }
}
...
[[nodiscard]] inline std::vector<BlockEdge> build_block_edges(const std::vector<Block>& blocks,
    ...
    for (const auto& op : ops) {
        for (uint32_t j = 0; j < op.n_out && j < 4; j++) {
            if (op.data_ptr_out[j]) ptr_producer[op.data_ptr_out[j]] = op.idx;
        }
    }
...
[[nodiscard]] inline UShapeSplit detect_u_shape(const std::vector<Block>& blocks,
    ...
    if (detection.architecture == Architecture::UNET) {
        ...
        auto draw_u_skips = [&](const std::vector<uint32_t>& phase_idx) {
            ...
            for (uint32_t bi : phase_idx) {
                ...
                (pos[bi].col == 0 ? enc_by_res : dec_by_res)[blocks[bi].spatial_h].push_back(bi);
            }
        ...
        for (const auto& e : block_edges) {
            ...
            svg.orthogonal_edge(pos[e.src_block].x + pos[e.src_block].w + 2,
                                pos[e.src_block].y + pos[e.src_block].h / 2, pos[e.dst_block].x - 2,
                                pos[e.dst_block].y + pos[e.dst_block].h / 2, Color::hex(0xA78BFA), 0.4f, true);
            count++;
        }
    } else {
        ...
        for (const auto& e : block_edges) {
            ...
            svg.orthogonal_edge(pos[e.src_block].x + pos[e.src_block].w + 2,
                                pos[e.src_block].y + pos[e.src_block].h / 2, pos[e.dst_block].x - 2,
                                pos[e.dst_block].y + pos[e.dst_block].h / 2, palette::EDGE_SKIP, 0.5f, true);
        }
    }
    svg.end_group();
}

inline void render_seq_connectors(SvgRenderer& svg, const std::vector<Block>& blocks, const std::vector<GridPos>& pos) {
    svg.begin_group("seq-edges");
    for (uint32_t col = 0; col < 2; col++) {
        std::vector<uint32_t> col_blocks;
        for (uint32_t i = 0; i < blocks.size(); i++)
            if (pos[i].col == col) col_blocks.push_back(i);
        std::ranges::sort(col_blocks, [&](uint32_t a, uint32_t b) { return pos[a].row < pos[b].row; });
        for (uint32_t j = 0; j + 1 < col_blocks.size(); j++) {
            ...
            if (pos[b].y > pos[a].y + pos[a].h)
                svg.line(x, pos[a].y + pos[a].h, x, pos[b].y, Color::hex(0xD1D5DB), 0.4f);
        }
    }
    svg.end_group();
}

inline void render_resolution_bands(SvgRenderer& svg, float svg_w, const std::vector<Block>& blocks,
    ...
    for (uint32_t i = 0; i < bands.size(); i++) {
        ...
        svg.rect(2, band.y_min, svg_w - 4, band.y_max - band.y_min, band_colors[i % 2], Color::hex(0xE5E7EB), 0, 0,
                 false);
        std::string label = std::to_string(band.res) + "x" + std::to_string(band.res);
        svg.text(4, (band.y_min + band.y_max) / 2 + 3, label, 6.0f, Color::hex(0xC0C0C0), "start");
    }
    svg.end_group();
}

inline void render_block_nodes(SvgRenderer& svg, const std::vector<Block>& blocks, const std::vector<GridPos>& pos) {
    ...
    for (uint32_t i = 0; i < blocks.size(); i++) {
        ...
        svg.text(p.x + p.w / 2, p.y + p.h / 2 + 1, b.label, 7.0f, Color::hex(0x1F2937), "middle", true);
        std::string ops_str = std::to_string(b.num_ops);
        svg.text(p.x + p.w - 3, p.y + p.h - 3, ops_str, 5.0f, Color::hex(0xA0A0A0), "end");
        svg.end_group();
    }
    svg.end_group();
}

inline void render_phase_borders(SvgRenderer& svg, const std::vector<Block>& blocks, const std::vector<GridPos>& pos) {
    ...
    auto draw_cluster = [&](Phase phase, Color border_color) {
        ...
        if (count == 0) return;
        svg.rect_dashed(cx_min, cy_min, cx_max - cx_min, cy_max - cy_min, border_color, 8, 0.6f);
    };
...
[[nodiscard]] inline std::string render_live_block_svg(const MerkleDagBlockView& view,
    ...
    for (const auto& edge : view.edges) {
        ...
        if (edge.kind == DagEdgeKind::LOOP_FEEDBACK) {
            ...
            svg.bezier_arrow(sx, sy, sx + 34.0f, sy - lift, dx - 34.0f, dy - lift, dx, dy, color, 0.7f, true);
            if (!edge.label.empty()) {
                svg.text_mono((sx + dx) * 0.5f, std::min(sy, dy) - lift, edge.label, 7.0f, color);
            }
            continue;
        }

        svg.bezier_arrow(x1, y1, x1, (y1 + y2) * 0.5f, x2, (y1 + y2) * 0.5f, x2, y2, color,
                         edge.kind == DagEdgeKind::SEQUENTIAL ? 0.55f : 0.75f, edge.kind == DagEdgeKind::BRANCH_ARM);
        if (!edge.label.empty()) {
            svg.text_mono((x1 + x2) * 0.5f + 4.0f, (y1 + y2) * 0.5f - 4.0f, edge.label, 7.0f, color);
        }
    }
    svg.end_group();
    ...
    for (uint32_t i = 0; i < blocks.size(); ++i) {
        ...
        std::string info = std::string{block_kind_name(block.kind)} + " | " + std::to_string(block.num_ops) + " ops";
        if (!block.scope_path.empty()) info += " | " + block.scope_path;
        svg.begin_block_group(info);
        svg.rect(x, y, w, h, fill, border, 4, 0.8f, true);
        svg.text(x + w * 0.5f, y + h * 0.5f + 2.0f, block.label, 8.0f, Color::hex(0x1F2937), "middle", true);
        if (block.num_ops != 0) {
            svg.text(x + w - 4.0f, y + h - 4.0f, std::to_string(block.num_ops), 5.5f, Color::hex(0x6B7280), "end");
        }
        svg.end_group();
    }
    svg.end_group();
