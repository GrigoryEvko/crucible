...
public:
    ...
    explicit LiveTraceVisualizer(const TraceNode& root) noexcept : root_{&root} {}

    explicit LiveTraceVisualizer(const RegionNode& root) noexcept : root_{static_cast<const TraceNode*>(&root)} {}

    LiveTraceVisualizer(const TraceNode& root, observe::RuntimeMetricsSample metrics) noexcept
        : root_{&root}, metrics_{metrics}, has_metrics_{true} {}

    LiveTraceVisualizer(const RegionNode& root, observe::RuntimeMetricsSample metrics) noexcept
        : root_{static_cast<const TraceNode*>(&root)}, metrics_{metrics}, has_metrics_{true} {}

    void update_root(const TraceNode& root) noexcept { root_ = &root; }

    void update_root(const RegionNode& root) noexcept { root_ = static_cast<const TraceNode*>(&root); }
    ...
    [[nodiscard]] MerkleDagBlockView extract() const { return extract_block_view(root_); }

    [[nodiscard]] std::string render_svg(std::string_view title = "Crucible Live Trace") const {
        auto view = extract();
        return render_live_block_svg(view, title_with_metrics(title));
    }

    [[nodiscard]] std::string sample_tick(const TraceNode& root, observe::RuntimeMetricsSample metrics,
    ...
    [[nodiscard]] bool stream_svg(Sink&& sink, std::string_view title = "Crucible Live Trace") const {
