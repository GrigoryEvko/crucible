...
struct is_sharded_calendar_grid_session_surface<::crucible::concurrent::PermissionedShardedCalendarGrid<
    T, NumShards, NumBuckets, BucketCap, KeyExtractor, QuantumNs, UserTag>> : std::true_type {};

template <typename Grid>
concept ShardedCalendarGridSessionSurface = is_sharded_calendar_grid_session_surface<std::remove_cvref_t<Grid>>::value;

template <ShardedCalendarGridSessionSurface Grid, std::size_t S>
[[nodiscard]] constexpr auto mint_sharded_calendar_grid_producer(
    Grid& grid, ::crucible::safety::Permission<typename Grid::template shard_producer_tag<S>>&& perm) noexcept {
    return grid.template producer<S>(std::move(perm));
}

template <ShardedCalendarGridSessionSurface Grid, std::size_t S>
[[nodiscard]] constexpr auto mint_sharded_calendar_grid_consumer(
    Grid& grid, ::crucible::safety::Permission<typename Grid::template shard_consumer_tag<S>>&& perm) noexcept {
...
using ProducerSessionHandle = decltype(mint_producer_session<Grid, S>(
    std::declval<Ctx const&>(), std::declval<typename Grid::template ProducerHandle<S>&>()));

template <ShardedCalendarGridSessionSurface Grid, std::size_t S,
          ::crucible::effects::IsExecCtx Ctx = ::crucible::effects::HotFgCtx>
using ConsumerSessionHandle = decltype(mint_consumer_session<Grid, S>(
    std::declval<Ctx const&>(), std::declval<typename Grid::template ConsumerHandle<S>&>()));
...
producer_session_try_push(ProducerSessionHandle<Grid, S>& session, typename Grid::value_type const& value) noexcept {
...
using Grid = ::crucible::concurrent::PermissionedShardedCalendarGrid<Job, 2, 8, 16, Key, 1'000'000ULL, Tag>;
...
static_assert(std::is_same_v<typename ConsumerSession1::protocol, Recv<Job, Continue>>);
static_assert(std::is_same_v<typename ProducerSession0::perm_set, EmptyPermSet>);
static_assert(std::is_same_v<typename ConsumerSession1::perm_set, EmptyPermSet>);

static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, Producer0*>)
              == sizeof(SessionHandle<End, Producer0*>));
static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, Consumer1*>)
...
static_assert(
    sizeof(ConsumerSession1)
    == sizeof(SessionHandle<typename ConsumerSession1::protocol, Consumer1*, typename ConsumerSession1::loop_ctx>));
