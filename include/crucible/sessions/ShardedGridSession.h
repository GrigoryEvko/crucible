...
template <::crucible::concurrent::SpscValue T, std::size_t M, std::size_t N, std::size_t Capacity, typename UserTag,
          typename Routing>
struct is_sharded_grid_session_surface<
    ::crucible::concurrent::PermissionedShardedGrid<T, M, N, Capacity, UserTag, Routing>> : std::true_type {};

template <typename Grid>
concept ShardedGridSessionSurface = is_sharded_grid_session_surface<std::remove_cvref_t<Grid>>::value;

template <ShardedGridSessionSurface Grid, std::size_t I>
[[nodiscard]] constexpr auto mint_sharded_grid_producer(
    Grid& grid, ::crucible::safety::Permission<::crucible::concurrent::grid_tag::Producer<typename Grid::user_tag, I>>&&
                    perm) noexcept {
...
[[nodiscard]] constexpr auto mint_sharded_grid_consumer(
    Grid& grid, ::crucible::safety::Permission<::crucible::concurrent::grid_tag::Consumer<typename Grid::user_tag, J>>&&
                    perm) noexcept {
...
using ProducerSessionHandle = decltype(mint_producer_session<Grid, I>(
    std::declval<Ctx const&>(), std::declval<typename Grid::template ProducerHandle<I>&>()));

template <ShardedGridSessionSurface Grid, std::size_t J,
          ::crucible::effects::IsExecCtx Ctx = ::crucible::effects::HotFgCtx>
using ConsumerSessionHandle = decltype(mint_consumer_session<Grid, J>(
    std::declval<Ctx const&>(), std::declval<typename Grid::template ConsumerHandle<J>&>()));
...
template <ShardedGridSessionSurface Grid, std::size_t I>
[[nodiscard, gnu::always_inline]] inline bool producer_session_try_push(ProducerSessionHandle<Grid, I>& session,
...
static_assert(std::is_same_v<typename ProducerSession0::protocol, Send<int, Continue>>);
static_assert(std::is_same_v<typename ConsumerSession1::protocol, Recv<int, Continue>>);
