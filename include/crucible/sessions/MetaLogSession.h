...
concept MetaLogSessionSurface =
    requires(Log& log, typename Log::ProducerHandle& producer, typename Log::ConsumerHandle& consumer,
             ...
             ::crucible::safety::Permission<typename Log::consumer_tag> cons_perm, const MetaLogRecord* records,
...
mint_metalog_producer(Log& log, ::crucible::safety::Permission<typename Log::producer_tag>&& perm) noexcept {
    return log.producer(std::move(perm));
}

template <MetaLogSessionSurface Log>
[[nodiscard]] constexpr auto
mint_metalog_consumer(Log& log, ::crucible::safety::Permission<typename Log::consumer_tag>&& perm) noexcept {
...
template <MetaLogSessionSurface Log, ::crucible::effects::IsExecCtx Ctx = ::crucible::effects::HotFgCtx>
...
static_assert(std::is_same_v<ProducerProto, Loop<Send<MetaLogRecord, Continue>>>);
static_assert(std::is_same_v<ConsumerProto, Loop<Recv<MetaLogRecord, Continue>>>);
static_assert(std::is_same_v<typename ProducerSession::protocol, Send<MetaLogRecord, Continue>>);
static_assert(std::is_same_v<typename ConsumerSession::protocol, Recv<MetaLogRecord, Continue>>);
...
static_assert(sizeof(ConsumerHandle) == sizeof(::crucible::MetaLog*),
              ...
              "the Permission token should collapse through EBO.");
static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, ProducerHandle*>)
...
static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, ConsumerHandle*>)
...
static_assert(sizeof(ProducerSession)
                  == sizeof(SessionHandle<typename ProducerSession::protocol, ProducerHandle*,
...
static_assert(sizeof(ConsumerSession)
                  == sizeof(SessionHandle<typename ConsumerSession::protocol, ConsumerHandle*,
