...
concept ChainEdgeSessionSurface =
    requires(Edge& edge, typename Edge::SignalerHandle& signaler, typename Edge::WaiterHandle& waiter,
             ::crucible::safety::Permission<typename Edge::signaler_tag> sig_perm,
             ::crucible::safety::Permission<typename Edge::waiter_tag> wait_perm, const Signal& signal) {
        ...
        { edge.signaler(std::move(sig_perm)) } -> std::same_as<typename Edge::SignalerHandle>;
        { edge.waiter(std::move(wait_perm)) } -> std::same_as<typename Edge::WaiterHandle>;
...
mint_chainedge_signaler(Edge& edge, ::crucible::safety::Permission<typename Edge::signaler_tag>&& perm) noexcept {
    return edge.signaler(std::move(perm));
}

template <ChainEdgeSessionSurface Edge>
[[nodiscard]] constexpr auto
mint_chainedge_waiter(Edge& edge, ::crucible::safety::Permission<typename Edge::waiter_tag>&& perm) noexcept {
...
template <ChainEdgeSessionSurface Edge, ::crucible::effects::IsExecCtx Ctx = ::crucible::effects::HotFgCtx>
using SignalerSessionHandle = decltype(mint_chainedge_signaler_session<Edge>(
    std::declval<Ctx const&>(), std::declval<typename Edge::SignalerHandle&>()));

template <ChainEdgeSessionSurface Edge, ::crucible::effects::IsExecCtx Ctx = ::crucible::effects::HotFgCtx>
using WaiterSessionHandle = decltype(mint_chainedge_waiter_session<Edge>(std::declval<Ctx const&>(),
...
static_assert(std::is_same_v<typename SignalerSession::protocol, SignalerProto>);
static_assert(std::is_same_v<typename WaiterSession::protocol, WaiterProto>);
static_assert(std::is_same_v<typename SignalerSession::perm_set, EmptyPermSet>);
static_assert(std::is_same_v<typename WaiterSession::perm_set, EmptyPermSet>);

static_assert(sizeof(SignalerHandle) == sizeof(Edge*));
static_assert(sizeof(WaiterHandle) == sizeof(Edge*));
static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, SignalerHandle*>)
              == sizeof(SessionHandle<End, SignalerHandle*>));
static_assert(sizeof(PermissionedSessionHandle<End, EmptyPermSet, WaiterHandle*>)
