...
template <int Node>
struct Pinned {
    static_assert(Node >= 0, "ctx_numa::Pinned<N> requires N >= 0; NUMA node ids are "
...
struct ByteBudget {
    static_assert(Bytes > 0, "ctx_workload::ByteBudget<N> requires N > 0; use "
...
struct ItemBudget {
    static_assert(Items > 0, "ctx_workload::ItemBudget<N> requires N > 0; use "
                             "ctx_workload::Unspecified when no budget is declared.");
};
...
struct ChannelBudget {
    static_assert(Bytes > 0, "ctx_workload::ChannelBudget requires Bytes > 0; use "
                             "ctx_workload::Unspecified when no budget is declared.");
    static_assert(Producers > 0, "ctx_workload::ChannelBudget requires Producers > 0; use "
                                 "ctx_workload::ConsumerOnlyChannel for consumer-only "
                                 "fan shapes (audit log readers, sink-side observers).");
    static_assert(Consumers > 0, "ctx_workload::ChannelBudget requires Consumers > 0; use "
...
struct ProducerOnlyChannel {
    static_assert(Bytes > 0, "ctx_workload::ProducerOnlyChannel requires Bytes > 0; "
...
struct ConsumerOnlyChannel {
    static_assert(Bytes > 0, "ctx_workload::ConsumerOnlyChannel requires Bytes > 0; "
                             "use ctx_workload::Unspecified when no budget is declared.");
...
public:
    ...
    static_assert(heat_resid_coherent_v<Heat, Resid>, "ExecCtx Heat × Resid incoherent: Heat=Hot requires Resid ∈ "
    ...
    static_assert(heat_alloc_coherent_v<Heat, Alloc>, "ExecCtx Heat × Alloc incoherent: Heat=Hot forbids "
    ...
    static_assert(heat_progress_coherent_v<Heat, Progress>, "ExecCtx Heat × Progress incoherent: Heat=Hot forbids "
...
static_assert(!CtxOwnsAnyOf<BgDrainCtx>, "fixy-A5-039: empty-pack disjunction ≡ false — vacuously no atom "
...
static_assert(CtxOwnsAllOf<BgDrainCtx>, "fixy-A5-039: empty-pack conjunction ≡ true — vacuous AND over "
                                        "no atoms; matches std::conjunction_v on empty pack");
static_assert(!CtxOwnsAllOf<HotFgCtx, Effect::Bg>, "fixy-A5-039: HotFgCtx::row = Row<> — conjunctive lift must "
...
static_assert(!std::is_same_v<HeatLow, HeatHigh>, "ExecCtx variants differing in hot-path tier must be distinct types");
