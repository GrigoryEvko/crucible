...
// carry the guarantee.
static_assert(row_size_v<AllRow> == effect_count, "AllRow must enumerate every atom in Effect; update both together");

static_assert(row_contains_v<AllRow, Effect::Alloc>,
              "AllRow missing Alloc — universe row out of sync with Effect enum");
static_assert(row_contains_v<AllRow, Effect::IO>, "AllRow missing IO — universe row out of sync with Effect enum");
static_assert(row_contains_v<AllRow, Effect::Block>,
              "AllRow missing Block — universe row out of sync with Effect enum");
static_assert(row_contains_v<AllRow, Effect::Bg>, "AllRow missing Bg — universe row out of sync with Effect enum");
static_assert(row_contains_v<AllRow, Effect::Init>, "AllRow missing Init — universe row out of sync with Effect enum");
static_assert(row_contains_v<AllRow, Effect::Test>, "AllRow missing Test — universe row out of sync with Effect enum");

static_assert(is_subrow_v<PureRow, TotRow>);
static_assert(is_subrow_v<TotRow, PureRow>);
static_assert(is_subrow_v<PureRow, GhostRow>);
static_assert(is_subrow_v<GhostRow, PureRow>);

static_assert(is_subrow_v<PureRow, DivRow>);
static_assert(is_subrow_v<DivRow, STRow>);
static_assert(is_subrow_v<STRow, AllRow>);
static_assert(is_subrow_v<DivRow, AllRow>);
static_assert(is_subrow_v<PureRow, AllRow>);

static_assert(!is_subrow_v<DivRow, PureRow>);
static_assert(!is_subrow_v<STRow, DivRow>);
static_assert(!is_subrow_v<AllRow, STRow>);
...
// Value-carrying forms.  A row and a termination class are the two
// discrete components, and these three aliases name every cell worth a
// name: ST and All are row names, so their value-carrying forms are Tot
// or Div with E_os substituted, and a Ghost form would expand exactly
// as Pure does.
//
...
using Pure = ::crucible::safety::Progress<
    ...
    ::crucible::safety::DetSafe<::crucible::safety::DetSafeTier_v::Pure, Computation<PureRow, T>>>;

template <typename E_os, typename T>
using Tot = ::crucible::safety::Progress<
    ::crucible::safety::ProgressClass_v::Terminating,
    ::crucible::safety::DetSafe<::crucible::safety::DetSafeTier_v::Pure, Computation<E_os, T>>>;
...
static_assert(
    std::is_same_v<PureInt::value_type,
                   ::crucible::safety::DetSafe<::crucible::safety::DetSafeTier_v::Pure, Computation<PureRow, int>>>);
...
static_assert(std::is_same_v<TotIoInt::value_type::value_type::row_type, Row<Effect::IO>>);
static_assert(TotIoInt::cls == ::crucible::safety::ProgressClass_v::Terminating);

static_assert(std::is_same_v<DivIoInt::value_type::value_type::row_type, Row<Effect::IO>>);
...
static_assert(sizeof(Div<AllRow, void*>) == sizeof(Computation<AllRow, void*>));
