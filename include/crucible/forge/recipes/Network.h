...
[[nodiscard]] std::string_view network_collective_algorithm_name(NetworkCollectiveAlgorithm algorithm) noexcept;

[[nodiscard]] std::string_view network_equivalence_class_name(NetworkEquivalenceClass eq) noexcept;

[[nodiscard]] constexpr bool is_power_of_two_participant_count(NetworkParticipantCount participants) noexcept {
...
[[nodiscard]] constexpr DeclaredNetworkRecipeConstraints query_constraints(NumericalRecipe const& recipe,
    ...
    constraints.requires_sorted_participants = recipe.determinism == ReductionDeterminism::BITEXACT_STRICT;
    constraints.sharp_eligible =
        laws.associative && laws.commutative && recipe.determinism != ReductionDeterminism::BITEXACT_STRICT;
...
algorithm_eligible(DeclaredNetworkRecipeConstraints constraints, NetworkCollectiveAlgorithm algorithm,
                   NetworkParticipantCount participants) noexcept {
    auto const& raw = constraints.value();
    switch (algorithm) {
        case NetworkCollectiveAlgorithm::Ring:
            return raw.ring_eligible ? std::expected<void, NetworkRecipeError>{}
                                     : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        case NetworkCollectiveAlgorithm::DeterministicTree:
            return raw.deterministic_tree_eligible ? std::expected<void, NetworkRecipeError>{}
                                                   : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        case NetworkCollectiveAlgorithm::Sharp:
            return raw.sharp_eligible ? std::expected<void, NetworkRecipeError>{}
                                      : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        case NetworkCollectiveAlgorithm::RecursiveHalvingDoubling:
            ...
            if (!is_power_of_two_participant_count(participants)) {
                return std::unexpected(NetworkRecipeError::NonPowerOfTwoParticipantSet);
            }
            return {};
        case NetworkCollectiveAlgorithm::TopKCompressed:
            return raw.lossy_compression_allowed ? std::expected<void, NetworkRecipeError>{}
                                                 : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        case NetworkCollectiveAlgorithm::Quantized:
            return raw.quantization_allowed ? std::expected<void, NetworkRecipeError>{}
                                            : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        case NetworkCollectiveAlgorithm::FecProtected:
            return raw.fec_allowed ? std::expected<void, NetworkRecipeError>{}
                                   : std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
        default:
            return std::unexpected(NetworkRecipeError::AlgorithmForbiddenByRecipe);
    }
...
concept DeclaresNetworkRecipe = requires {
...
concept DeclaresNetworkAlgorithm = requires {
    { Algorithm::kind } -> std::convertible_to<NetworkCollectiveAlgorithm>;
};

[[nodiscard]] constexpr bool compile_time_algorithm_eligible(ReductionDeterminism determinism, bool associative,
                                                             bool commutative, bool participant_count_power_of_two,
    ...
    switch (algorithm) {
        ...
        case NetworkCollectiveAlgorithm::FecProtected:
            return true;
        case NetworkCollectiveAlgorithm::Sharp:
            return associative && commutative && determinism != ReductionDeterminism::BITEXACT_STRICT;
        case NetworkCollectiveAlgorithm::RecursiveHalvingDoubling:
            return participant_count_power_of_two;
        case NetworkCollectiveAlgorithm::TopKCompressed:
        case NetworkCollectiveAlgorithm::Quantized:
            return determinism == ReductionDeterminism::UNORDERED || determinism == ReductionDeterminism::ORDERED;
        default:
            return false;
    }
}
...
concept NetworkRecipeEligible =
    DeclaresNetworkRecipe<Recipe> && DeclaresNetworkAlgorithm<Algorithm>
    && compile_time_algorithm_eligible(Recipe::determinism, Recipe::associative, Recipe::commutative,
                                       Recipe::participant_count_power_of_two, Algorithm::kind);

struct RingAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::Ring;
};
struct DeterministicTreeAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::DeterministicTree;
};
struct SharpAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::Sharp;
};
struct RecursiveHalvingDoublingAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::RecursiveHalvingDoubling;
};
struct TopKCompressedAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::TopKCompressed;
};
struct QuantizedAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::Quantized;
};
struct FecProtectedAlgorithm {
    static constexpr NetworkCollectiveAlgorithm kind = NetworkCollectiveAlgorithm::FecProtected;
};
