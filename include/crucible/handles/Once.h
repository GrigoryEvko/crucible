...
public:
        ...
        requires std::is_invocable_r_v<T, F>
    {
        ...
        // Both specifiers are unconditional rather than derived from
        // F. A derived one reads false for a lambda whose call
        // operator is not itself declared noexcept, and the warning
        // that infers from the body then flags the mismatch.
