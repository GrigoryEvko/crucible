...
private:
    ...
    // Despite the name this does not block. It reads the semaphore value once
    // and compares. A true result says the value had reached signal.value at
    // the moment of that read, which may already be stale when the caller sees
    // it. A false result says either that the value had not reached it, or
    // that the signal does not describe this edge. A caller that needs to wait
    // has to retry.
