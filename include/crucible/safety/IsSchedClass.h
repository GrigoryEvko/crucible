...
template <SchedulerPolicy_v Policy, typename U, std::uint64_t RuntimeNs, std::uint64_t DeadlineNs,
          std::uint64_t PeriodNs>
struct is_sched_class_impl<::crucible::safety::SchedClass<Policy, U, RuntimeNs, DeadlineNs, PeriodNs>>
    : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_sched_class_v = detail::is_sched_class_impl<std::remove_cvref_t<T>>::value;
...
using sched_class_value_t = typename detail::is_sched_class_impl<std::remove_cvref_t<T>>::value_type;
...
struct LookalikeTask {
    int value;
    SchedulerPolicy_v policy;
};
