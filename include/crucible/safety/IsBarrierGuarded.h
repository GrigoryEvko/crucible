...
struct is_barrier_guarded_impl<::crucible::safety::BarrierGuarded<Tier, U>> : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_barrier_guarded_v = detail::is_barrier_guarded_impl<std::remove_cvref_t<T>>::value;
...
using barrier_guarded_value_t = typename detail::is_barrier_guarded_impl<std::remove_cvref_t<T>>::value_type;
...
struct LookalikeBarrier {
    int value;
    BarrierStrength_v tier;
};
