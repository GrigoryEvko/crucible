...
// Capability ceiling for the hardware instruction class that a value's
// producing or consuming kernel issues.
//
// NoneAllowed is the bottom tier: no hardware-specific instruction,
// portable to every backend with no ISA proof, and the only tier
// admissible under strict bit-exact determinism.  PrivilegedMsr is the
// top tier: ring-0 MSR and port I/O.
//
// A consumer that admits a ceiling accepts any tier at or below it.
// widen() moves up the chain only.  Widening down would claim a
// narrower instruction class than the value actually needs, which
...
static_assert(!MsrInt::satisfies<HwInstruction_v::NonDeterministicTsc>,
              ...
              "an MSR kernel exceeds a TSC ceiling.");
...
static_assert(!can_widen<VecInt, HwInstruction_v::Scalar>,
              ...
              "the instruction-legalization gate.");
...
static_assert(!is_portable_legal<VecInt>, "A Vectorizable kernel MUST be REJECTED at the portable-ISA gate — "
                                          "it needs an ISA-availability proof first.");
