...
[[nodiscard]] consteval bool catalog_category_bijection_impl(std::index_sequence<Is...>) noexcept {
    return ((std::is_same_v<std::tuple_element_t<Is, Catalog>, tag_of_t<static_cast<Category>(Is)>>) && ...);
}

[[nodiscard]] consteval bool catalog_category_bijection() noexcept {
    return catalog_category_bijection_impl(std::make_index_sequence<catalog_size>{});
}

static_assert(catalog_category_bijection(), "Catalog tuple ordering drifted from Category enum value ordering. "
...
[[nodiscard]] consteval bool name_of_covers_catalog_impl(std::index_sequence<Is...>) noexcept {
    ...
    return ((name_of(static_cast<Category>(Is)) != sentinel && description_of(static_cast<Category>(Is)) != sentinel
             && remediation_of(static_cast<Category>(Is)) != sentinel)
            && ...);
}

[[nodiscard]] consteval bool name_of_covers_catalog() noexcept {
    return name_of_covers_catalog_impl(std::make_index_sequence<catalog_size>{});
}

static_assert(name_of_covers_catalog(), "name_of / description_of / remediation_of switch is missing an "
...
[[nodiscard]] consteval bool name_of_matches_tag() noexcept {
    return name_of_matches_tag_impl(std::make_index_sequence<catalog_size>{});
}

static_assert(name_of_matches_tag(), "name_of(Category) and tag::name diverge for at least one "
