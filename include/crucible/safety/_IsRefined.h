...
struct is_refined_impl<::crucible::safety::Refined<Pred, T>> : std::true_type {
    using value_type = T;
...
template <auto Pred, typename T>
struct is_refined_impl<::crucible::safety::SealedRefined<Pred, T>> : std::true_type {
    using value_type = T;
    using predicate_type = decltype(Pred);
...
template <typename T>
inline constexpr bool is_refined_v = detail::is_refined_impl<std::remove_cvref_t<T>>::value;
...
using refined_value_t = typename detail::is_refined_impl<std::remove_cvref_t<T>>::value_type;

template <typename T>
    requires is_refined_v<T>
using refined_predicate_type_t = typename detail::is_refined_impl<std::remove_cvref_t<T>>::predicate_type;

template <typename T>
    requires is_refined_v<T>
inline constexpr bool refined_is_sealed_v = detail::is_refined_impl<std::remove_cvref_t<T>>::sealed;

namespace detail::is_refined_self_test {

using R_pos_int = ::crucible::safety::Refined<::crucible::safety::positive, int>;
using R_nonneg_int = ::crucible::safety::Refined<::crucible::safety::non_negative, int>;
using SR_pos_int = ::crucible::safety::SealedRefined<::crucible::safety::positive, int>;
...
static_assert(
    std::is_same_v<refined_predicate_type_t<R_pos_int>, std::remove_cv_t<decltype(::crucible::safety::positive)>>);
