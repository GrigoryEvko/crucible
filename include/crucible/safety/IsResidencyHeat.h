...
struct is_residency_heat_impl<::crucible::safety::ResidencyHeat<Tier, U>> : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_residency_heat_v = detail::is_residency_heat_impl<std::remove_cvref_t<T>>::value;
...
using residency_heat_value_t = typename detail::is_residency_heat_impl<std::remove_cvref_t<T>>::value_type;
...
using RH_int_hot = ::crucible::safety::ResidencyHeat<ResidencyHeatTag_v::Hot, int>;
using RH_double_warm = ::crucible::safety::ResidencyHeat<ResidencyHeatTag_v::Warm, double>;
using RH_int_cold = ::crucible::safety::ResidencyHeat<ResidencyHeatTag_v::Cold, int>;
...
struct LookalikeResidencyHeat {
    int value;
    ResidencyHeatTag_v tier;
};
...
static_assert(residency_heat_tag_v<RH_double_warm> == ResidencyHeatTag_v::Warm);
...
inline bool is_residency_heat_smoke_test() noexcept {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        ...
        ok = ok && (residency_heat_tag_v<RH_int_hot> == ResidencyHeatTag_v::Hot);
    }
    return ok;
}
