...
struct is_opaque_lifetime_impl<::crucible::safety::OpaqueLifetime<Scope, U>> : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_opaque_lifetime_v = detail::is_opaque_lifetime_impl<std::remove_cvref_t<T>>::value;
...
using opaque_lifetime_value_t = typename detail::is_opaque_lifetime_impl<std::remove_cvref_t<T>>::value_type;
...
using OL_int_fleet = ::crucible::safety::OpaqueLifetime<Lifetime_v::PER_FLEET, int>;
using OL_double_request = ::crucible::safety::OpaqueLifetime<Lifetime_v::PER_REQUEST, double>;
using OL_int_program = ::crucible::safety::OpaqueLifetime<Lifetime_v::PER_PROGRAM, int>;
...
static_assert(std::is_same_v<opaque_lifetime_value_t<OL_double_request>, double>);

static_assert(opaque_lifetime_scope_v<OL_int_fleet> == Lifetime_v::PER_FLEET);
static_assert(opaque_lifetime_scope_v<OL_double_request> == Lifetime_v::PER_REQUEST);
static_assert(opaque_lifetime_scope_v<OL_int_program> == Lifetime_v::PER_PROGRAM);
...
inline bool is_opaque_lifetime_smoke_test() noexcept {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        ...
        ok = ok && (opaque_lifetime_scope_v<OL_int_fleet> == Lifetime_v::PER_FLEET);
    }
    return ok;
}
