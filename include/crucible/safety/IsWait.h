...
struct is_wait_impl<::crucible::safety::Wait<Strategy, U>> : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_wait_v = detail::is_wait_impl<std::remove_cvref_t<T>>::value;
...
using wait_value_t = typename detail::is_wait_impl<std::remove_cvref_t<T>>::value_type;

template <typename T>
    requires is_wait_v<T>
inline constexpr WaitStrategy_v wait_strategy_v = detail::is_wait_impl<std::remove_cvref_t<T>>::strategy;
