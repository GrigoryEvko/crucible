...
struct is_vendor_impl<::crucible::safety::Vendor<Backend, U>> : std::true_type {
    using value_type = U;
...
template <typename T>
inline constexpr bool is_vendor_v = detail::is_vendor_impl<std::remove_cvref_t<T>>::value;
...
using vendor_value_t = typename detail::is_vendor_impl<std::remove_cvref_t<T>>::value_type;
...
inline bool is_vendor_smoke_test() noexcept {
    ...
    for (std::size_t i = 0; i < cap; ++i) {
        ...
        ok = ok && (vendor_backend_v<V_int_portable> == VendorBackend_v::Portable);
    }
    return ok;
}
