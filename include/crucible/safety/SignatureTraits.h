...
// The trait is a type-level computation, so exercising it means two
// things.  Calling each witness confirms at link time that the
// signatures the trait reports match the functions as declared.  The
// volatile bound then keeps the trait reads from being folded away.
