...
static_assert(conf_count == 2, "Conf catalog diverged from {Public, Secret}; confirm intent.");
...
static_assert(every_conf_has_name(), "conf_name() switch missing arm for at least one Conf — add the "
...
static_assert(every_at_conf_has_name(), "ConfLattice::At<C>::name() switch missing an arm for at least "
