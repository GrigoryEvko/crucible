...
// EXHAUSTIVE lattice-axiom coverage over (Consistency)³ = 125 triples.
    ...
    template for (constexpr auto ea : enumerators) {
        template for (constexpr auto eb : enumerators) {
            template for (constexpr auto ec : enumerators) {
                if (!verify_bounded_lattice_axioms_at<ConsistencyLattice>(
                        [:ea:], [:eb:], [:ec:])) {
                    return false;
                }
            }
...
// Distributive — chain orders are trivially distributive.  Verified
// at every triple to catch a future refactor that broke distributivity
// by inserting a non-comparable element.
...
#pragma GCC diagnostic ignored "-Wshadow"
    template for (constexpr auto ea : enumerators) {
        template for (constexpr auto eb : enumerators) {
            template for (constexpr auto ec : enumerators) {
                if (!verify_distributive_lattice<ConsistencyLattice>(
                        [:ea:], [:eb:], [:ec:])) {
                    return false;
                }
            }
