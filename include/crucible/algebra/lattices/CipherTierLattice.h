...
static_assert(cipher_tier_tag_count == 3, "CipherTierTag catalog diverged from {Cold, Warm, Hot}; confirm "
...
static_assert(every_cipher_tier_tag_has_name(), "cipher_tier_tag_name() switch missing arm for at least one tier.");
...
static_assert(every_at_cipher_tier_tag_has_name(), "CipherTierLattice::At<T>::name() switch missing an arm.");
