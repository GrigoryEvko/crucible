...
static_assert(kPrefetchLocality >= 0 && kPrefetchLocality <= 3, "FIXY-V-264: prefetch locality must be in [0, 3] — the "
...
struct alignas(crucible::warden::kHugePageBytes) CRUCIBLE_OWNER TraceRing {
    ...
    // drain entry point.
    pre(::crucible::decide::in_range<std::uint32_t>(max_count, std::uint32_t{0}, CAPACITY))
