...
// Runtime ownership for TCP congestion telemetry.
//
// topology/CongestionTelemetry.h owns socket harvesting and deterministic
// aggregation.  rt owns the bounded worker state and observation publication
// rail because congestion drift is a runtime routing-admission input, not a
// topology fact by itself.
...
using CongestionObservationSet =
    std::array<ObservationSnapshot, kCongestionObservationCount>;
using CongestionObservationBatch =
    std::array<Observation, kCongestionObservationCount>;
...
congestion_observations(
    ...
    return CongestionObservationBatch{{
        ...
        make_observation(
            ...
            sequence),
        make_observation(
            ObservationKind::BitsTransferred,
            source,
