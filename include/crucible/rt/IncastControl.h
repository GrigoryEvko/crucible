...
// Runtime ownership for CNT-P incast-control state.
//
// cntp/IncastControl.h owns socket-local DCTCP/RTO primitives.  rt owns the
// bounded per-flow credit controller because receiver-issued fan-in pacing is
// runtime state, not a congestion-control algorithm by itself.
