...
inline void runtime_smoke_test() {
    ...
    WeakRef<int> w_empty{};
    if (w_empty.has_value() || static_cast<bool>(w_empty)) std::abort();
    if (w_empty.try_get() != nullptr) std::abort();
    ...
    WeakRef<int> wa{box};
    WeakRef<int> wb = wa;
    if (!(wa == wb) || wa.try_get() != wb.try_get()) std::abort();

    if (!is_borrowed_v<B_A>) std::abort();
    if (is_borrowed_v<int>) std::abort();
    if (!IsBorrowed<B_cc>) std::abort();
