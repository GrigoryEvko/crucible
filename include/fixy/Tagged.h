...
inline void runtime_smoke_test() {
    ...
    Tagged<int, tags::source::FromUser> u = mint_tagged<tags::source::FromUser>(seed * 6);
    if (u.value() != 42) std::abort();

    auto um = mint_tagged<tags::source::FromUser, int>(seed * 6);
    if (um.value() != 42) std::abort();

    Tagged<int, tags::source::Sanitized> s = std::move(u).template retag<tags::source::Sanitized>();
    if (s.value() != 42) std::abort();

    int extracted = std::move(s).into();
    if (extracted != 42) std::abort();

    Tagged<long, tags::trust::Verified> v = mint_tagged<tags::trust::Verified>(static_cast<long>(seed * seed));
    Tagged<long, tags::trust::Unverified> uv = mint_tagged<tags::trust::Unverified>(static_cast<long>(seed * seed));
    if (v.value() != uv.value()) std::abort();

    Tagged<int, tags::version::V<3>> vt = mint_tagged<tags::version::V<3>>(seed);
    if (vt.value() != 7) std::abort();

    Tagged<int, tags::vessel_trust::FromPytorch> raw = mint_tagged<tags::vessel_trust::FromPytorch>(seed);
    auto validated = std::move(raw).template retag<tags::vessel_trust::Validated>();
    if (validated.value() != 7) std::abort();

    // Portable to x86 is the admitted weakening direction, through the
    // free-function door.
    Tagged<int, tags::source::PortablePinned> portable = mint_tagged<tags::source::PortablePinned>(seed);
    auto x86 = retag<tags::source::X86Pinned>(std::move(portable));
    if (x86.value() != 7) std::abort();

    // The second door: a slot array starts empty.
    Tagged<int, tags::source::RegionOps> slots[4]{};
    if (slots[3].value() != 0) std::abort();

    if (!is_tagged_v<T_int_user>) std::abort();
    if (!is_tagged_v<T_int_db const&>) std::abort();
    if (is_tagged_v<int>) std::abort();
    if (is_tagged_v<LookalikeTagged>) std::abort();
    if (!IsTagged<T_double_user&&>) std::abort();
}
