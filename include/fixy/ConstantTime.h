...
inline void runtime_smoke_test() {
    ...
    // Only the 32-bit and 64-bit widths are exercised.  At uint8_t and
    // uint16_t the body of select promotes through int before
    ...
    // Three corners matter beyond the small-magnitude anchors below:
    // a narrow width, where promotion to int could sign-extend into
    // the shift; a maximal operand, where a truncated subtraction
    // would lose the borrow; and a pair further apart than half the
    // range, which no borrow-free idiom handles.
