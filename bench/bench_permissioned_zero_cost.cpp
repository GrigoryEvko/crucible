...
bench::Report wrapped_mpsc_pop() {
    ...
    auto cons_perm = crucible::safety::mint_permission_root<typename Ch::consumer_tag>();
...
bench::Report bare_grid_push() {
    auto grid = std::make_unique<crucible::concurrent::ShardedSpscGrid<Item, 1, 1, kCap>>();
    Item i = 0;
...
bench::Report wrapped_grid_push() {
    using PG = crucible::concurrent::PermissionedShardedGrid<Item, 1, 1, kCap, GridBenchTag>;
    auto grid = std::make_unique<PG>();
    auto whole = crucible::safety::mint_permission_root<typename PG::whole_tag>();
    auto perms = crucible::safety::mint_grid_permissions<typename PG::whole_tag, 1, 1>(std::move(whole));
...
bench::Report wrapped_deque_push() {
    using PD = crucible::concurrent::PermissionedChaseLevDeque<Item, kCap, DequeBenchTag>;
    auto dq = std::make_unique<PD>();
    auto own_perm = crucible::safety::mint_permission_root<typename PD::owner_tag>();
...
int main() {
    ...
    bench::Compare cmps[] = {
        bench::compare(reports[0], reports[1]),   bench::compare(reports[2], reports[3]),
        bench::compare(reports[4], reports[5]),   bench::compare(reports[6], reports[7]),
        bench::compare(reports[8], reports[9]),   bench::compare(reports[10], reports[11]),
        bench::compare(reports[12], reports[13]), bench::compare(reports[14], reports[15]),
