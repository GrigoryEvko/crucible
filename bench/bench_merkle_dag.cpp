...
TraceEntry make_synthetic_entry(effects::Alloc a, Arena& arena, uint16_t n_in, uint16_t n_out, uint16_t n_scalar,
                                uint8_t ndim) {
    TraceEntry te{};
...
int main(int argc, char* argv[]) {
    ...
    reports.push_back([] {
        ...
        std::vector<ScopeHash> saved_scope(scope_batch.begin(), scope_batch.begin() + n);
        std::vector<CallsiteHash> saved_callsite(callsite_batch.begin(), callsite_batch.begin() + n);
    ...
    for (int i = 1; i < argc; i++) {
        reports.push_back([path = argv[i]] {
            ...
            char label[128];
            std::snprintf(label, sizeof(label), "build_trace (%u ops, from %s)", trace->num_ops, path);
