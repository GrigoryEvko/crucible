...
[[nodiscard]] constexpr std::string_view program_name(int argc, char** argv) {
    ...
    return (argc > 0 && argv != nullptr && argv[0] != nullptr) ? std::string_view{argv[0]}
...
[[nodiscard]] bool parse_depth(const char* text, uint32_t lo, uint32_t hi, uint32_t& out) {
...
[[nodiscard]] const char* arch_label(crucible::vis::Architecture a) {
    switch (a) {
        case crucible::vis::Architecture::UNET:
            return "UNet";
        case crucible::vis::Architecture::VIT:
            return "ViT";
        default:
            return "generic";
    }
...
int main(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "-o") {
            ...
            if (!parse_depth(argv[++i], 1u, 16u, scope_depth)) {
                std::fprintf(stderr, "render_trace: --depth must be an integer in [1, 16], got '%s'\n", argv[i]);
                return 1;
            }
        } else {
    ...
    const char* const output_path = (output_path_cli != nullptr) ? output_path_cli : default_out.c_str();
    ...
    const std::string svg = crucible::vis::render_block_svg(detection, ops, title);
