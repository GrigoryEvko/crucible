...
const Expr* build_deep_tree(effects::Alloc a, ExprPool& pool, int depth) {
    ...
    for (int i = 2; i < depth; ++i) {
        const Expr* z =
            pool.symbol(a, ("z" + std::to_string(i)).c_str(), SymbolId{static_cast<uint32_t>(i)}, NUM_FLAGS);
...
int main() {
    ...
    // ── Pre-bench diagnostic: hash-table load factor after 10 k entries.
    {
        ...
        std::printf("  hash-table load factor: %.1f%% (%zu / %zu entries, arena %zu B)\n\n", load * 100.0,
                    pool.intern_size(), pool.intern_capacity(), pool.arena_bytes());
    }
    ...
    for (int n : {100, 1000, 10000}) {
        reports.push_back([&, n] {
            ...
            char label[80];
            std::snprintf(label, sizeof(label), "integer(%ld)  [hit, pool=%d]", target_v, n);
    ...
    for (int depth : {2, 4, 8}) {
        reports.push_back([&, depth] {
            ...
            char label[80];
            std::snprintf(label, sizeof(label), "build_deep_tree(depth=%d) [all hits]", depth);
    ...
    reports.push_back([&] {
        ...
        return bench::run("mixed 90/10 hit/miss", [&] {
            if (idx % 10 == 0) {
                const Expr* r = pool.mul(a, adds[idx % 100], pool.integer(a, static_cast<int64_t>(idx) + 10000));
                bench::do_not_optimize(r);
            } else {
    ...
    for (size_t cap : {size_t{512}, ExprPool::kDefaultInitialCapacity, size_t{1u} << 16, size_t{1u} << 20}) {
        char label[64];
        std::snprintf(label, sizeof(label), "ExprPool ctor + dtor [init_cap=%zu]", cap);
    ...
    for (size_t n_inserts : {size_t{100}, size_t{1000}, size_t{10000}}) {
        char label[64];
        std::snprintf(label, sizeof(label), "grow sequence: %zu inserts from default", n_inserts);
        reports.push_back([&, n_inserts] {
            return bench::run(label, [&, n_inserts] {
                ExprPool pool(a);  // default init cap
                for (size_t i = 0; i < n_inserts; ++i)
                    bench::do_not_optimize(pool.integer(a, 10000 + static_cast<int64_t>(i)));
            });
        }());
    }
    ...
    for (size_t n_inserts : {size_t{100}, size_t{1000}, size_t{10000}}) {
        char label[64];
        std::snprintf(label, sizeof(label), "reserve(%zu) + %zu inserts", n_inserts, n_inserts);
        reports.push_back([&, n_inserts] {
            return bench::run(label, [&, n_inserts] {
                ExprPool pool(a);
    ...
    for (size_t k = 0; k < 3; ++k) {
        std::printf("  %s  →  %s:\n  ", reports[L - 6 + k].name.c_str(), reports[L - 3 + k].name.c_str());
